...
def ensure_models() -> None:
    ...
    xml_paths = [MODELS_ROOT / name for name in ECG_MODELS]
    bin_names = [name.replace(".xml", ".bin") for name in ECG_MODELS]
    bin_paths = [MODELS_ROOT / name for name in bin_names]
    ...
    hls_xml_paths = [hls_dir / name for name in ECG_MODELS]
    hls_bin_paths = [hls_dir / name for name in bin_names]
    ...
    if not (_all_present(hls_xml_paths) and _all_present(hls_bin_paths)):
        raise RuntimeError("ECG models did not appear within timeout in hls_assets")
...
def validate_assets() -> None:
    ...
    xml_paths = [MODELS_ROOT / name for name in ECG_MODELS]
    bin_paths = [MODELS_ROOT / name.replace(".xml", ".bin") for name in ECG_MODELS]
