...
- Machine IP: `10.223.22.20` (check with `hostname -I`)
...
- Remote: `rtsp://10.223.22.20:8554/north`
...
**Before (local file):**
```
multifilesrc loop=true location=/home/pipeline-server/videos/1122north_h264.ts name=source ! decodebin3 ! videorate ! videoconvert ! video/x-raw,format=BGR,framerate=15/1
```

**After (RTSP source):**
```
urisourcebin uri=rtsp://10.223.22.20:8554/north name=source ! decodebin3 ! videorate ! videoconvert ! video/x-raw,format=BGR,framerate=15/1
```

Replace `10.223.22.20` with the IP of the machine running mediamtx.
