...
def generate_launch_description():
    ...
    twist_param_file = os.path.join(
        get_package_share_directory('wandering_irobot_tutorial'), 'params', 'twist-mux-params.yaml'
    )
    rviz_config_file = os.path.join(
        get_package_share_directory('wandering_irobot_tutorial'),
        'rviz',
        'wandering-irobot-tutorial.rviz',
    )
    ...
    declare_irobot_ns_cmd = DeclareLaunchArgument(
        'irobot_ns', description='iRobot create 3 namespace, set in the web interface of the robot'
    )

    declare_lidar_type_cmd = DeclareLaunchArgument(
        'lidar_type', default_value='a2m8', description='RPLidar model type: a2m8 or a3.'
    )
    ...
    declare_use_joystick_cmd = DeclareLaunchArgument(
        'use_joystick', default_value='false', description='Start nodes for joystick control'
    )
        ...
        PythonLaunchDescriptionSource(
            [
                PathJoinSubstitution(
                    [get_package_share_directory('rplidar_ros'), 'launch', 'rplidar_']
                ),
                lidar_type,
                '_launch.py',
            ]
    ...
    twist_mux_node = Node(
        ...
        parameters=[twist_param_file],
    )

    joy_node = Node(package='joy', executable='joy_node', condition=IfCondition(use_joystick))
    ...
    groundfloor_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                get_package_share_directory('pointcloud_groundfloor_segmentation'),
                'launch',
    ...
    wandering_app_node = Node(
        package='wandering_app', executable='wandering', parameters=[params_file]
    )
