...
: Node("wandering_mapper"),
  initialized_(false),
  transform_tolerance_(1),
  robotRadius_(0),
  global_frame_("map"),
  robot_base_frame_("base_link"),
  occupancyGridTopic_("/map"),
  costMapTopic_("/global_costmap/costmap"),
  shouldStop_(false),
  initRotate_(false),
  shouldResetAll_(false),
  shouldPauseWandering_(false)
{
  ...
  //   this->occupancyGridTopic_, map_qos, std::bind(
  //     &MapEngine::mapCallback, this->mapEngine_, std::placeholders::_1));
  costMapSubscription_ = this->create_subscription<nav_msgs::msg::OccupancyGrid>(
    this->costMapTopic_, map_qos,
    std::bind(&MapEngine::mapCallback, this->mapEngine_, std::placeholders::_1));
...
bool WanderingMapper::areTfFramesPresent()
{
  ...
  if (std::find(ids.begin(), ids.end(), this->global_frame_) != ids.end()) {
    if (!(std::find(ids.begin(), ids.end(), this->robot_base_frame_) != ids.end())) {
      RCLCPP_WARN(
        this->get_logger(), "Frame %s not present in tf list...", this->robot_base_frame_.c_str());
      return false;
    }
  } else {
    RCLCPP_WARN(
      this->get_logger(), "Frame %s not present in tf list...", this->global_frame_.c_str());
    return false;
  }
...
void WanderingMapper::doMap()
{
  ...
  if (!this->mapEngine_->getNextGoalCoord(x, y)) {
    RCLCPP_INFO(
      this->get_logger(),
      "No more coordinates for mapping! Area is either mapped or robot is stuck.");
...
void WanderingMapper::rotate()
{
  ...
  goal->behavior_tree = std::string(
    ament_index_cpp::get_package_share_directory("wandering_app") + "/behavior_trees/spin360.xml");
