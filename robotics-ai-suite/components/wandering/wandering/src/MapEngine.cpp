...
static const std::array<uint8_t, 256> costs_mapping = occup_cost_mapping();

MapEngine::MapEngine(bool useCostMap, double robotRadius)
: robotIMapIndex_(0),
  robotJMapIndex_(0),
  unexploredPrctg_(0),
  freePrctg_(0),
  ...
  logRate_(256),
  mapPrintCounter_(0),
  useCostMap_(useCostMap),
  resetDone_(false),
  storeMapToFile_(false)
{
  this->map_ = std::make_shared<nav_msgs::msg::OccupancyGrid>();
}

MapEngine::~MapEngine() { this->printMap(); }
...
void MapEngine::updateMapInfo()
{
  ...
  uint32_t diffThr = 20;
  if (
    this->map_->info.width < this->costmap_.getSizeInCellsX() &&
    this->map_->info.height < this->costmap_.getSizeInCellsY()) {
    RCLCPP_WARN(
      rclcpp::get_logger("wandering_mapper"),
      "Received map update with smaller dimensions %dx%d, compared to %dx%d.",
      this->map_->info.width, this->map_->info.height, this->costmap_.getSizeInCellsX(),
      this->costmap_.getSizeInCellsY());
    if (
      (this->costmap_.getSizeInCellsX() - this->map_->info.width) > diffThr &&
      (this->costmap_.getSizeInCellsY() - this->map_->info.height) > diffThr) {
  ...
  this->costmap_.resizeMap(
    this->map_->info.width, this->map_->info.height, this->map_->info.resolution,
  ...
  for (uint32_t k = (i - range); k < (i + range); k++) {
    for (uint32_t l = (j - range); l < (j + range); l++) {
      ...
      if (!this->useCostMap_) {
        ...
        if (
          this->visitedMap_[index] ||
          costmap_data[index] > nav2_costmap_2d::INSCRIBED_INFLATED_OBSTACLE) {
          return false;
        }
      }
    }
  ...
  for (i = this->robotIMapIndex_; i < (this->costmap_.getSizeInCellsX() - range); i += range) {
    for (j = range; j < (this->costmap_.getSizeInCellsY() - range); j += range) {
      ...
      if (this->checkAreaForValue(i, j, boxSize, nav2_costmap_2d::FREE_SPACE)) {
        ...
        RCLCPP_INFO(rclcpp::get_logger("wandering_mapper"), "Free cell found (%d, %d).", i, j);
        return true;
      }
    }
  }
...
bool MapEngine::setRobotPose(const geometry_msgs::msg::PoseStamped pose)
{
  ...
  RCLCPP_INFO_EXPRESSION(
    rclcpp::get_logger("wandering_mapper"), shouldLog, "Current pose: (%f, %f), orientation %f.",
    this->robotPose_.pose.position.x, this->robotPose_.pose.position.y,
    this->robotPose_.pose.orientation.w);

  if (!this->costmap_.worldToMap(
        this->robotPose_.pose.position.x, this->robotPose_.pose.position.y, this->robotIMapIndex_,
        this->robotJMapIndex_)) {
...
double MapEngine::getUnexploredPrstg() const { return this->unexploredPrctg_; }
...
void MapEngine::checkMapCoverage()
{
  ...
  RCLCPP_INFO_EXPRESSION(
    rclcpp::get_logger("wandering_mapper"), shouldLog, "Map W: %d H: %d", this->map_->info.width,
    this->map_->info.height);
  RCLCPP_INFO_EXPRESSION(
    rclcpp::get_logger("wandering_mapper"), shouldLog, "Origin position %f %f",
    this->costmap_.getOriginX(), this->costmap_.getOriginY());
  RCLCPP_INFO_EXPRESSION(
    rclcpp::get_logger("wandering_mapper"), shouldLog, "Resolution %f",
  ...
  this->unexploredPrctg_ = (static_cast<double>(unexplored) / static_cast<double>(total)) * 100;
  this->freePrctg_ = (static_cast<double>(freeSpace) / static_cast<double>(total)) * 100;
  this->occupiedPrctg_ = (static_cast<double>(occupiedSpace) / static_cast<double>(total)) * 100;

  RCLCPP_INFO_EXPRESSION(
    rclcpp::get_logger("wandering_mapper"), shouldLog,
    "Space: Free: %.0f%%, Occupied: %.0f%%, Unexplored: %.0f%%", this->freePrctg_,
    this->occupiedPrctg_, this->unexploredPrctg_);
  RCLCPP_INFO_EXPRESSION(
    rclcpp::get_logger("wandering_mapper"), shouldLog, "Robot cell distance %d",
    this->coordOffset_);
  this->logCounter++;
}
...
bool MapEngine::exploreDirections(double & x, double & y, bool unknownArea)
{
  ...
  for (auto & movement : this->movMethodNames_) {
    ...
    if (unknownArea) {
      if (
        this->moveToNearestUnknownCell(
          i, j, this->directions_[directionIndex], this->directions_[directionIndex + 1]) ==
        nav2_costmap_2d::NO_INFORMATION) {
        foundCell = true;
      }
    } else {
      this->moveToFarestFreeCell(
        i, j, this->directions_[directionIndex], this->directions_[directionIndex + 1]);
    ...
    if (foundCell) {
      ...
      if (!this->visitedMap_[index]) {
        ...
        RCLCPP_INFO(
          rclcpp::get_logger("wandering_mapper"), "Move %s at (%d, %d), with coord (%f, %f).",
...
bool MapEngine::getNextGoalCoord(double & x, double & y)
{
  ...
  RCLCPP_INFO(
    rclcpp::get_logger("wandering_mapper"),
    "Farest free non-visited cell not found! Trying to explore unknown space...");
  ...
  RCLCPP_INFO(
    rclcpp::get_logger("wandering_mapper"),
    "Did not find unknown space cell. Trying to find any free non-visited cell...");
  uint32_t i = this->robotIMapIndex_;
  uint32_t j = this->robotJMapIndex_;
  ...
  this->costmap_.mapToWorld(i, j, x, y);
  RCLCPP_INFO(
    rclcpp::get_logger("wandering_mapper"), "Moving robot at (%d, %d), with coord (%f, %f).", i, j,
    x, y);
...
bool MapEngine::shouldMove(uint32_t targetI, uint32_t targetJ)
{
  ...
  uint32_t iDiff =
    std::max(targetI, this->robotIMapIndex_) - std::min(targetI, this->robotIMapIndex_);
  uint32_t jDiff =
    std::max(targetJ, this->robotJMapIndex_) - std::min(targetJ, this->robotJMapIndex_);
...
{
  if (
    i <= (this->costmap_.getSizeInCellsX() - abs(thrI)) &&
    j <= (this->costmap_.getSizeInCellsY() - abs(thrJ)) && i >= abs(thrI) && j >= abs(thrJ)) {
    return true;
  }
