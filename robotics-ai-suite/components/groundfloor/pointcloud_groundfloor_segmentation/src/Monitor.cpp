...
bool Sensor::isCliff(
  const uint32_t rowIndex, const uint32_t colIndex, const float maxInclineVal, bool & runFirstCheck)
{
  bool isCliff = false;
...
uint32_t Sensor::getNumHighInlinePixels(
  ...
  for (uint32_t j = 0; j < cols; j++) {
    ...
    for (uint32_t i = rows - 1; i > 0; i--) {
      ...
      if (std::isnan(currentHeight)) {
        ...
        mLabeledPointCloud[index].label = PointLabel::ABOVE;
        mHeightConfidenceMatrix[index] = std::max(
          std::min(
            (mParameters.ceilingHeight + currentFloorHeight - currentHeight) /
        ...
        if (on_gp && (incline < maxInclineVal) && !cliff) {
          ...
          const uint32_t allowedHighInclinePixels =
            std::ceil(mParameters.acceptableInclineRatio * analysisSample);
          const uint32_t numHighIncline =
            getNumHighInlinePixels(i, j, analysisSample, maxInclineVal);
          ...
          if (on_gp) {
            ...
            if (
              (std::fabs(surfaceHeightMax - currentHeight) <
               std::min(0.1f, mParameters.maxSurfaceHeight)) &&
        ...
        if (!on_gp) {
          mHeightConfidenceMatrix[index] = std::min(
            1.f, (std::fabs(currentFloorHeight - currentHeight) /
                  std::fabs(mParameters.maxSurfaceHeight)));
        }
      } else {
        // This should not happen --> Set Probability to 1 for safety reasons
        mHeightConfidenceMatrix[index] = 1.;
      }
    }
  }
