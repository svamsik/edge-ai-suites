...
: Node("maps_publisher_node")
{
  ...
  pub_fused_map_ =
    create_publisher<visualization_msgs::msg::MarkerArray>("world/fused_map", map_qos);
  pub_occupancy_ =
    create_publisher<visualization_msgs::msg::MarkerArray>("world/occupancy", map_qos);
...
void MapManager::updateFreeOrUnknownSpace(std::shared_ptr<ImageFrame> currFrame)
{
  ...
  // TODO: this implementation may be unnecessarily strict since it does not allow pitch
  {
    ...
    if (
      std::abs(camera_y_axis(0)) > projection_tol || std::abs(camera_y_axis(1)) > projection_tol) {
      ROS_INFO_THROTTLE(
        1, "Non-horizontal camera orientation! Free space modeling may be insufficient.");
    }
  }
...
void MapManager::publish_volumetric_map()
{
  ...
  for (const auto & block : blocks) {
    ...
    for (int i = 0; i < block_side; ++i) {
      for (int j = 0; j < block_side; ++j) {
        ...
        if (dataid >= static_cast<int>(block->side * block->sideSq)) {
          break;
        }
  ...
  addColorToMarkerPoints(
    *fused_marker, config_.p_voxel_color_axis_, config_.p_voxel_color_axis_min_,
  ...
  for (const auto & block : blocks) {
    ...
    for (int i = 0; i < block_side; ++i) {
      for (int j = 0; j < block_side; ++j) {
        ...
        for (int k = 0; k < block_side; ++k, center.z += voxel_dim, dataid += block->sideSq) {
          if (center.z >= config_.p_projection_max_z_ || center.z <= config_.p_projection_min_z_) {
            continue;
          }
          ...
          if (prob > unknown_upper) {
            ogdata = occupied_prob;  // Occupied
          } else if (prob < unknown_lower) {
            if (ogdata == unknown_prob) {
