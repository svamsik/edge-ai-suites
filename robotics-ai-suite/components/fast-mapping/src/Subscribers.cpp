...
: setIntrinsicsCb_(std::move(setIntrinsics)), frame_queue_(frame_queue)
{
  ...
  std::string camera_1_info_topic = node->declare_parameter<std::string>(
    "depth_info_topic", "camera/aligned_depth_to_color/camera_info");

  std::string camera_1_topic = node->declare_parameter<std::string>(
    "depth_topic_1", "camera/aligned_depth_to_color/image_raw");
  std::string camera_2_topic = node->declare_parameter<std::string>(
    "depth_topic_2", "camera_left/aligned_depth_to_color/image_raw");
  std::string camera_3_topic = node->declare_parameter<std::string>(
    "depth_topic_3", "camera_right/aligned_depth_to_color/image_raw");
  std::string camera_4_topic = node->declare_parameter<std::string>(
    "depth_topic_4", "camera_rear/aligned_depth_to_color/image_raw");
  ...
  if (depthCameras_ == 4) {
    sync_4_.reset(new sync4(
      syncPolicy4(30), sub_caminfo_, sub_depth_1_, sub_depth_2_, sub_depth_3_, sub_depth_4_));
    sync_4_->registerCallback(
      std::bind(
        &CameraSubscribers::depthImgCallback4, this, std::placeholders::_1, std::placeholders::_2,
        std::placeholders::_3, std::placeholders::_4, std::placeholders::_5));
  } else if (depthCameras_ == 3) {
    sync_3_.reset(
      new sync3(syncPolicy3(30), sub_caminfo_, sub_depth_1_, sub_depth_2_, sub_depth_3_));
    sync_3_->registerCallback(
      std::bind(
        &CameraSubscribers::depthImgCallback3, this, std::placeholders::_1, std::placeholders::_2,
    ...
    sync_2_->registerCallback(
      std::bind(
        &CameraSubscribers::depthImgCallback2, this, std::placeholders::_1, std::placeholders::_2,
        std::placeholders::_3));
  } else if (depthCameras_ == 1) {
    sync_1_.reset(new sync1(syncPolicy1(100), sub_caminfo_, sub_depth_1_));
    sync_1_->registerCallback(
      std::bind(
        &CameraSubscribers::depthImgCallback1, this, std::placeholders::_1, std::placeholders::_2));
  }
}
