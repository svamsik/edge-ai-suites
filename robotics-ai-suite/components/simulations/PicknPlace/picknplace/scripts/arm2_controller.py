...
class ApproachObject(State):
    ...
    def execute(self, userdata):
        target_pos = utils.entity_location(
            self.robot_controller, self.robot_controller.get_parameter('object_name').value, 'arm2'
        )
        ...
        self.robot_controller.moveit2_robot0.move_to_pose(
            position=target_pos.position, quat_xyzw=quat_xyzw, cartesian=True
        )
        self.robot_controller.moveit2_robot0.wait_until_executed()
        return 'approached'
...
class PlaceObject(State):
    ...
    def execute(self, userdata):
        ...
        self.robot_controller.moveit2_robot0.move_to_pose(
            position=[
                self.robot_controller.last_pose.position.x,
                self.robot_controller.last_pose.position.y,
                0.35,
            ],
            quat_xyzw=[1.0, 0.0, 0.0, 0.0],
            cartesian=True,
        ...
        self.robot_controller.moveit2_robot0.move_to_pose(
            position=self.turtlebot_pos, quat_xyzw=[1.0, 0.0, 0.0, 0.0], cartesian=True
        )
        self.robot_controller.moveit2_robot0.wait_until_executed()
        self.robot_controller.gripper_off()
...
def run_smach(client):
    ...
    with sm:
        ...
        StateMachine.add(
            'PLACE', PlaceObject(client), transitions={'home': 'HOME', 'failed': 'HOME'}
        )
