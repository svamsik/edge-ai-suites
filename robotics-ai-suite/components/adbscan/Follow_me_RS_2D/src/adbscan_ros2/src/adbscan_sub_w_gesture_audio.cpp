...
{
public:
  MinimalSubscriber() : Node("adbscan_sub_w_gesture_audio_node")
  {
    ...
    this->declare_parameter("audio_enable");
    ...
    this->declare_parameter("audio_enable", true);
    ...
    rclcpp::Parameter audio_enable_param = this->get_parameter("audio_enable");
    ...
    audio_enable_ = audio_enable_param.as_bool();
    ...
    audio_category_ = std::string("no_action");

    if (Verbose_) {
      RCLCPP_INFO(this->get_logger(), "adbscan_sub_w_gesture_audio_node started; ");
    }
    ...
    if (audio_enable_) {
      subscription_audio_ = this->create_subscription<follow_me_interfaces::msg::AudioCommand>(
        "audio_command", defalt_qos,
        std::bind(&MinimalSubscriber::audio_callback, this, std::placeholders::_1));
    }
...
private:
  ...
  void audio_callback(const follow_me_interfaces::msg::AudioCommand::SharedPtr _msg)
  {
    // RCLCPP_INFO(this->get_logger(), "Subscribing to audio_command topic");
    this->audio_category_ = _msg->audio_command;
  }
  ...
  void topic_2D_callback(const sensor_msgs::msg::LaserScan::SharedPtr _msg)
  {
    ...
    if (result == 0) {
      ...
      new_target_loc = doDBSCAN(
        p_Data, point_list.size() * sizeof(LiDAR_data_4D_t), 2, this->target_loc, nbr_frame_blocked,
        obstacles);
      // Publish the new_target_loc
      auto target_loc_msg = geometry_msgs::msg::Point();
      target_loc_msg.x = new_target_loc.x;
      target_loc_msg.y = new_target_loc.y;
      target_loc_msg.z = new_target_loc.z;
      publisher_target_loc_->publish(target_loc_msg);

      cout << "Gesture = " << this->gesture_category_ << endl;
      cout << "Audio = " << this->audio_category_ << endl;

      if (
        new_target_loc.x > 0 &&
        (this->gesture_category_ == "Thumb_Up" || this->audio_category_ == "start")) {
        this->motor_initiated_ = true;
      }

      if (
        this->motor_initiated_ && new_target_loc.x > 0 && this->gesture_category_ != "Thumb_Down" &&
        this->audio_category_ != "stop") {
      ...
      // Publish motor state
      auto motor_state_msg = std_msgs::msg::Bool();
      motor_state_msg.data = this->motor_initiated_;
      publisher_motor_state_->publish(motor_state_msg);
    ...
    cout << "================================================================================"
         << std::endl;
    cout << "Total execution time (From reading pointcloud data to publishing twist msg)= "
         << diff(t1, t2) * 1000 << " [ms]" << std::endl;
    cout << "================================================================================"
         << std::endl;
    count_++;
  }
  ...
  void topic_3D_callback(const sensor_msgs::msg::PointCloud2::SharedPtr _msg)
  {
    ...
    int result = Convert_vec_pData(point_list, p_Data);
    if (result == 0) {
      // convert successful, run ADBSCAN to obtain a list of objects
      vector<Obstacle> obstacles;
      if (Lidar_type_ == "3D") {
        new_target_loc = doDBSCAN(
          p_Data, point_list.size() * sizeof(LiDAR_data_4D_t), 3, this->target_loc,
          nbr_frame_blocked, obstacles);
      } else if (Lidar_type_ == "RS") {
        // subsampling but not filtering
        new_target_loc = doDBSCAN(
          p_Data, point_list.size() * sizeof(LiDAR_data_4D_t), 4, this->target_loc,
          nbr_frame_blocked, obstacles);
      }
  ...
  void publish_message()
  {
    ...
    if (target_found == false) {
      // target not found, set linear and angular velocity to 0.0
  ...
  string
    audio_category_;  // supported commands: "no_action", "start", "stop", "speed", "slow", "right", "left"
  ...
  rclcpp::Subscription<follow_me_interfaces::msg::AudioCommand>::SharedPtr subscription_audio_;
  // rclcpp::Publisher<nav2_dynamic_msgs::msg::ObstacleArray>::SharedPtr publisher_;
  rclcpp::Publisher<geometry_msgs::msg::Twist>::SharedPtr publisher_;
  rclcpp::Publisher<geometry_msgs::msg::Point>::SharedPtr publisher_target_loc_;
  // the following is subscribed by the text_to_speech node
  rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr publisher_motor_state_;
