...
Point_xyz doDBSCAN(
  void * p_Data, int LiDAR_data_size, int dimension, Point_xyz target_loc, int & nbr_frame_blocked,
  vector<Obstacle> & obstacle_list)
{
  ...
  if (true) {
    ...
    cout << "target_loc x,y,z: " << target_loc.x << "," << target_loc.y << "," << target_loc.z
         << endl;
    cout << "new_target_loc x,y,z: " << new_target_loc.x << "," << new_target_loc.y << ","
         << new_target_loc.z << endl;
