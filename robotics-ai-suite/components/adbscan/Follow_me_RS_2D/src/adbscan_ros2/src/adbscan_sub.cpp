...
private:
  void topic_2D_callback(const sensor_msgs::msg::LaserScan::SharedPtr _msg)
  {
    ...
    if (result == 0) {
      ...
      if (new_target_loc.x > 0) {
        // target found; if no target found, new target_loc.x was set to -1.0
        this->target_loc = new_target_loc;
        this->target_found = true;

        if (nbr_frame_blocked == 0) {
          // used up 5 blocked frames, reset
        ...
        // 12/21/2022 no target found, this->target_loc := (0.5, 0, 0), no twist msg published
        this->target_loc.x = CONFIG_PARAMS.init_tgt_loc;  // 0.5;
        this->target_loc.y = 0;
        this->target_loc.z = 0;
        this->target_found = false;
  ...
  void topic_3D_callback(const sensor_msgs::msg::PointCloud2::SharedPtr _msg)
  {
    ...
    if (result == 0) {
      ...
      if (new_target_loc.x > 0) {
        // target found; if no target found, new target_loc.x was set to -1
        this->target_loc = new_target_loc;
        this->target_found = true;
      } else {
        this->target_loc.x = CONFIG_PARAMS.init_tgt_loc;  // 0.5;
