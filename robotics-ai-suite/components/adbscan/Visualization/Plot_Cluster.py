...
def print_frame(frame):  # print all info in a frame
    ...
    for cluster in frame.cluster_list:
        ...
        output_str = (
            str(cluster.cluster_nbr)
            + ','
            + str(cluster.centroid[0])
            + ','
            + str(cluster.centroid[1])
            + ','
            + str(cluster.centroid[2])
            + '\n'
        )
    ...
    for cluster in clusters:  # one iteration for one cluster
        ...
        for idx in range(len(clusters)):
            ...
            if len(points) > 20:
                ...
                ax.text(
                    cluster.centroid[0] + 0.5,
                    cluster.centroid[1] - 0.9,
                    cluster.centroid[2],
                    cluster.cluster_nbr,
                    color='#b40426',
                    size=16,
...
if __name__ == '__main__':
    ...
    # frame_files.append('2011-09-26-drive-0005-sync-0-9_DBSCAN_results.txt')
    # add all consecutive input frame
        ...
        for line in frame_file:
            ...
            if cluster_nbr > 0:  # valid cluster
                pt = (
                    ...
                    float(col[2]),
                    float(col[3]),
                )  # point value, to be appended to point_list
