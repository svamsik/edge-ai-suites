...
#ifdef USE_ONEAPI
#include <pcl/kdtree/kdtree_flann.h>         // NOLINT
#include <pcl/oneapi/kdtree/kdtree_flann.h>  // NOLINT
...
#ifdef USE_ONEAPI
void get_active_map_using_oneapi_search_octree(
  LiDAR_data_3D_t * x, FPDS_t * esp_array, vector<int> * neighbors, INT32_t num_points)
{
  cout << "Using get_active_map_using_oneapi_search_octree\n";
  // local
  const int max_answers = 333;

  std::vector<float> radiuses;
  std::vector<pcl::oneapi::Octree::PointType> cloud;
  cloud.resize(num_points);
  radiuses.resize(num_points);
  // transform input data into PointCloud format;
  for (size_t i = 0; i < cloud.size(); ++i) {
    (cloud)[i].x = x[i].val[0];
    (cloud)[i].y = x[i].val[1];
    (cloud)[i].z = x[i].val[2];
    radiuses[i] = esp_array[i];
  }

  pcl::oneapi::Octree::PointCloud cloud_device;
  cloud_device.upload(cloud);
  ...
  result_device.data.download(downloaded_buffer);
  for (std::size_t i = 0; i < num_points; ++i) {
    int beg = i * max_answers;
    int end = beg + sizes[i];
    neighbors[i].assign(downloaded_buffer.begin() + beg, downloaded_buffer.begin() + end);
  }
}

void get_active_map_using_oneapi_search_kdtree(
  LiDAR_data_3D_t * x, FPDS_t * esp_array, vector<int> * neighbors, INT32_t num_points)
{
  std::cout << "Using get_active_map_using_oneapi_search_kdtree" << std::endl;
  // local
  ...
  // create kd-tree
  pcl::oneapi::KdTreeFLANN<pcl::PointXYZ> s_kdtree_oneapi(false);
  cloud->width = cloud->size();
  cloud->height = 1;
  s_kdtree_oneapi.setInputCloud(cloud);

  // split into sub-clouds
  std::unordered_map<float, std::pair<pcl::PointCloud<pcl::PointXYZ>, vector<int>>> list_clouds;
  std::set<float> unique_radius;
  bool is_unique_radius_greater_1 = true;

  for (auto i = 0; i < num_points; ++i) {
    unique_radius.insert(esp_array[i]);
  }

  if (unique_radius.size() > 1) {
    for (auto i = 0; i < num_points; ++i) {
      if (list_clouds.find(esp_array[i]) == list_clouds.end()) {
        pcl::PointCloud<pcl::PointXYZ> new_cloud;
        vector<int> new_indices;
        list_clouds[esp_array[i]] = make_pair(new_cloud, new_indices);
      }
      list_clouds[esp_array[i]].first.emplace_back((*cloud)[i].x, (*cloud)[i].y, (*cloud)[i].z);
      list_clouds[esp_array[i]].second.push_back(i);
    }
  } else {
    // If vertor is empty that indicates need to use default indices
    is_unique_radius_greater_1 = false;
    vector<int> new_indices;
    list_clouds[*unique_radius.begin()] = make_pair(*cloud, new_indices);
  }

  // run radius search
  for (auto it = list_clouds.begin(); it != list_clouds.end(); ++it) {
    std::vector<pcl::Indices> k_indices;
    std::vector<std::vector<float>> k_distances;
    auto radius = it->first;

    it->second.first.width = it->second.first.size();
    it->second.first.height = 1;

    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_ptr(
      new pcl::PointCloud<pcl::PointXYZ>(it->second.first));
    s_kdtree_oneapi.radiusSearch(cloud_ptr, radius, k_indices, k_distances);

    // Update neighbors
    for (auto i = 0; i < k_indices.size(); ++i) {
      auto index = is_unique_radius_greater_1 ? it->second.second[i] : i;
      neighbors[index].assign(k_indices[i].begin(), k_indices[i].end());
    }
  }
}
  ...
  LiDAR_data_3D_t * x, FPDS_t * esp_array, vector<int> * neighbors, INT32_t num_points)
{
  ...
  for (size_t i = 0; i < num_points; i++) {
    ...
    if (num_of_neighbors > 0) {
      // found neighboring points
      // cout << "num_of_neighbors = " << num_of_neighbors << endl;

      // j is point in neighborhood
      for (size_t j = 0; j < num_of_neighbors; ++j) {
...
/*-----------------------------------------------------------------------*
 * \brief This function is used to add an item to the item list
 *-----------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------*
 * \brief This function is used to remove an item from the item list
 *-----------------------------------------------------------------------*/
...
void setClassTouched(
  INT32_t oclass[], UINT8_t touched[], ADBSCAN_list_t * p_list, ADBSCAN_list_t * p_list1,
  INT32_t value)
{
  ADBSCAN_list_item_t * p_item = p_list->pFirst;
  int result;
...
void dbscan_adaptiveK(
  LiDAR_data_3D_t * x, FPDS_t * k_array, FPDS_t * esp_array, INT32_t * oclass, INT32_t * type,
  INT32_t num_points, std::string oneapi_library_name)
{
  UINT8_t touched[num_points];  // NOLINT
  FPDS_t D[num_points];
  FPDS_t k, esp;
  LiDAR_data_3D_t ob;
  INT32_t length;
...
{
  UINT8_t touched[num_points];  // NOLINT
  INT32_t active[num_points];   // NOLINT
  INT32_t active_count;
  INT32_t active_count_at[num_points];  // NOLINT KDTree
  INT32_t active1[num_points];          // NOLINT
  ...
  // #if defined(USE_ONEAPI_SEARCH_KDTREE)
  //   get_active_map_using_oneapi_search_kdtree(x, esp_array, neighbors, num_points);
  // #elif defined(USE_ONEAPI_SEARCH_OCTREE)
  //   get_active_map_using_oneapi_search_octree(x, esp_array, neighbors, num_points);
  // #else
  //   get_active_map(x, esp_array, neighbors, num_points);
  // #endif

  if (oneapi_library_name == "oneapi_kdtree") {
    get_active_map_using_oneapi_search_kdtree(x, esp_array, neighbors, num_points);
  } else if (oneapi_library_name == "oneapi_octree") {
    get_active_map_using_oneapi_search_octree(x, esp_array, neighbors, num_points);
  } else {
    if (oneapi_library_name != "pcl_kdtree") {
      std::cout << "Invalid parameter for oneapi_library: " << oneapi_library_name
                << ", Supported options: oneapi_kdtree | oneapi_octree | pcl_kdtree" << std::endl;
      std::cout << "Using default option: pcl_kdtree (Standard PCL library)..." << std::endl;
    }
    get_active_map(x, esp_array, neighbors, num_points);
  }
  ...
  // cout << "run time in micro s:" << duration.count() << endl;

  // Group points into objects
  memset(touched, 0, sizeof(UINT8_t) * num_points);
  ...
  for (int i = 0; i < num_points; i++) {
    if (touched[i] == 0) {
      ...
      std::queue<INT32_t> active_q;

      memset(active, 0, sizeof(INT32_t) * num_points);
      ...
      if ((length > 1) && (length < k + 1)) {
        ...
        while (!active_q.empty()) {
          ...
          // look up KDTree to find neighboring points
          length = neighbors[ind].size();

          if (length > 1) {
            type[ind] = (length >= k + 1) ? 1 : 0;

            for (int m = 0; m < length; ++m) {
              auto j = neighbors[ind][m];
              {
                oclass[j] = no;
...
void print_device_info(std::vector<cl::Device> * DeviceList)
{
  ...
  for (int i = 0; i < DeviceList->size(); i++) {
    ...
    std::cout << "Is Device Available?: " << DeviceList->at(i).getInfo<CL_DEVICE_AVAILABLE>()
              << "\n";
    std::cout << "Is Device Little Endian?: "
              << DeviceList->at(i).getInfo<CL_DEVICE_ENDIAN_LITTLE>() << "\n";
    std::cout << "Device Max Compute Units: "
              << DeviceList->at(i).getInfo<CL_DEVICE_MAX_COMPUTE_UNITS>() << "\n";
    std::cout << "Device Max Work Item Dimensions: "
              << DeviceList->at(i).getInfo<CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS>() << "\n";
    std::cout << "Device Max Work Group Size: "
              << DeviceList->at(i).getInfo<CL_DEVICE_MAX_WORK_GROUP_SIZE>() << "\n";
    std::cout << "Device Max Frequency: "
              << DeviceList->at(i).getInfo<CL_DEVICE_MAX_CLOCK_FREQUENCY>() << "\n";
    std::cout << "Device Max Mem Alloc Size: "
              << DeviceList->at(i).getInfo<CL_DEVICE_MAX_MEM_ALLOC_SIZE>() << "\n\n";
  }
}

void fill_generate(
  cl_float X[], cl_float Y[], cl_float Z[], cl_float LO, cl_float HI, size_t vectorSize)
{
...
bool verification(float X[], float Y[], float Z[], float D[], float CalcD[], size_t vectorSize)
{
  ...
  for (int i = 0; i < vectorSize - 4; i++) {
    if (fabs(CalcD[i] - D[i]) > EPSILON) {
      printf(
        "\nVERIFICATION FAILED! index %d, X:%f, Y:%f, Z:%f, OpenCL Result:%f != Result %f)", i,
        X[i], Y[i], Z[i], D[i], CalcD[i]);
  ...
  printf("------------------------------------\n");
  for (int i = 0; i < static_cast<int>(vectorSize); i = i + (static_cast<int>(vectorSize)) / 5) {
    printf(
      "Index %d: Input 1 is %f, Input 2 is %f, Input 3 is %f, Result is %f\n", i,
      (reinterpret_cast<float *>(X))[i], (reinterpret_cast<float *>(Y))[i],
      (reinterpret_cast<float *>(Z))[i], (reinterpret_cast<float *>(D))[i]);
  }
...
void dbscan_adaptiveK_fpga_init(ADBScan_fpga_context_t * pContext, int vectorSize)
{
  ...
  pContext->aocx_stream = std::ifstream(
    "../Fpga/OpenCL/dbscan_adaptiveK_hw.aocx", std::ios::in | std::ios::binary);
  checkErr(pContext->aocx_stream.is_open() ? CL_SUCCESS : -1, "dbscan_adaptiveK_hw.aocx");
  pContext->prog = std::string(
    std::istreambuf_iterator<char>(pContext->aocx_stream), (std::istreambuf_iterator<char>()));
  pContext->mybinaries = cl::Program::Binaries(
    1, std::make_pair(pContext->prog.c_str(), pContext->prog.length() + 1));
...
void dbscan_adaptiveK_fpga(
  LiDAR_data_3D_t * x, FPDS_t * k_array, FPDS_t * esp_array, INT32_t * oclass, INT32_t * type,
  INT32_t num_points, INT32_t num_iter)
{
  ...
  cout << "FPGA: Buffers write time :" << diff(time1, time2).tv_sec << "."
       << diff(time1, time2).tv_nsec << " [s]" << endl;
#endif
  ...
  for (int i = 0; i < num_iter; i++) {
    ...
    clock_gettime(clk_id, &time1);
    // Launch Kernel
    // err=myqueue.enqueueNDRangeKernel(
    //   kernel, cl::NullRange, cl::NDRange(vectorSize), cl::NDRange(workSize));
