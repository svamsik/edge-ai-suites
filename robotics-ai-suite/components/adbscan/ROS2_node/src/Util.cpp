...
Point calc_centroid(vector<Point> point_list)
{
  ...
  return Point(
    total_x / static_cast<double>(num_pt), total_y / static_cast<double>(num_pt),
    total_z / static_cast<double>(num_pt));
}

// given a vector of point(x,y,z, cluster_id), return a vector of clusters
// input is a vector of points (x,y,z).
vector<Cluster> get_clusters(vector<Point> points_in_frame)
{
  vector<Cluster> clusters;  // empty cluster, to be filled

  // for each input point
  for (unsigned i = 0; i < points_in_frame.size(); i++) {
    Point cur_point(
      points_in_frame[i].x, points_in_frame[i].y, points_in_frame[i].z,
      points_in_frame[i].cluster_id);
    int cluster_id = cur_point.cluster_id;
    // cout << "cur_point.cluster_id: " << cluster_id << endl;
    if (cluster_id > 0) {
      // valid cluster
      if (clusters.size() == 0) {
        // empty cluster list, first cluster in the frame
        ...
        // traverse the point_list to find same cluster number; if cluster found, append point to it
        bool cluster_nbr_found = false;

        // for each cluster
        for (unsigned j = 0; j < clusters.size(); j++) {
          if (clusters[j].cluster_id == cluster_id) {
            // cluster found
