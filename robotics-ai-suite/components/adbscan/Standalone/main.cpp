...
ADBSCAN_Result_t load_config_params(std::string file_name, ADBSCAN_params_t & adbscan_params)
{
  ...
  if (p_inFile == NULL) {
    ...
    flags = SUBSAMPLE_RATIO_BIT | X_FILTER_BACK_BIT | Y_FILTER_LEFT_BIT | Y_FILTER_RIGHT_BIT |
            Z_FILTER_BIT | GROUND_REMOVAL_BIT;
    ...
    if (flags != 0) {
      ...
      if (flags & SUBSAMPLE_RATIO_BIT) {
        std::cout << "   subsample_ratio" << std::endl;
      }
      if (flags & X_FILTER_BACK_BIT) {
        std::cout << "   x_filter_back" << std::endl;
      }
      if (flags & Y_FILTER_LEFT_BIT) {
        std::cout << "   y_filter_left" << std::endl;
      }
      if (flags & Y_FILTER_RIGHT_BIT) {
        std::cout << "   y_filter_right" << std::endl;
      }
      if (flags & Z_FILTER_BIT) {
        std::cout << "   z_filter" << std::endl;
      }
      if (flags & GROUND_REMOVAL_BIT) {
        std::cout << "   Z_based_ground_removal" << std::endl;
      }
    }
  }
...
ADBSCAN_Result_t load_ROS2_LiDAR_data(
  std::string file_name, LiDAR_data_4D_t *& p_LiDAR_data, int & LiDAR_data_size)
{
  ADBSCAN_Result_t result = ADBSCAN_SUCCESS;
  FILE * p_inFile;
  const char * pInFileName = file_name.c_str();
  ...
  p_LiDAR_data = (LiDAR_data_4D_t *)malloc(num_points * sizeof(LiDAR_data_4D_t));
  for (int i = 0; i < num_points; i++) {
    // need to cast into LiDAR_data_4D_t
      ...
      if (result == ADBSCAN_SUCCESS) {
        ...
        if (result == ADBSCAN_SUCCESS) {
          for (int i = 0; i < num_points; i++) {
            if (p_outFile) {
              fprintf(
                p_outFile, "%d %f %f %f %d %d \n", i + 1, xyz_n[i].val[0], xyz_n[i].val[1],
                xyz_n[i].val[2], p_class[i], p_type[i]);
            } else {
              std::cout << i << " " << xyz_n[i].val[0] << " " << xyz_n[i].val[1] << " "
                        << xyz_n[i].val[2] << " " << p_class[i] << " " << p_type[i] << std::endl;
            }
