...
univloc_global_optimization_module::univloc_global_optimization_module(
  ...
  const bool fix_scale, double front_rear_camera_constraint_thr, int iteration_times,
  bool segment_optimize, bool correct_loop)
: global_optimization_module(
    map_db, bow_db, bow_vocab, fix_scale, front_rear_camera_constraint_thr, iteration_times,
    segment_optimize),
...
void univloc_global_optimization_module::Local_BA_for_merging_map(
  std::vector<data::keyframe *> vec_adjust_keyframes,
  std::vector<data::keyframe *> vec_fixed_keyframes)
{
  ...
  spdlog::info(
    "Merging map local BA involved adjust keyframes num: {}", vec_adjust_keyframes.size());
  ...
  g2o::se3::shot_vertex_container keyfrm_vtx_container(
    0, vec_adjust_keyframes.size() + vec_fixed_keyframes.size());
  ...
  // landmark vertex
  g2o::landmark_vertex_container lm_vtx_container(
    keyfrm_vtx_container.get_max_vertex_id() + 1, lms.size());

  // reprojection edge
  using reproj_edge_wrapper = g2o::se3::reproj_edge_wrapper<data::keyframe>;
  std::vector<reproj_edge_wrapper> reproj_edge_wraps;
  reproj_edge_wraps.reserve(
    (vec_adjust_keyframes.size() + vec_fixed_keyframes.size()) * lms.size());
  ...
  for (auto & id_lm : lms) {
    ...
    for (const auto & obs : observations) {
      ...
      const auto keyfrm_vtx = keyfrm_vtx_container.get_vertex(
        keyfrm);  // If we don't use all the keyframes, here should be changed!
      if (!keyfrm_vtx) {
        continue;
      }
      ...
      auto reproj_edge_wrap = reproj_edge_wrapper(
        keyfrm, keyfrm_vtx, lm, lm_vtx, idx, undist_keypt.pt.x, undist_keypt.pt.y, x_right,
  ...
  for (auto & reproj_edge_wrap : reproj_edge_wraps) {
    ...
    if (reproj_edge_wrap.is_monocular_) {
      if (chi_sq_2D < edge->chi2() || !reproj_edge_wrap.depth_is_positive()) {
        outlier_observations.emplace_back(
          std::make_pair(reproj_edge_wrap.shot_, reproj_edge_wrap.lm_));
      }
    } else {
      if (chi_sq_3D < edge->chi2() || !reproj_edge_wrap.depth_is_positive()) {
        outlier_observations.emplace_back(
  ...
  spdlog::info(
    "Successfully excute local map for map merging!, cost time {} ms",
...
void univloc_global_optimization_module::merge_map_via_loop()
{
  ...
  for (const auto & opt_client_thread : thread_for_optimization_client_) {
    if (
      opt_client_thread.second && graph_optimizers_client_[opt_client_thread.first]->is_running() &&
      (graph_optimizers_client_[opt_client_thread.first]->get_map_id() ==
         cur_keyfrm_->get_map_id() ||
       graph_optimizers_client_[opt_client_thread.first]->get_map_id() ==
         merge_keyfrm->get_map_id())) {
  ...
  if (
    thread_for_optimization_map_[cur_keyfrm_->get_map_id()] &&
  ...
  if (
    thread_for_optimization_map_[merge_keyfrm->get_map_id()] &&
  ...
  spdlog::info(
    "current_keyframeID: {}, mapID {}, ClientID: {}", cur_keyfrm_->id_, cur_keyfrm_->get_map_id(),
    cur_keyfrm_->client_id_);
  spdlog::info(
    "merge_keyframeID: {}, mapID {}, ClientID: {}", merge_keyfrm->id_, merge_keyfrm->get_map_id(),
  ...
  spdlog::debug(
    "merge_keyfrm get_map_id: {}, relative client map id : {}", merge_keyfrm->get_map_id(),
  ...
  if (false && merge_keyfrm->get_map_id() != server_->get_running_client_map_id(merge_keyfrm)) {
    ...
    transform_from_loopframe_to_currentframe = merge_keyfrm->get_cam_pose_inv() *
                                               transform_from_loopframe_to_currentframe *
                                               cur_keyfrm_->get_cam_pose();

    g2o::Sim3 T_transform(
      transform_from_loopframe_to_currentframe.block(0, 0, 3, 3),
      transform_from_loopframe_to_currentframe.block(0, 3, 3, 1), 1.0);

    transform_map_via_sim3(
      merge_keyfrm, Temp_transform, Sim3s_nw_before_correction, Sim3s_nw_after_correction);

    const auto curr_match_lms_observed_in_cand =
      loop_detector_->current_matched_landmarks_observed_in_candidate();
    ...
    // Transform map
    transform_map_via_sim3(
      Sim3s_nw_before_correction, Sim3s_nw_after_correction,
    ...
    // The transform from the world coordinate of the reference map to the image coordinate
    // of the map to be aligned.
    Mat44_t Tcw_aligned_ref =
      util::converter::to_eigen_mat(loop_detector_->get_Sim3_world_to_current());
    // The transform from the world coordinate of the reference map to the world coordinate
    // of the map to be aligned.
  ...
  if (!map_db_->is_keyframe_loaded(cur_keyfrm_)) {
    running_clients.emplace(cur_keyfrm_->client_id_);
  }
  if (!map_db_->is_keyframe_loaded(merge_keyfrm)) {
    running_clients.emplace(merge_keyfrm->client_id_);
  }
...
void univloc_global_optimization_module::merge_map_via_front_rear_camera_constraint()
{
  ...
  if (graph_optimizers_client_[thread_idx]) {
    if (thread_for_optimization_client_[thread_idx]) {
      spdlog::debug(
        "merge_map_via_front_rear_camera_constraint: Waiting for last graph_optimizers_client "
        "thread exiting!");
  ...
  spdlog::debug(
    "merge_keyfrm map id: {},  cur_keyfrm map id: {}", merge_keyfrm_map, cur_keyfrm_map);
  ...
  Mat44_t transform_from_cur_to_constraint = larger_map_client_id < smaller_map_client_id
                                               ? constrait_pose.matrix()
                                               : constrait_pose.inverse().matrix();

  if (cur_keyfrm_map > merge_keyfrm_map) {
    transform_from_cur_to_constraint = cur_keyfrm_->get_cam_pose_inv() *
                                       transform_from_cur_to_constraint *
                                       merge_keyfrm->get_cam_pose();
  } else {
    transform_from_cur_to_constraint = merge_keyfrm->get_cam_pose_inv() *
                                       transform_from_cur_to_constraint *
                                       cur_keyfrm_->get_cam_pose();
  }
  ...
  if (cur_keyfrm_map > merge_keyfrm_map) {
    transform_map_via_sim3(
      cur_keyfrm_, T_transform, Sim3s_nw_before_correction, Sim3s_nw_after_correction);
  } else {
    spdlog::debug("Transform merge_keyfrm map to cur_keyfrm_ map!");
    transform_map_via_sim3(
      merge_keyfrm, T_transform, Sim3s_nw_before_correction, Sim3s_nw_after_correction);
  }
  ...
  Mat44_t error =
    constrain_pose_m.inverse() * cur_keyfrm_->get_cam_pose() * merge_keyfrm->get_cam_pose_inv();
  ...
  std::vector<data::keyframe *> keyframes =
    map_db_->get_all_keyframes(changed_map_id, changed_map_id);
  std::vector<data::landmark *> landmarks =
    map_db_->get_all_landmarks(changed_map_id, changed_map_id);
  for (auto & kf : keyframes) {
    kf->set_map_id(merged_map_id);
  }
  for (auto & lm : landmarks) {
  ...
  server_->post_process_after_loop(
    cur_keyfrm_->client_id_, merge_keyfrm->client_id_, cur_keyfrm_map, merge_keyfrm_map);
}
...
bool univloc_global_optimization_module::detect_front_rear_camera_constraint()
{
  ...
  if (
    cur_keyfrm_->is_origin() &&
    last_corrected_front_rear_camera_constraints_.count(cur_keyfrm_->client_id_)) {
    return true;
  }

  if (!cur_keyfrm_->graph_node_->has_front_rear_camera_constraint()) {
    return false;
  }

  Mat44_t cur_keyframe_pose = cur_keyfrm_->get_cam_pose();
  data::keyframe * constraint_keyframe =
    cur_keyfrm_->graph_node_->get_front_rear_camera_constraint().second;
  ...
  if (cur_keyfrm_->get_map_id() != constraint_keyframe->get_map_id()) {
    return true;
  }
...
void univloc_global_optimization_module::correct_loop()
{
  ...
  for (const auto & opt_client_thread : thread_for_optimization_client_) {
    if (
      opt_client_thread.second && graph_optimizers_client_[opt_client_thread.first]->get_map_id() ==
                                    cur_keyfrm_->get_map_id()) {
  ...
  if (map_db_->is_keyframe_loaded(final_candidate_keyfrm)) {
    spdlog::info(
      "[detect loop]: current keyframe {} belongs to client {}, and its loop candidate keyframe {} "
      "is loaded from previous map with client ID {}",
      cur_keyfrm_->id_, cur_keyfrm_->client_id_, final_candidate_keyfrm->id_,
      final_candidate_keyfrm->client_id_);
  } else {
    spdlog::info(
      "[detect loop]: current keyframe {} belongs to client {}, and its loop candidate keyframe {} "
      "belongs to running client {}",
      cur_keyfrm_->id_, cur_keyfrm_->client_id_, final_candidate_keyfrm->id_,
      final_candidate_keyfrm->client_id_);
  }
  ...
  // 1. compute the Sim3 of the covisibilities of the current keyframe whose Sim3 is already
  //    estimated by the loop detector then, the covisibilities are moved to the corrected positions
  //    finally, landmarks observed in them are also moved to the correct position using the camera
  //    poses before and after camera pose correction
  ...
  const auto g2o_Sim3_cw_after_correction = loop_detector_->get_Sim3_world_to_current();
  {
    ...
    std::lock_guard<std::mutex> lock1(
      data::map_database::mtx_map_database_[cur_keyfrm_->get_map_id()]);
    ...
    Sim3s_nw_after_correction = get_Sim3s_after_loop_correction(
      cam_pose_wc_before_correction, g2o_Sim3_cw_after_correction, curr_neighbors);
  ...
  if (
    new_connections[cur_keyfrm_].find(final_candidate_keyfrm) ==
    new_connections[cur_keyfrm_].end()) {
  ...
  if (segment_optimize_) {
    thread_for_optimization_map_[thread_idx] = std::unique_ptr<std::thread>(new std::thread(
      std::bind(
        &optimize::window_graph_optimizer::optimize,
    ...
    thread_for_optimization_map_[thread_idx] = std::unique_ptr<std::thread>(new std::thread(
      std::bind(
        &optimize::univloc_graph_optimizer::optimize, graph_optimizers_map_[thread_idx].get(),
        final_candidate_keyfrm, cur_keyfrm_, loop_connections_.at(map_id),
        unused_neighbors_[map_id], loop_connections_with_pose_.at(map_id), iteration_times_)));
  }
}
...
void univloc_global_optimization_module::correct_front_rear_camera_constraint()
{
  if (cur_keyfrm_->is_origin()) {
    ...
    if (kf1->get_map_id() != kf2->get_map_id()) {
      return;
    }
    ...
    if (graph_optimizers_client_[thread_idx] && graph_optimizers_client_[thread_idx]->is_running()) {
      if (kf1 && graph_optimizers_client_[thread_idx]->get_keyframe_id() !=
        std::min(kf1->id_, kf2->id_)) {
          graph_optimizers_client_[thread_idx]->abort_graph_optimizaion();
        }

      if (thread_for_optimization_client_[thread_idx]) {
          spdlog::debug(
            "when start a new map session Waiting for last graph_optimizers_client thread exiting!");
          while (graph_optimizers_client_[thread_idx]->is_running()) {
              std::this_thread::sleep_for(std::chrono::microseconds(100));
          }
          thread_for_optimization_client_[thread_idx]->join();
      ...
      if (kf1 && kf2 && graph_optimizers_client_[thread_idx]->get_keyframe_id() !=
        std::min(kf1->id_, kf2->id_)) {
          // It means that a map session created by this client finished, its front_rear_camera_constraints should
          // be corrected
          spdlog::debug(
            "Correct front and rear camera constraints when start a new map session for client {}",
            cur_keyfrm_->client_id_);
          ...
          thread_for_optimization_client_[thread_idx] = std::unique_ptr<std::thread>(
            new std::thread(
              std::bind(
                &optimize::univloc_graph_optimizer::optimize_involved,
                graph_optimizers_client_[thread_idx].get(), kf1, kf2, iteration_times, true)));
        ...
        spdlog::debug(
          "Create thread and optimizer for front rear camera constraint when starting a new session!");
        graph_optimizers_client_[thread_idx] = nullptr;
        if (graph_optimizers_client_.count(thread_idx)) {
          graph_optimizers_client_.erase(thread_idx);
        }
  ...
  if (graph_optimizers_client_[thread_idx]) {
    if (
      thread_for_optimization_client_[thread_idx] &&
  ...
  thread_for_optimization_client_[thread_idx] = std::unique_ptr<std::thread>(new std::thread(
    std::bind(
      &optimize::univloc_graph_optimizer::optimize_involved,
      graph_optimizers_client_[thread_idx].get(), cur_keyfrm_, merge_keyfrm, iteration_times,
      true)));
  last_corrected_front_rear_camera_constraints_[cur_keyfrm_->client_id_] = cur_keyfrm_;
  last_corrected_front_rear_camera_constraints_[merge_keyfrm->client_id_] = merge_keyfrm;
}
...
void univloc_global_optimization_module::run()
{
  ...
  while (true) {
    ...
    if (!loop_detector_->validate_candidates(g2o_sim3_cand_to_curr_)) {
      ...
      if (
        loop_detector_->get_selected_candidate_keyframe()->get_map_id() >
        cur_keyfrm_->get_map_id()) {
        spdlog::warn(
          "Currently we only allow the map with larger ID merge to the map with smaller ID, \
  ...
  Optimization:
    ...
    if (looped_keyframe->get_map_id() != cur_keyfrm_->get_map_id() && !cur_keyfrm_->is_origin()) {
      if (detect_front_rear_camera_constraint_) {
        ...
        spdlog::info(
          "merge_map_via_front_rear_camera_constraint comsumes time: {} ms",
      ...
      if (detect_front_rear_camera_constraint_) {
        correct_front_rear_camera_constraint();
        spdlog::info(
          "correct_front_rear_camera_constraint comsumes time: {} ms", duration_ms(now() - start));
