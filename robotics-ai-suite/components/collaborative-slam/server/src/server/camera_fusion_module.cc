...
CameraFusionModule::CameraFusionModule(Map_database * map_db, rclcpp::Node * node_ptr)
: map_db_(map_db), pnh_(node_ptr)
{
  int buffer_size;
...
void CameraFusionModule::get_relative_transform_from_rosparam(rclcpp::Node * pnh)
{
  std::vector<Item> items = {
    Item{0, 1, Eigen::Matrix4d::Identity()},  Item{1, 1, Eigen::Matrix4d::Identity()},
  ...
  for (const auto & item : items) {
    ...
    if (group_clients_.find(group) == group_clients_.end()) {
      group_clients_[group] = std::vector<ClientID>();
    }
    ...
    for (ClientID client1 : it.second) {
      for (ClientID client2 : it.second) {
        ...
        if (pnh->get_parameter(rospara_name, T_vec)) {
          ...
          Eigen::Matrix3d rotationMatrix = rollAngle.toRotationMatrix() *
                                           pitchAngle.toRotationMatrix() *
                                           yawAngle.toRotationMatrix();
    ...
    for (ClientID client1 : it.second) {
      for (ClientID client2 : it.second) {
        ...
        Eigen::Affine3d relative_pose(
          client_initial_poses_[client1].inverse() * client_initial_poses_[client2]);
        map_db_->add_constraint(constraint_id, relative_pose);
        spdlog::debug(
          "add_constraint {} for client {} and {}", constraint_id, (int)client1, (int)client2);
        constraint_id++;
      }
    }
  }
...
void CameraFusionModule::process_new_keyframe(Keyframe * keyframe)
{
  ...
  for (ClientID client2 : group_clients_[group]) {
    ...
    for (auto kf_id : buffer) {
      ...
      // if (keyframe->get_map_id() < kf2->get_map_id()) {
      //   continue;
      // }
      double timediff = std::abs(kf2->timestamp_ - keyframe->timestamp_);
      if (timediff < stamp_diff_threshold_) {
        spdlog::debug(
          "Constrain client-{} keyframe {} with client{} keyframe {} (stamp diff {} ms)",
        ...
        if (keyframe->client_id_ > kf2->client_id_) {
          spdlog::debug(
            "Relative pose in map {} and map {}", keyframe->get_map_id(), kf2->get_map_id());
          spdlog::debug(
            "Traj Length1: {}, Traj length2: {}",
            keyframe->get_cam_pose_inv().block(0, 3, 3, 1).norm(),
            kf2->get_cam_pose_inv().block(0, 3, 3, 1).norm());
        } else {
          spdlog::debug(
            "Relative pose in map {} and map {}", kf2->get_map_id(), keyframe->get_map_id());
          spdlog::debug(
            "Traj Length1: {}, Traj length2: {}", kf2->get_cam_pose_inv().block(0, 3, 3, 1).norm(),
            keyframe->get_cam_pose_inv().block(0, 3, 3, 1).norm());
        }
      }
    }
