...
window_graph_optimizer::window_graph_optimizer(
...
bool window_graph_optimizer::slerp_pose_between_keyframes(
...
void window_graph_optimizer::optimize(
  ...
  std::map<data::keyframe *, std::set<data::keyframe *>> & loop_connections, int iteration_times)
{
  ...
  for (auto keyfrm : opt_keyfrms) {
    ...
    if (parent_node) {
      ...
      std::set<unsigned int>::iterator bound_iter =
        order_opt_keyfrm_ids.lower_bound(parent_node->id_);
    ...
    for (const auto & connected_keyfrm : connected_keyfrms) {
      ...
      const auto id2 = connected_keyfrm->id_;
      if (
        opt_keyfrm_ids.find(id1) == opt_keyfrm_ids.end() ||
        opt_keyfrm_ids.find(id2) == opt_keyfrm_ids.end()) {
        continue;
      }
      if (id1 <= id2) {
        continue;
    ...
    for (size_t idx = 0; idx < all_windows.size(); idx++) {
      for (size_t idy = 0; idy < edge_size_; idy++) {
        ...
        const auto keyfrm_end =
          window_keyfrms.at(2 * edge_size_ * idx + 2 * edge_size_ - 2 * idy - 1);
        ...
        const ::g2o::Sim3 Sim3_w1 =
          (keyfrm_start->loop_BA_identifier_ == loop_identifier)
            ? ::g2o::Sim3(
                keyfrm_start->cam_pose_cw_before_BA_.block(0, 0, 3, 3),
        ...
        const ::g2o::Sim3 & Sim3_2w =
          (keyfrm_end->loop_BA_identifier_ == loop_identifier)
            ? ::g2o::Sim3(
                keyfrm_end->cam_pose_cw_before_BA_.block(0, 0, 3, 3),
    ...
    data::map_database::mtx_map_database_[curr_keyfrm->get_map_id()]);
  {
    ...
    for (size_t idx = 0; idx < all_windows.size(); idx++) {
      ...
      opt_window->update_cam_pose_slerp(
        window_sim3s, map_corrected_Sim3s_wc, edge_size_, merged_map_id, loop_identifier);
    }
...
void window_graph_optimizer::classify_keyframes(
  const std::map<KeyframeID, data::keyframe *> & all_keyframes,
...
bool window_graph_optimizer::judge_window(int kf_count)
{
  ...
  if (last_kf_) {
    ...
    Vec7_t distance_vec =
      ::g2o::Sim3(velocity_now.block<3, 3>(0, 0), velocity_now.block<3, 1>(0, 3), 1).log();
...
void window_graph_optimizer::finished_current_window(
  std::vector<data::keyframe *> & single_keyframes, std::vector<data::window *> & all_windows,
  ...
  std::unordered_map<KeyframeID, data::keyframe *> & opt_keyfrm_ids)
{
  ...
  if (curr_window_ptr_->get_window_size() >= 2 * edge_size_ + error_window_size_) {
    ...
    for (size_t idx = velocity_vector_copy.size() - error_window_size_;
         idx < velocity_vector_copy.size(); idx++) {
