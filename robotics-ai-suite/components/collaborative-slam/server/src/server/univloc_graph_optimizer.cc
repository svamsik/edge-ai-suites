...
univloc_graph_optimizer::univloc_graph_optimizer(
  data::map_database * map_db, const bool fix_scale, univloc_server::Server * server)
: graph_optimizer(map_db, fix_scale), optimization_type_(Loop_Optimization), server_(server)
{
}

bool univloc_graph_optimizer::slerp_pose_between_keyframes(
  data::keyframe * kf, double time, Mat44_t & T_interp) const
{
  ...
  if (!(time2 < time && time < time3)) {
    spdlog::debug("time2 {}, time {}, time3 {}", time2, time, time3);
  }
  ...
  Eigen::Vector3d t_interp(
    ratio * t3[0] + (1.0 - ratio) * t2[0], ratio * t3[1] + (1.0 - ratio) * t2[1],
...
void univloc_graph_optimizer::optimize_involved(
  data::keyframe * cur_keyframe, data::keyframe * constraint_keyframe, int iteration_times,
  bool post_process)
{
  ...
  while (!keyframe_to_check.empty()) {
    ...
    for (const auto & child : children) {
      keyframe_to_check.push_back(child);
    }
  }
  ...
  // 1, If only one client starts a new session map, then the origin keyframe of another client
  //    map should be fixed;
  // 2, Else, the earlier created origin keyframe of the two clients should be set fixed!

  if (post_process) {
    if (
      origin_keyframe1->id_ == server_->get_client_origin_keyframe(cur_keyframe->client_id_)->id_) {
      server_->record_mapid_before_optimization(cur_keyframe);
    } else {
      fixed_keyfrm = origin_keyframe2;
    }
    if (
      origin_keyframe2->id_ ==
      server_->get_client_origin_keyframe(constraint_keyframe->client_id_)->id_) {
      server_->record_mapid_before_optimization(constraint_keyframe);
    } else {
      if (!fixed_keyfrm) {
        fixed_keyfrm = origin_keyframe1;
      } else {
        fixed_keyfrm = origin_keyframe1->timestamp_ < origin_keyframe2->timestamp_
                         ? origin_keyframe1
                         : origin_keyframe2;
      }
    }
  ...
  if (!fixed_keyfrm) {
    fixed_keyfrm = origin_keyframe1->timestamp_ < origin_keyframe2->timestamp_ ? origin_keyframe1
                                                                               : origin_keyframe2;
  }
  ...
  for (const auto & multi_camera_constraint :
       all_multi_camera_constraints) {  // use sim3 or se3 parameters
    ...
    front_rear_camera_constraint_vtx->setId(
      keyframeID_to_vertexID(id + max_kfm_id));  // max_kfm_id as the id offset
  ...
  // constraint edge
  const auto insert_edge = [&optimizer, &map_vertices, &inserted_edge_pairs](
  ...
  const auto insert_multi_camera_edge =
    [&optimizer, &map_vertices, &front_rear_camera_constraint_vertices](
      ...
      if (edge->numUndefinedVertices()) {
        spdlog::debug("Invalid edge front and rear camera constraint!");
      }
  ...
  for (const auto keyfrm : involved_keyfrms) {
    ...
    if (keyfrm->graph_node_->has_front_rear_camera_constraint()) {
      ...
      if (keyfrm->client_id_ > multi_camera_constraint.second->client_id_) {
        continue;
      }
      if (multi_camera_constraint.second->will_be_erased()) {
        continue;
      }
      ...
      Mat44_t pose_c_v =
        keyfrm->get_cam_pose() * slerp_pose;  // Relative pose from keyframe to virtual keyframe
  ...
  std::lock_guard<std::mutex> lock1(
    data::map_database::mtx_client_map_database_[std::make_pair(
      ...
      constraint_keyframe->client_id_, constraint_keyframe->get_map_id())]);
  {
    ...
    for (auto & lm : involved_landmarks) {
      ...
      if (ref_keyfrm->loop_BA_identifier_ != loop_identifier) {
        spdlog::debug(
          "Invalid ref keyframe: {}, ref keyframe loop_identifier {}, current loop_identifire {}",
  ...
  if (post_process) {
    ...
    if (
      ...
      server_->post_process_after_loop(
        constraint_keyframe->client_id_, constraint_keyframe->client_id_);
    }
  }
  ...
  auto end_time = now();
  spdlog::debug(
    "Correct front and rear camera constraint optimization costs time {} s",
    duration_ms(end_time - start_time));

  spdlog::debug(
    "optimize involved keyframes costs time {} s for {} edges", duration_ms(end_time - start_time),
    optimizer.activeEdges().size());
}

void univloc_graph_optimizer::optimize(
  ...
  int iteration_times)
{
  ...
  if (!fixed_keyframe) {
    spdlog::error(
      "map {} doesn't exist in the origin_keyfrms_ unordered_map of map_db_!",
  ...
  const auto all_keyfrms =
    map_db_->get_all_keyframes(loop_keyfrm->get_map_id(), curr_keyfrm->get_map_id());
  auto all_multi_camera_constraints = map_db_->get_all_multicamera_constraints();
  ...
  const auto insert_multi_camera_edge =
    [&optimizer, &map_vertices, &front_rear_camera_constraint_vertices](
      ...
      // if id1 and id2 are not in the same map, the constraint has no impact
      if (!map_vertices.count(id1)) {
        spdlog::debug("No such map_vertices id1 {}!", id1);
      }
      edge->setVertex(0, map_vertices[id1]);
      if (!map_vertices.count(id2)) {
        spdlog::debug("No such map_vertices id2 {}!", id2);
      }
      edge->setVertex(1, map_vertices[id2]);
      if (!front_rear_camera_constraint_vertices.count(id3)) {
  ...
  //       // spdlog::debug(
  //       //   "Loop connection weight: {}", keyfrm->graph_node_->get_weight(connected_keyfrm));
  //       continue;
  //     }
  //
  ...
  for (const auto & loop_connection_with_pose : loop_connections_with_pose) {
    ...
    const ::g2o::Sim3 & Sim3_1w = ::g2o::Sim3(
      keyfrm_pose_1.second.block(0, 0, 3, 3), keyfrm_pose_1.second.block(0, 3, 3, 1), 1.0);
    const ::g2o::Sim3 Sim3_w1 = Sim3_1w.inverse();

    if (keyfrm1->graph_node_->get_weight(keyfrm2) >= loop_min_weight) {
      const ::g2o::Sim3 & Sim3_2w = ::g2o::Sim3(
        keyfrm_pose_2.second.block(0, 0, 3, 3), keyfrm_pose_2.second.block(0, 3, 3, 1), 1.0);
      const ::g2o::Sim3 Sim3_21 = Sim3_2w * Sim3_w1;
      insert_edge(id1, id2, Sim3_21);
  ...
  for (auto keyfrm : all_keyfrms) {
    ...
    const ::g2o::Sim3 Sim3_w1 =
      keyfrm->loop_BA_identifier_ == loop_identifier
        ? ::g2o::Sim3(
            cam_pose_wc_before_BA.block(0, 0, 3, 3), cam_pose_wc_before_BA.block(0, 3, 3, 1), 1.0)
    ...
    if (parent_node) {
      ...
      const ::g2o::Sim3 & Sim3_2w =
        (parent_node->loop_BA_identifier_ == loop_identifier)
          ? ::g2o::Sim3(
              parent_node->cam_pose_cw_before_BA_.block(0, 0, 3, 3),
    ...
    for (const auto & connected_keyfrm : loop_edges) {
      ...
      const ::g2o::Sim3 & Sim3_2w =
        (connected_keyfrm->loop_BA_identifier_ == loop_identifier)
          ? ::g2o::Sim3(
              connected_keyfrm->cam_pose_cw_before_BA_.block(0, 0, 3, 3),
              connected_keyfrm->cam_pose_cw_before_BA_.block(0, 3, 3, 1), 1.0)
          : map_Sim3s_cw
              [id2];  // (iter2 != non_corrected_Sim3s.end()) ? iter2->second : map_Sim3s_cw[id2];
    ...
    //                     connected_keyfrm->cam_pose_cw_before_BA_.block(0, 3, 3, 1), 1.0)
    //       : map_Sim3s_cw[id2];  // (iter2 != non_corrected_Sim3s.end()) ? iter2->second
    //                             // : map_Sim3s_cw[id2];
    //
    //   const ::g2o::Sim3 Sim3_21 = Sim3_2w * Sim3_w1;
    //
    ...
    if (keyfrm->graph_node_->has_front_rear_camera_constraint()) {
      ...
      if (
        multi_camera_constraint.second->get_map_id() != loop_keyfrm->get_map_id() &&
  ...
  std::lock_guard<std::mutex> lock1(
    data::map_database::mtx_map_database_[curr_keyfrm->get_map_id()]);
  {
    ...
    if (origin_keyfrm_for_curr) {
      keyfrms_to_check.emplace_back(origin_keyfrm_for_curr);
    }
    if (origin_keyfrm_for_loop) {
      keyfrms_to_check.emplace_back(origin_keyfrm_for_loop);
    }
  ...
  server_->post_process_after_loop(
    loop_keyfrm->client_id_, curr_keyfrm->client_id_, loop_keyfrm->get_map_id(),
    curr_keyfrm->get_map_id());
  std::lock_guard<std::mutex> lock_thread(mtx_thread_);
  abort_graph_optimization_ = false;
