...
{
  spdlog::info(
    "~~~~~~~~~~~~~~~~~Create thread for client {} ~~~~~~~~~~~~~~~~~ ", static_cast<int>(client_id));
  running_ = true;
  ...
  map_id_ = 0xFFFFFFFF;
  spdlog::info(
    "~~~~~~~~~~~~~~~~~~Run thread to process message from client {} ~~~~~~~~~~~~~~~~~~",
...
void ClientHandler::save_trajectory(std::string folder)
{
  ...
  for (auto kfid : keyframes_) {
    ...
    if (streams.find(mapid) == streams.end()) {
      std::string filename = folder + "/client" + std::to_string(get_client_id()) + "map" +
                             std::to_string(mapid) + ".txt";
      streams[mapid] = std::ofstream(filename);
    }
    Eigen::Matrix4d Twc = kf->get_cam_pose_inv();
    Eigen::Matrix3d R = Twc.block(0, 0, 3, 3);
    Eigen::Quaterniond q(R);
    streams[mapid] << std::setprecision(18) << kf->timestamp_ << " " << Twc(0, 3) << " "
                   << Twc(1, 3) << " " << Twc(2, 3) << " " << q.x() << " " << q.y() << " " << q.z()
                   << " " << q.w() << std::endl;
    num++;
  }
...
std::vector<Keyframe *> ClientHandler::get_latest_keyframe_of_all_sessions() const
{
  ...
  if (current_keyframe_) {
    ret.push_back(current_keyframe_);
  }
...
void ClientHandler::pause(bool wait_until_paused)
{
  ...
  lock.lock();
  spdlog::info(
    " client {} Paused and left {} msg in the queue", (int)client_id_, message_queue_.size());
  lock.unlock();
}
...
void ClientHandler::record_mapid_before_optimization(MapID map_id)
{
  mapid_before_optimization_ = map_id;
}
...
void ClientHandler::process_msg_stored_in_queue(MapID merged_mapid)
{
  ...
  spdlog::info(
    "There exists {} old msg in the queue for client {} to be processed ", stored_msg_size,
    client_id_);
  while (!stored_msgs_queue.empty()) {
    ...
    if (last_msg_id_ != 0 && msg->msg_id != last_msg_id_ + 1) {
      spdlog::warn(
        "Lose message in client {}  between message {}  and {}", client_id_, last_msg_id_,
        msg->msg_id);
    }
  ...
  if (new_keyframes_.size() > 0) {
    ...
    for (auto & kf : new_keyframes_) {
      if (kf->get_map_id() == original_map_id) {
        kf->set_map_id(merged_mapid);
      }
    }
  ...
  auto end_time = now();
  spdlog::debug(
    "client {} Processed  {} msgs in the queue cost time {} ms", client_id_, stored_msg_size,
...
void ClientHandler::worker()
{
  ...
  while (rclcpp::ok()) {
    ...
    if (unlikely((message_queue_.empty() || paused_) && paused_ == requesting_pause_)) {
      message_event_.wait_for(message_lock, 100ms);
    }
    ...
    if (unlikely(last_msg_id_ != 0 && msg->msg_id != last_msg_id_ + 1)) {
      spdlog::warn(
        "Lost message in client {}  between message {}  and {}", client_id_, last_msg_id_,
        msg->msg_id);
    }
    ...
    if (msg->is_request_for_server_landmarks) {
      ...
      auto break_time = now();
      spdlog::debug(
        "Processing request for server landmarks cost time: {} ms",
    ...
    if (msg->need_relocalization) {
      ...
      if (current_keyframe_ && current_keyframe_->graph_node_->get_spanning_parent()) {
        ...
        if (
          !end_keyframe_of_old_sessions_.empty() &&
    ...
    run_times++;
    spdlog::debug(
      "[Client Handler {}]: utill now processed {} msgs with time cost: {} ms", client_id_,
      run_times, cost_time);
  }
  spdlog::info(
    "[Client Handler {}]: Totally Processed {} msgs with total time cost: {} ms", client_id_,
    run_times, cost_time);
}
...
void ClientHandler::register_new_session(Map_Msg_ConstPtr & msg)
{
  ...
  spdlog::info(
    "Client-{0} starts a new session at {1:09.3f}, registered new map-{2}", client_id_,
...
Keyframe * ClientHandler::process_keyframes(const Map_Msg_ConstPtr & msg)
{
  ...
  for (const Old_Keyframe_Msg & old_keyframe_msg : msg->keyframe_updates) {
    update_keyframe(old_keyframe_msg);
  }
  ...
  for (const Keyframe_Msg & keyframe_msg : msg->keyframes) {
    if (
      ...
      camera_type_ == Camera::setup_type_t::RGBD_Inertial ||
      camera_type_ == Camera::setup_type_t::Stereo_Inertial ||
      camera_type_ == Camera::setup_type_t::Stereo) {
      current_keyframe = create_rgbd_keyframe_from_msg(keyframe_msg, msg->need_relocalization);
    } else if (
      camera_type_ == Camera::setup_type_t::Monocular ||
...
void ClientHandler::update_keyframe(Keyframe * p_keyframe, const Keyframe_Msg & keyframe_msg)
{
  ...
  // Only if this message is new to the last loop optimization and don't start a new session when
  // optimization, we will update the pose
...
void ClientHandler::update_keyframe(const Old_Keyframe_Msg & msg)
{
  ...
  if (!p_keyframe) {
    spdlog::debug(
      "This keyframe {} was not newly created, but also not in mapdateset, maybe it was deleted by "
      "the Server or "
      "msg lost!",
      msg.mn_id);
    return;
...
Keyframe * ClientHandler::create_monocular_keyframe_from_msg(
  const Keyframe_Msg & keyframe_msg, bool is_relocalization_keyfrm)
{
  ...
  // If this message is old and no new session is created when optimization, then we should allign
  // the pose to the existed parent frame
  if (merge_times_ > merge_times_from_msg_ && map_id_ == mapid_before_optimization_) {
    ...
    if (!p_parent) {
      spdlog::warn(
        "{}: failed to get parent keyframe with id {}", __func__, keyframe_msg.mn_parent_id);
      return nullptr;
    }

    spdlog::debug(
      "merge_times: {}, merge_times_from_msg: {} clientID: {}", merge_times_, merge_times_from_msg_,
      client_id_);

    spdlog::debug(
      "Keyframe {} Position Before Aligning: {} {} {}", id, cam_pose_cw.inverse()(0, 3),
      cam_pose_cw.inverse()(1, 3), cam_pose_cw.inverse()(2, 3));

    cam_pose_cw =
      cam_pose_cw * p_parent->cam_pose_cw_before_BA_.inverse() * p_parent->get_cam_pose();
    spdlog::debug(
      "Position After Aligning: {} {} {}", cam_pose_cw.inverse()(0, 3), cam_pose_cw.inverse()(1, 3),
  ...
  auto p_keyframe = new Keyframe(
    id, src_frm_id, timestamp, cam_pose_cw, camera, depth_thr, num_keypts, keypts, undist_keypts,
    bearings_, stereo_x_right, depths, mv_descriptors, num_scale_levels, scale_factor,
    server_->get_bow_vocabulary(), server_->get_bow_database(), p_map, client_id, map_id_);
  ...
  if (!is_relocalization_keyfrm) {
    ...
    if (!p_map->get_origin_keyframe(map_id_)) {
      ...
      spdlog::debug(
        "Successfully set origin  keyframe {} for client {}  map {}", p_keyframe->id_,
    ...
    spdlog::debug(
      "Successfully create keyframe {0} on map-{1} with stamp {2:9.3f}", p_keyframe->id_,
...
Keyframe * ClientHandler::create_rgbd_keyframe_from_msg(
  const Keyframe_Msg & keyframe_msg, bool is_relocalization_keyfrm)
{
  uint client_id = keyframe_msg.mn_client_id;
  std::vector<cv::KeyPoint> undist_keypts, keypts;
  ...
  if (merge_times_ > merge_times_from_msg_ && map_id_ == mapid_before_optimization_) {
    ...
    if (!p_parent) {
      ...
      for (unsigned int i = 1; i <= trial_times; i++) {
        ...
        if (p_parent && p_parent->get_map_id() == map_id_) {
          break;
        }

        if (keyframe_msg.mn_parent_id == i) {
          break;
        }
      }
    }
    ...
    spdlog::debug(
      "Parent keyframe {} Position cam_pose_cw_before_BA_: {} {} {}", keyframe_msg.mn_parent_id,
    ...
    spdlog::debug(
      "Parent keyframe {} Position cam_pose_cw_after_BA_: {} {} {}", keyframe_msg.mn_parent_id,
...
void ClientHandler::process_mappoints(const Map_Msg_ConstPtr & msg)
{
  ...
  for (const Mappoint_Msg & mappoint_msg : msg->landmarks) {
    ...
    if (!landmark) {
      ...
      if (!p_Refkeyframe) {
        spdlog::debug(
          "There is no reference frame {} for this mappoint {}, we will choose another \
                              keyframe as its reference keyframe",
          mappoint_msg.mn_refer_keyframe_id, mappoint_msg.mn_id);
      ...
      Eigen::Vector3d pos_w(
        (double)mappoint_msg.mv_position[0], (double)mappoint_msg.mv_position[1],
      ...
      landmark = new Mappoint(
        id, 0, pos_w, p_Refkeyframe, num_visible, num_found, p_map, client_id, map_id_);
  ...
  auto end = now();
  spdlog::debug(
    "Processed {} landmark messages (cost {} ms). Added {}. Erased {}. Now {} in database",
    msg->landmarks.size(), duration_ms(end - start), num_add, num_erase,
    p_map->get_num_landmarks());
  spdlog::debug(
    "There are total_landmark_size {}  mappoints received from client {}", total_landmark_size,
    client_id_);
}
...
void ClientHandler::update_mappoint(const Mappoint_Msg & mappoint_msg, Map_database * p_map)
{
  ...
  if (unlikely(!p_mappoint)) {
    ...
    throw std::runtime_error(
      "You should add landmark into database before calling update_mappoint");
  }
  Eigen::Vector3d pos_w(
    (double)mappoint_msg.mv_position[0], (double)mappoint_msg.mv_position[1],
    (double)mappoint_msg.mv_position[2]);
  ...
  for (uint idx = 0; idx < observation_size; idx++) {
    ...
    if (unlikely(!p_keyframe)) {
      // This can happen because we have not yet received some of the latest keyframes
      // at the client side
  ...
  if (!mappoint_msg.mv_observation_keyframe_ids.empty()) {
    p_mappoint->compute_descriptor();
  }
...
void ClientHandler::update_connections()
{
  for (Keyframe * p_new_keyframe : new_keyframes_) {
    if (p_new_keyframe) {
      ...
      if (
        !p_new_keyframe->graph_node_->get_spanning_parent() && !p_new_keyframe->is_origin() &&
        last_keyframe_) {
...
void ClientHandler::send_back_server_landmarks(
  Eigen::Matrix4d curr_pose, KeyframeID curr_id, unsigned int localize_map_id)
{
  ...
  // spdlog::debug("This frame id: {}, last frame id of relative client: {} ", frame->id_, current_frame->id_);
  {
    ...
    server_landmarks = server_->p_all_map_db_->get_landmarks_in_frustum(
      curr_pose, camera, localize_map_id, 0, false, point_3d, point_2d, convex_hull,
      server_->near_distance_, server_->far_distance_, server_->back_distance_);
    if (server_landmarks.size() < server_landmarks_num_thr) {
      spdlog::debug(
        "{} less than {} server landmarks, so we will not send them", server_landmarks.size(),
...
void ClientHandler::relocalize_monocular_image([[maybe_unused]] double msg_timestamp)
{
  ...
  MapID reloc_map_id;
  auto succeeded = relocalizer_.relocalize(
    current_frame, reloc_map_id, server_->p_all_map_db_, server_->bow_vocab_,
    server_->near_distance_, server_->far_distance_, server_->back_distance_);
  if (succeeded) {
  ...
  Keyframe * current_keyframe_after_relocalization =
    new Keyframe(current_frame, nullptr, nullptr, client_id_, map_id_);
  current_keyframe_after_relocalization->id_ = current_keyframe->id_;
  spdlog::debug(
    "{}: constructed keyframe id: {}", client_id_, current_keyframe_after_relocalization->id_);
  // process keyframe
  ...
  std::vector<Mappoint *> nearby_landmarks = server_->get_map_database()->get_landmarks_in_frustum(
    current_keyframe_after_relocalization->get_cam_pose(),
    current_keyframe_after_relocalization->camera_,
...
{
  std::unique_lock lock(this->current_keyframe_mutex_);
  if (current_keyframe_) {
...
std::optional<Eigen::Matrix4d> ClientHandler::get_coordinate_transform_matrix()
{
  return coordinate_transform_;
}

void ClientHandler::set_coordinate_transform_matrix(
  const std::optional<Eigen::Matrix4d> & Tww_aligned_ref)
{
