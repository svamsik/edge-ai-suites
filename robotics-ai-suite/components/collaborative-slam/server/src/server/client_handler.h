...
// 1, Getting local landmarks(map) and newest keyframe from the slam system,
//    convert them to Ros message and send it to Server.
// 2, Receiving updated message(local landmarks and keyframes) from and Server, and then
//    updating slam state.
//
...
public:
  ...
  // Get transform from the world coordinate of the reference map to the world coordinate
  // of the map to be aligned
  std::optional<Eigen::Matrix4d> get_coordinate_transform_matrix();

  // Set transform from the world coordinate of the reference map to the world coordinate
  // of the map to be aligned
...
private:
  ...
  Keyframe * create_rgbd_keyframe_from_msg(
    const Keyframe_Msg & msg, bool is_relocalization_keyfrm = false);
  Keyframe * create_monocular_keyframe_from_msg(
    const Keyframe_Msg & msg, bool is_relocalization_keyfrm = false);
  ...
  void send_back_server_landmarks(
