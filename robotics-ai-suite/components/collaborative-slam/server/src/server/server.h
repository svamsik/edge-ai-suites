...
// 1, Getting local landmarks(map) and newest keyframe from the slam system , convert them to Ros
//    message and send it to Server.
// 2, Receiving updated message(local landmarks and keyframes) from and Server, and then updating
//    slam state.
//
...
public:
  Server(
    ...
    Map_database * p_map_db, rclcpp::Node * node_ptr,
  ...
  void transform_voxel_and_add_to_merged_octree(
    ...
    const int side = 0, const float min_acceptable_z = 0.0);
  void integrate_voxelblocks_to_merged_octree(
  ...
  void integrate_leaf_nodes_to_merged_octree(
    std::shared_ptr<se::Octree<SE_FIELD_TYPE>> octree_to_transform,
    std::shared_ptr<se::Octree<SE_FIELD_TYPE>> merged_octree,
    const Eigen::Matrix4d & coordinate_transform,
    const std::vector<std::array<int, 4>> & leaf_node_region);
...
protected:
  ...
  openvslam::univloc_global_optimization_module * get_global_optimizer() const
  {
...
private:
  ...
  std::unordered_map<ClientID, struct fast_mapping::camera_intrinsics>
    octree_intrinsic_from_trackers_;
  ...
  std::unordered_map<ClientID, std::unordered_map<std::string, std_msgs::msg::ColorRGBA>>
    camera_view_color_mapping_;

  Map_Msg map_msg_;
  std::string save_traj_folder_;

  void send_message(ClientID client_id, const univloc_msgs::Map & msg);
  void service_message_callback(
  ...
  void get_local_keyframes_and_mappoints(
    Keyframe * cur_keyframe, std::vector<Keyframe *> & local_keyframes,
    std::vector<Mappoint *> & local_mappoints);

  void convert_mappoints_to_msg(
    std::vector<Mappoint *> landmarks_buff, univloc_msgs::Map & msg_out,
    bool only_send_extra = false);
  void convert_keyframes_to_msg(
    std::vector<Keyframe *> keyframes_buff, univloc_msgs::Map & msg_out, ClientID client_id);
