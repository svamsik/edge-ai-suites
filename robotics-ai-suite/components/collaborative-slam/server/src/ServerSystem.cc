...
ServerSystem::ServerSystem(const std::shared_ptr<config> & cfg, rclcpp::Node * node_ptr)
: cfg_(cfg), pnh_(node_ptr)
{
  ...
  if (
    cfg_->server_mode_ == "localization" || cfg_->server_mode_ == "relocalization" ||
  ...
  global_optimizer_ = new univloc_global_optimization_module(
    map_db_, bow_db_, bow_vocab_, cfg_->fix_scale_, cfg_->front_rear_camera_constraint_thr_,
    cfg_->iteration_times_, cfg_->segment_optimize_, cfg_->correct_loop_);
...
void ServerSystem::startup(const bool)
{
  ...
  global_optimization_thread_ = std::make_unique<std::thread>(
    &openvslam::univloc_global_optimization_module::run, global_optimizer_);
}
...
void ServerSystem::save_keyframe_trajectory(
...
void ServerSystem::save_map_database(const std::string & path) const
{
  ...
  //   spdlog::warn(
  //     "Remove keyframes/landmarks from clients which are outside remapping region before saving to file");
...
bool ServerSystem::loop_detector_is_enabled() const
{
