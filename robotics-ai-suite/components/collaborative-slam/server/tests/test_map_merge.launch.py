...
def args_parse(default_bagfile_dir):
    ...
    parser.add_argument(
        ...
        type=str,
        dest='bagfile_dir',
        help='The directory which stores two bags used for map merge',
    ...
    parser.add_argument(
        ...
        type=str,
        dest='exefile_dir',
        help='The directory which stores the test_map_merge_exe',
        default='./',
    )
...
def main():
    ...
    test = ExecuteProcess(
        cmd=[args_dict['EXE']], output='screen', shell=False, on_exit=EmitEvent(event=Shutdown())
    )
    ...
    rosbag2 = ExecuteProcess(
        cmd=[
            ...
            args_dict['BAG2'],
            '--remap',
            '/camera/aligned_depth_to_color/camera_info:=/camera1/aligned_depth_to_color/camera_info',  # noqa: E501
