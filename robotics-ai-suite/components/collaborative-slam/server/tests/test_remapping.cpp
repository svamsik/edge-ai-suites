...
public:
  RemappingTest() : Node("test_remapping")
  {
    ...
    keyframes_within_remapping_region_num_threshold =
      declare_parameter<int>("keyframes_threshold", 0);
    ...
    remapping_region_vertexes_ =
      declare_parameter<std::vector<double>>("remapping_region", remapping_descriptor);
    ...
    // Qos with KeepAll setting means the system will store all published poses by tracker
    // as much as possible to pass the test.
    auto qos = rclcpp::QoS{rclcpp::KeepAll()};
    pose_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
      "univloc_tracker_0/pose", qos,
      std::bind(&RemappingTest::pose_callback, this, std::placeholders::_1));

    // SensorDataQoS means the system will store the latest data as soon as they are captured.
    // And our purpose is also to obtain the number of keyframes and voxels within remapping region
    // before system exit.
  ...
  void pose_callback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
  {
    ...
    if (current_pose == 0.0 || current_pose == last_pose_) {
      return;
    }
  ...
  void keyframe_callback(const visualization_msgs::msg::MarkerArray::SharedPtr msg)
  {
    ...
    for (auto & m : msg->markers) {
      if (m.ns == "client-0-0") {
        // We use seven points to form a rectangle to represent each keyframe in rviz on server side.
        // The starting point (1-th) and ending point (7-th) represent the position of keyframe
  ...
  int get_keyframes_within_remapping_region_num() const
  {
    return keyframes_within_remapping_region_num_;
  }
...
TEST(testRemappingMode, testRemappingMode)
{
  ...
  // 1. The number of published poses successfully tracked by tracker node should be higher than
  //    a certain threshold.
  // 2. Some special characters should be detected that only occur when map merge happens.
  // 3. The number of keyframe newly constructed in remapping region should be higher than a certain
  //    threshold.
  // 4. The number of voxels newly constructed in remapping region should be higher than a certain
  //    threshold.
  ...
  if (map_merge_happen) {
    std::cout << "Detected \"loop\" and \"MapID: 0\" from univloc_server/keyframes topic!"
              << std::endl;
  } else {
    std::cout << "Didn't detected \"loop\" and \"MapID: 0\" from univloc_server/keyframes topic!"
              << std::endl;
    FAIL();
  }

  if (octree_map_merge_happen) {
    std::cout << "Detected \"octree_map_merged\" from univloc_tracker_0/fused_map topic!"
              << std::endl;
  } else {
    std::cout << "Didn't detected \"octree_map_merged\" from univloc_tracker_0/fused_map topic!"
              << std::endl;
    FAIL();
  }

  if (
    keyframes_within_remapping_region_num > keyframes_within_remapping_region_num_threshold &&
    ...
    std::cout << "The number of keyframes newly constructed within remapping_region: "
              ...
              << " is larger than threshold: " << keyframes_within_remapping_region_num_threshold
              << std::endl;
    std::cout << "The number of voxels newly constructed within remapping_region: "
              << octree_within_remapping_region_num
              << " is larger than threshold: " << octree_within_remapping_region_num_threshold
              << std::endl;
  } else {
...
void sigint_handler(int)
{
  ...
  keyframes_within_remapping_region_num =
    testRemapping->get_keyframes_within_remapping_region_num();
