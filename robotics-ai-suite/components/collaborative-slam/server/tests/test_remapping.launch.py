...
def args_parse(default_bagfile_dir):
    parser = argparse.ArgumentParser(description='Automatically test remapping feature')
    parser.add_argument(
        '-b',
        '--bagfile_dir',
        type=str,
        dest='bagfile_dir',
        help='The directory which stores the .db3 file',
    ...
    parser.add_argument(
        ...
        '--keyframes_threshold',
        type=int,
        dest='keyframes_threshold',
        help='The threshold of keyframes constructed within remapping region ',
        default=0,
    )
    parser.add_argument(
        '-o',
        '--octree_threshold',
        type=int,
        dest='octree_threshold',
        help='The threshold of voxels constructed within remapping region',
        default=0,
    )
    parser.add_argument(
        '-e',
        ...
        type=str,
        dest='exefile_dir',
        help='The directory which stores the test_remapping_exe',
        default='./',
    )
    parser.add_argument(
        '-r',
        '--remapping_region',
        type=str,
        dest='remapping_region',
        help='The four vertexes of remapping region, [P1.x, P1.y, ... , P4.x, P4.y], must be in clockwise or anti-clockwise order',  # noqa: E501
        default='[0.0]',
    )
...
def main():
    ...
    octree_load_path = 'octree_load_path:=' + args_dict['LOADEDMAP'] + 'octree.bin'
    load_map_path = 'load_map_path:=' + args_dict['LOADEDMAP'] + 'map.msg'

    remapping_region_temp = args_dict['REMAPPING_REGION'][1:-1].split(',')
    if len(remapping_region_temp) != 8:
        print(
            'The remapping region consists of four vertexes, so the size of this parameter should be 8. The current size is %d, please check the input!'  # noqa: E501
    ...
    tracker = ExecuteProcess(
        cmd=[
            ...
            'log_level:=warning',
            'camera:=camera1',
            'baselink_frame:=base_link1',
            'image_frame:=camera1_color_optical_frame',
            'use_odom:=true',
            'odom_frame:=odom1',
    ...
    server_path = os.path.join(
        get_package_prefix('univloc_server'), 'lib/univloc_server/univloc_server'
    )
    server = ExecuteProcess(
        cmd=[
            ...
            '--ros-args',
            '-r',
            '__node:=univloc_server',
            '-p',
            'server_mode:=remapping',
            '-p',
            load_map_path,
            '-p',
            'log_level:=info',
    ...
    tracker_monitor_cmd = (
        ...
        + ['--ros-args']
        + ['-p']
        + ['pose_threshold:=' + str(args_dict['POSE_THRESHOLD'])]
        + ['-p']
        + ['keyframes_threshold:=' + str(args_dict['KEYFRAMES_THRESHOLD'])]
        + ['-p']
        + ['voxels_threshold:=' + str(args_dict['VOXELS_THRESHOLD'])]
        + ['-p']
        + ['remapping_region:=' + str(args_dict['REMAPPING_REGION'])]
    )

    test = ExecuteProcess(
        cmd=tracker_monitor_cmd, cwd=args_dict['EXE'], output='screen', shell=False
    )
    ...
    process_exit_handler = OnProcessExit(target_action=test, on_exit=on_monitor_process_exit)

    # Group tracker node, server node, tracker monitor and playback execute process to launch service # noqa: E501
