...
: ref_camera_(ref_frm.camera_),
  ...
  ref_bearings_(ref_frm.bearings_),
  cur_camera_(nullptr),
  num_ransac_iters_(num_ransac_iters),
  min_num_triangulated_(min_num_triangulated),
  parallax_deg_thr_(parallax_deg_thr),
  reproj_err_thr_(reproj_err_thr)
{
}

Mat33_t base::get_rotation_ref_to_cur() const { return rot_ref_to_cur_; }

Vec3_t base::get_translation_ref_to_cur() const { return trans_ref_to_cur_; }

eigen_alloc_vector<Vec3_t> base::get_triangulated_pts() const { return triangulated_pts_; }

std::vector<bool> base::get_triangulated_flags() const { return is_triangulated_; }

bool base::find_most_plausible_pose(
  const eigen_alloc_vector<Mat33_t> & init_rots, const eigen_alloc_vector<Vec3_t> & init_transes,
  const std::vector<bool> & is_inlier_match, const bool depth_is_positive)
{
  ...
  for (unsigned int i = 0; i < num_hypothesis; ++i) {
    nums_valid_pts.at(i) = check_pose(
      init_rots.at(i), init_transes.at(i), is_inlier_match, depth_is_positive,
  ...
  const auto max_num_valid_pts_iter =
    std::max_element(nums_valid_pts.begin(), nums_valid_pts.end());
  // get the index of the hypothesis
  const unsigned int max_num_valid_index =
    std::distance(nums_valid_pts.begin(), max_num_valid_pts_iter);
  ...
  const auto num_similars = std::count_if(
    nums_valid_pts.begin(), nums_valid_pts.end(),
...
unsigned int base::check_pose(
  const Mat33_t & rot_ref_to_cur, const Vec3_t & trans_ref_to_cur,
  const std::vector<bool> & is_inlier_match, const bool depth_is_positive,
  ...
  float & parallax_deg)
{
  ...
  for (unsigned int i = 0; i < ref_cur_matches_.size(); ++i) {
    ...
    const Vec3_t pos_c_in_ref =
      solve::triangulator::triangulate(ref_bearing, cur_bearing, rot_ref_to_cur, trans_ref_to_cur);

    if (
      !std::isfinite(pos_c_in_ref(0)) || !std::isfinite(pos_c_in_ref(1)) ||
    ...
    const auto is_valid_ref = ref_camera_->reproject_to_image(
      Mat33_t::Identity(), Vec3_t::Zero(), pos_c_in_ref, reproj_in_ref, x_right_in_ref);
    ...
    const auto is_valid_cur = cur_camera_->reproject_to_image(
      rot_ref_to_cur, trans_ref_to_cur, pos_c_in_ref, reproj_in_cur, x_right_in_cur);
