...
public:
  ...
  perspective(
    ...
    const float reproj_err_thr, bool use_odom);
  ...
  bool initialize(
    const data::frame & cur_frm, const std::vector<int> & ref_matches_with_cur) override;
...
private:
  ...
  Mat44_t ref_odom_ = Eigen::Matrix4d::Identity();
};
