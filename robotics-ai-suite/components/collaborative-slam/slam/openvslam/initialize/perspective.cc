...
#include "solve/homography_solver.h"
#include "solve/fundamental_solver.h"
...
perspective::perspective(
  const data::frame & ref_frm, const unsigned int num_ransac_iters,
  const unsigned int min_num_triangulated, const float parallax_deg_thr, const float reproj_err_thr,
  bool use_odom)
: base(ref_frm, num_ransac_iters, min_num_triangulated, parallax_deg_thr, reproj_err_thr),
  ...
  use_odom_(use_odom),
  ref_odom_(ref_frm.odom_)
{
  spdlog::debug("CONSTRUCT: initialize::perspective");
}

perspective::~perspective() { spdlog::debug("DESTRUCT: initialize::perspective"); }

bool perspective::initialize(
  const data::frame & cur_frm, const std::vector<int> & ref_matches_with_cur)
{
  ...
  if (use_odom_) {
    // Tco
    Mat44_t cur_odom = cur_frm.odom_;

    Mat33_t rot_c2_o = cur_odom.block<3, 3>(0, 0);
    Vec3_t trans_c2_o = cur_odom.block<3, 1>(0, 3);
    Mat33_t rot_o_c1 = ref_odom_.block<3, 3>(0, 0).transpose();
    Vec3_t trans_o_c1 = -rot_o_c1 * ref_odom_.block<3, 1>(0, 3);

    Mat33_t rot_c2_c1 = rot_c2_o * rot_o_c1;
    Vec3_t trans_c2_c1 = rot_c2_o * trans_o_c1 + trans_c2_o;

    eigen_alloc_vector<Mat33_t> init_rots(1, rot_c2_c1);
    eigen_alloc_vector<Vec3_t> init_transes(1, trans_c2_c1);
    std::vector<bool> is_inlier_match(ref_cur_matches_.size(), true);

    const auto pose_is_found =
      find_most_plausible_pose(init_rots, init_transes, is_inlier_match, true);

    // update the ref_odom_ with the odometry data of current frame to keep consistent with
    // the initial frame in openvslam::module::initializer
    ref_odom_ = cur_odom;
    if (!pose_is_found) {
      return false;
    }

    spdlog::info("initialization succeeded with odom");

    return true;

  } else {
    // compute H and F matrices
    auto homography_solver =
      solve::homography_solver(ref_undist_keypts_, cur_undist_keypts_, ref_cur_matches_, 1.0);
    auto fundamental_solver =
      solve::fundamental_solver(ref_undist_keypts_, cur_undist_keypts_, ref_cur_matches_, 1.0);
    std::thread thread_for_H(
      &solve::homography_solver::find_via_ransac, &homography_solver, num_ransac_iters_, true);
    std::thread thread_for_F(
      &solve::fundamental_solver::find_via_ransac, &fundamental_solver, num_ransac_iters_, true);
    thread_for_H.join();
    thread_for_F.join();
...
bool perspective::reconstruct_with_H(
  const Mat33_t & H_ref_to_cur, const std::vector<bool> & is_inlier_match)
{
  ...
  if (!solve::homography_solver::decompose(
        H_ref_to_cur, ref_cam_matrix_, cur_cam_matrix_, init_rots, init_transes, init_normals)) {
    return false;
  }
...
bool perspective::reconstruct_with_F(
  const Mat33_t & F_ref_to_cur, const std::vector<bool> & is_inlier_match)
{
  ...
  eigen_alloc_vector<Mat33_t> init_rots;
  eigen_alloc_vector<Vec3_t> init_transes;
  if (!solve::fundamental_solver::decompose(
        F_ref_to_cur, ref_cam_matrix_, cur_cam_matrix_, init_rots, init_transes)) {
    return false;
  }
