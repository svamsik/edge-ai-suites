...
void mapping_module::set_odom_data(std::shared_ptr<data::odom> odom_data_buf)
{
...
void mapping_module::mapping_with_new_keyframe()
{
  ...
  if (use_odom_) {
    if (cur_keyfrm_->odom_ == Eigen::Matrix4d::Identity()) {
      spdlog::debug(
        "mapping module do not get good kf odom, try again! kf id is {}", cur_keyfrm_->id_);
  ...
  if (2 < map_db_->get_num_keyframes() && !cur_keyfrm_->should_be_fixed_in_optimization_) {
    bool is_large =
      ((tracker_->get_num_tracked_lms() > 75 && is_monocular_) ||
...
void mapping_module::create_new_landmarks()
{
  ...
  const auto cur_covisibilities = cur_keyfrm_->graph_node_->get_top_n_covisibilities(
    num_covisibilities * (is_monocular_ ? 2 : 1));
  ...
  for (unsigned int i = 0; i < cur_covisibilities.size(); ++i) {
    ...
    const Mat33_t E_ngh_to_cur = solve::essential_solver::create_E_21(
      ngh_keyfrm->get_rotation(), ngh_keyfrm->get_translation(), cur_keyfrm_->get_rotation(),
      cur_keyfrm_->get_translation());
...
void mapping_module::triangulate_with_two_keyframes(
...
std::unordered_set<data::keyframe *> mapping_module::get_second_order_covisibilities(
  const unsigned int first_order_thr, const unsigned int second_order_thr)
{
  const auto cur_covisibilities =
    cur_keyfrm_->graph_node_->get_top_n_covisibilities(first_order_thr);
  ...
  for (const auto first_order_covis : cur_covisibilities) {
    ...
    const auto ngh_covisibilities =
      first_order_covis->graph_node_->get_top_n_covisibilities(second_order_thr);
...
void mapping_module::fuse_landmark_duplication(
  const std::unordered_set<data::keyframe *> & fuse_tgt_keyfrms)
{
  match::fuse matcher;
