...
tracking_module::tracking_module(
  std::shared_ptr<univloc_tracker::Config> cfg, camera::base * camera, data::map_database * map_db,
...
void tracking_module::set_odom_data(std::shared_ptr<data::odom> odom_data_buf)
{
  odom_data_buf_ = odom_data_buf;
}

void tracking_module::set_imu_data(std::shared_ptr<data::IMU_data> imu_data_buf)
{
  imu_data_buf_ = imu_data_buf;
}

void tracking_module::queue_frm_and_keyfrm_to_be_relocalized(
  data::keyframe * keyfrm, std::shared_ptr<data::frame> frm)
{
  std::scoped_lock<std::mutex> lock(mtx_frm_and_keyfrm_);
...
bool tracking_module::get_frm_and_keyfrm(
  KeyframeID id, std::pair<data::keyframe *, std::shared_ptr<data::frame>> & frm_keyfrm)
{
  std::scoped_lock<std::mutex> lock(mtx_frm_and_keyfrm_);
...
void tracking_module::monocular_initialization_using_relocalization(
...
{
  return tracking_state_ == tracker_state_t::NotInitialized ||
...
void tracking_module::align_state_to_inertial_frame(
  const Eigen::Matrix3d R_inertial_c0, const double scale)
{
  ...
  last_cam_pose_from_ref_keyfrm_.block(0, 3, 3, 1) =
    scale * last_cam_pose_from_ref_keyfrm_.block(0, 3, 3, 1);
}
...
void tracking_module::predict_camerapose_from_imu()
{
  ...
  for (unsigned int i = 0; i < imu_mearsurement_vec_.size(); i++) {
    if (i == 0) {
      ...
      imu_measurement =
        (imu_mearsurement_vec_[i].second + imu_mearsurement_vec_[i - 1].second) * 0.5 -
    ...
    imu_pose.block(0, 3, 3, 1) =
      imu_pose.block(0, 3, 3, 1) + predicted_imu_velocity_ * delta_t +
      gI * delta_t * delta_t * 0.5 +
      0.5 * imu_pose.block(0, 0, 3, 3) * imu_measurement.head(3) * delta_t * delta_t;
    imu_pose.block(0, 0, 3, 3) =
      imu_pose.block(0, 0, 3, 3) * skew(imu_rotation_speed * delta_t).exp();
  ...
  if (curr_frm_->too_few_features_) {
    if (imu_estimated_poses_.empty()) {
      imu_estimated_poses_.push_back(
        last_keyfrm_->get_cam_pose_inv() * data::IMU_Preintegration::Tci_);
    }
...
bool tracking_module::get_odom_data()
{
  ...
  if (get_odom_stat == 1) {
    ...
    spdlog::debug(
      "can't get odom pose for current frame (id: {}), use last odom pose instead", curr_frm_->id_);
    return true;
    ...
    spdlog::debug(
      "able to get odom pose for current frame (id {}), update last odom pose", curr_frm_->id_);
    return true;
  } else {
...
void tracking_module::get_local_keyframes_poses(
  std::vector<Eigen::Matrix4d> & poses, bool sort) const
{
  poses.clear();
  if (data::IMU_Preintegration::state_ == data::IMU_Preintegration::IMU_State::Not_Initialized) {
    ...
    for (const auto & item : tempral_keyfrms) {
      poses.push_back(item.second->get_cam_pose_inv());
    }
  }
}
  ...
  std::vector<Eigen::Matrix4d> & non_covisibility_poses) const
{
  ...
  for (const auto & kf : local_keyfrms_) {
    if (
      ref_keyfrm_->graph_node_->get_weight(kf) > covisibility_weigth_thr ||
      ref_keyfrm_->id_ == kf->id_) {
  ...
  for (const auto & it : covisibility_poses_with_id) {
    covisibility_poses.push_back(it.second);
  }
  for (const auto & it : non_covisibility_poses_with_id) {
    non_covisibility_poses.push_back(it.second);
  }
}
...
void tracking_module::get_tracked_server_landmarks_positions(
...
bool tracking_module::optimize_current_frame_with_local_map()
{
  ...
  for (unsigned int idx = 0; idx < curr_frm_->num_keypts_; ++idx) {
    ...
    if (!curr_frm_->outlier_flags_.at(idx)) {
      ...
      assert(
        (std::string("Landmark id: ") + std::to_string(lm->id_),
  ...
  if (
    ...
    spdlog::debug(
      "local map tracking failed: {} matches < {}", num_tracked_lms_, 2 * num_tracked_lms_thr);
    return false;
  }
  ...
  if (num_tracked_lms_ < num_tracked_lms_thr) {
    spdlog::debug(
      "local map tracking failed: {} matches < {}", num_tracked_lms_, num_tracked_lms_thr);
    return false;
  }
...
bool tracking_module::search_all_landmarks()
{
  ...
  Vec2_t reproj;
  float x_right;
  unsigned int pred_scale_level, observerd_local_landmarks_num = 0,
                                 observed_server_landmarks_num = 0;
  ...
  spdlog::debug(
    " {} of {} local landmarks are obserrved ", observerd_local_landmarks_num,
    local_landmarks_.size());

  spdlog::debug(
    " {} of {} server landmarks(not existed in local map) are obserrved ",
    observed_server_landmarks_num, server_landmarks.size());

  spdlog::debug(
    " {} of {} landmarks are succesfully matched ", matched_num, nearby_landmarks.size());
...
bool tracking_module::search_local_landmarks()
{
  ...
  spdlog::debug(
    " {} of {} local landmarks are observed when tracking local map", observed_local_landmarks_num,
    local_landmarks_.size());

  spdlog::debug(
    " {} of {} local landmarks are succesfully matched ", matched_num, local_landmarks_.size());
...
void tracking_module::insert_new_keyframe()
{
  ...
  if (camera_->use_imu_ && ref_keyfrm) {
    ...
    if (n2 - n1) {
      spdlog::debug("Got {} more imu data during insert new keyframe!", n2 - n1);
    }
    ...
    spdlog::debug(
      "Create imu constraint between keyframe {} and {}", last_keyfrm_->id_, ref_keyfrm->id_);
  }
  ...
  if (camera_->use_imu_ && ref_keyfrm) {
    keyfrm_inserter_.queue_keyframe(ref_keyfrm);
  }
}
