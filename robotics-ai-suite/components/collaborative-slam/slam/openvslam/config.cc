...
bool cmpf(double A, double B, double epsilon = EPSILON) { return fabs(A - B) < epsilon; }
...
config::config(rclcpp::Node * pnh, std::string config_path) : pnh_(pnh), config_path_(config_path)
{
  ...
  if (iteration_times_ < 0 || iteration_times_ > MAX_ITERATIONS) {
    throw std::invalid_argument(
      "Number of iteration times should be between : 0 - " + std::to_string(MAX_ITERATIONS));
  }
...
config::~config() { spdlog::debug("DESTRUCT: config"); }
...
bool config::is_config_valid()
{
  if (server_mode_ == "mapping") {
    ...
    if (remapping_region_vertexes_.size() != 8) {
      spdlog::critical(
        "Option remapping_region should include 4 points in the format of [P1.x, P1.y \
    ...
    if (load_map_path_.empty()) {
      spdlog::critical(
        "Option load_map_path shouldn't be empty in localization and relocalization mode!");
      return false;
    }
  } else {
    spdlog::critical(
      "Invalid value of server_mode option, only support mapping, localization or remapping!");
    return false;
  }
