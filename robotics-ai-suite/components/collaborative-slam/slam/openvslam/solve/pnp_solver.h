...
public:
  ...
  //! constructor
  pnp_solver(
    const eigen_alloc_vector<Vec3_t> & valid_bearings,
  ...
  pnp_solver(const pnp_solver &) = delete;
  pnp_solver & operator=(const pnp_solver &) = delete;
  ...
  Mat33_t get_best_rotation() const { return best_rot_cw_; }

  //! get the most reliable translation (as the world reference)
  Vec3_t get_best_translation() const { return best_trans_cw_; }
...
private:
  ...
  void gauss_newton(
    const MatRC_t<6, 10> & L_6x10, const MatRC_t<6, 1> & Rho, double current_betas[4]);

  void compute_A_and_b_gauss_newton(
    const MatRC_t<6, 10> & L_6x10, const MatRC_t<6, 1> & Rho, double cb[4], MatRC_t<6, 4> & A,
    MatRC_t<6, 1> & b);
