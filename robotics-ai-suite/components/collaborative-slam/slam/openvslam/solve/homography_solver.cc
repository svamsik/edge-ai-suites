...
void homography_solver::find_via_ransac(const unsigned int max_num_iter, const bool recompute)
{
  ...
  const Mat33_t normalized_H_21 =
    solve::homography_solver::compute_H_21(inlier_normalized_keypts_1, inlier_normalized_keypts_2);
  best_H_21_ = transform_2_inv * normalized_H_21 * transform_1;
  best_score_ = check_inliers(best_H_21_, is_inlier_match_);
}

Mat33_t homography_solver::compute_H_21(
  const std::vector<cv::Point2f> & keypts_1, const std::vector<cv::Point2f> & keypts_2)
{
  ...
  for (unsigned int i = 0; i < num_points; ++i) {
    ...
    A.block<1, 3>(2 * i + 1, 6) =
      -keypts_2.at(i).x * util::converter::to_homogeneous(keypts_1.at(i));
  }
...
bool homography_solver::decompose(
  const Mat33_t & H_21, const Mat33_t & cam_matrix_1, const Mat33_t & cam_matrix_2,
  ...
  eigen_alloc_vector<Vec3_t> & init_normals)
{
  ...
  // if use Vec3_t, build will failure in Humble saying lambda(2) may be used uninitialized
  const VecX_t & lambda = svd.singularValues();
  ...
  // when d' > 0
  // eq.(13)
  const float aux_sin_theta =
    std::sqrt((d1 * d1 - d2 * d2) * (d2 * d2 - d3 * d3)) / ((d1 + d3) * d2);
  const float cos_theta = (d2 * d2 + d1 * d3) / ((d1 + d3) * d2);
  const std::array<float, 4> aux_sin_thetas = {
    {aux_sin_theta, -aux_sin_theta, -aux_sin_theta, aux_sin_theta}};
