  ...
  Mat33_t & transform)
{
  ...
  if (num_keypts == 0) {
    spdlog::warn("normalize: no keypoints given");
    return;
  }
  ...
  if (FP_ZERO == std::fpclassify(mean_l1_dev_x)) {
    spdlog::warn("normalize mean_l1_dev_x: division by zero");
    return;
  }
  if (FP_ZERO == std::fpclassify(mean_l1_dev_y)) {
    spdlog::warn("normalize mean_l1_dev_y: division by zero");
    return;
  }
