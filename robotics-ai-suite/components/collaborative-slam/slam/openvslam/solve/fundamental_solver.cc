...
fundamental_solver::fundamental_solver(
  const std::vector<cv::KeyPoint> & undist_keypts_1,
  const std::vector<cv::KeyPoint> & undist_keypts_2,
  const std::vector<std::pair<int, int>> & matches_12, const float sigma)
: undist_keypts_1_(undist_keypts_1),
  undist_keypts_2_(undist_keypts_2),
...
void fundamental_solver::find_via_ransac(const unsigned int max_num_iter, const bool recompute)
{
  ...
  const Mat33_t normalized_F_21 =
    solve::fundamental_solver::compute_F_21(inlier_normalized_keypts_1, inlier_normalized_keypts_2);
  best_F_21_ = transform_2_t * normalized_F_21 * transform_1;
  best_score_ = check_inliers(best_F_21_, is_inlier_match_);
}

Mat33_t fundamental_solver::compute_F_21(
  const std::vector<cv::Point2f> & keypts_1, const std::vector<cv::Point2f> & keypts_2)
{
  ...
  VecX_t lambda = svd.singularValues();
...
bool fundamental_solver::decompose(
  const Mat33_t & F_21, const Mat33_t & cam_matrix_1, const Mat33_t & cam_matrix_2,
...
Mat33_t fundamental_solver::create_F_21(
  const Mat33_t & rot_1w, const Vec3_t & trans_1w, const Mat33_t & cam_matrix_1,
...
float fundamental_solver::check_inliers(const Mat33_t & F_21, std::vector<bool> & is_inlier_match)
{
  ...
  for (unsigned int i = 0; i < num_points; ++i) {
    ...
    const float dist_sq_2 =
      residual_in_2 * residual_in_2 / epiline_in_2.block<2, 1>(0, 0).squaredNorm();
    ...
    const float dist_sq_1 =
      residual_in_1 * residual_in_1 / epiline_in_1.block<2, 1>(0, 0).squaredNorm();
