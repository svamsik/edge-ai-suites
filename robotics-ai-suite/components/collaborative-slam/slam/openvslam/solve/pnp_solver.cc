...
pnp_solver::pnp_solver(
  const eigen_alloc_vector<Vec3_t> & valid_bearings, const std::vector<cv::KeyPoint> & valid_keypts,
  const eigen_alloc_vector<Vec3_t> & valid_landmarks, const std::vector<float> & scale_factors,
  const unsigned int min_num_inliers)
: num_matches_(valid_bearings.size()),
  valid_bearings_(valid_bearings),
  valid_landmarks_(valid_landmarks),
  min_num_inliers_(min_num_inliers)
{
  ...
  for (unsigned int i = 0; i < num_matches_; ++i) {
    const auto max_rad_error_with_scale =
      scale_factors.at(valid_keypts.at(i).octave) * max_rad_error;
...
void pnp_solver::find_via_ransac(const unsigned int max_num_iter, const bool recompute)
{
  ...
  for (unsigned int iter = 0; iter < max_num_iter; ++iter) {
    ...
    // skip this iteration if not having enough correspondences
    // this also avoids the situation of num_correspondences_ == 0, which will cause
    // devision-by-zero in compute_pose
    if (num_correspondences_ < min_set_size) {
      continue;
    }
  ...
  // above we've made sure num_correspondences_ == min_set_size for every candidate of
  // (best_rot_cw_, best_trans_cw_) and exit the function if there is no valid solution
  // so we can safely assume the following, and devision-by-zero not happen in the next
  // compute_pose
  assert(num_correspondences_ == min_set_size);
  compute_pose(best_rot_cw_, best_trans_cw_);
}

unsigned int pnp_solver::check_inliers(
  const Mat33_t & rot_cw, const Vec3_t & trans_cw, std::vector<bool> & is_inlier)
{
...
void pnp_solver::reset_correspondences() { num_correspondences_ = 0; }
...
void pnp_solver::choose_control_points()
{
  ...
  if (num_correspondences_ == 0) {
    throw std::runtime_error("num_correspondences_ must be non-zero");
  }
...
void pnp_solver::compute_barycentric_coordinates()
{
  ...
  for (unsigned int i = 0; i < num_correspondences_; ++i) {
    ...
    for (unsigned int j = 0; j < 3; ++j) {
      a[1 + j] = CC_inv(j, 0) * (pi[0] - cws[0][0]) + CC_inv(j, 1) * (pi[1] - cws[0][1]) +
...
void pnp_solver::fill_M(
  MatX_t & M, const int row, const double * as, const double u, const double v)
{
  assert(as);
...
double pnp_solver::compute_R_and_t(
  const MatX_t & Ut, const double * betas, double R[3][3], double t[3])
{
...
void pnp_solver::find_betas_approx_1(
  const MatRC_t<6, 10> & L_6x10, const MatRC_t<6, 1> & Rho, double * betas)
{
  MatX_t L_6x4(6, 4);
...
void pnp_solver::find_betas_approx_2(
  const MatRC_t<6, 10> & L_6x10, const MatRC_t<6, 1> & Rho, double * betas)
{
  assert(betas);
...
void pnp_solver::find_betas_approx_3(
  const MatRC_t<6, 10> & L_6x10, const MatRC_t<6, 1> & Rho, double * betas)
{
  assert(betas);
...
void pnp_solver::compute_A_and_b_gauss_newton(
  const MatRC_t<6, 10> & L_6x10, const MatRC_t<6, 1> & Rho, double betas[4], MatRC_t<6, 4> & A,
  MatRC_t<6, 1> & b)
{
  for (unsigned int i = 0; i < 6; ++i) {
    A(i, 0) = 2 * L_6x10(i, 0) * betas[0] + L_6x10(i, 1) * betas[1] + L_6x10(i, 3) * betas[2] +
              L_6x10(i, 6) * betas[3];
    A(i, 1) = L_6x10(i, 1) * betas[0] + 2 * L_6x10(i, 2) * betas[1] + L_6x10(i, 4) * betas[2] +
              L_6x10(i, 7) * betas[3];
    A(i, 2) = L_6x10(i, 3) * betas[0] + L_6x10(i, 4) * betas[1] + 2 * L_6x10(i, 5) * betas[2] +
              L_6x10(i, 8) * betas[3];
    A(i, 3) = L_6x10(i, 6) * betas[0] + L_6x10(i, 7) * betas[1] + L_6x10(i, 8) * betas[2] +
              2 * L_6x10(i, 9) * betas[3];

    b(i, 0) = Rho[i] - (L_6x10(i, 0) * betas[0] * betas[0] + L_6x10(i, 1) * betas[0] * betas[1] +
                        L_6x10(i, 2) * betas[1] * betas[1] + L_6x10(i, 3) * betas[0] * betas[2] +
                        L_6x10(i, 4) * betas[1] * betas[2] + L_6x10(i, 5) * betas[2] * betas[2] +
                        L_6x10(i, 6) * betas[0] * betas[3] + L_6x10(i, 7) * betas[1] * betas[3] +
                        L_6x10(i, 8) * betas[2] * betas[3] + L_6x10(i, 9) * betas[3] * betas[3]);
  }
}

void pnp_solver::gauss_newton(
  const MatRC_t<6, 10> & L_6x10, const MatRC_t<6, 1> & Rho, double betas[4])
{
