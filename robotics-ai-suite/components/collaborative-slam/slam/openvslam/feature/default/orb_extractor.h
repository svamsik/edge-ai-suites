...
/**
 * openvslam and feature namespace removed due to potential use of
 * external orb extractor (GPU based).
 *
 * For now, it is placed under openvlsam folder, but in future
 * should be moved in a separate folder.
 */
...
public:
  ...
  //! Get image pyramid
  void get_image_pyramid(std::vector<cv::Mat> & image_pyramid);
...
private:
  ...
  std::list<orb_extractor_node<cv::KeyPoint>> initialize_nodes(
    const std::vector<cv::KeyPoint> & keypts_to_distribute, const int min_x, const int max_x,
  ...
  //! size of maximum ORB patch radius
  static constexpr unsigned int orb_cell_size_ = 64;
