...
: max_num_keypts_(max_num_keypts),
  ...
  ini_fast_thr_(ini_fast_thr),
  min_fast_thr(min_fast_thr),
...
: orb_params(
    yaml_node["Feature.max_num_keypoints"].as<unsigned int>(2000),
    ...
    yaml_node["Feature.mask_rectangles"].as<std::vector<std::vector<float>>>(
...
void orb_params::show_parameters() const
{
  ...
  if (!mask_rects_.empty()) {
    ...
    for (const auto & mask_rect : mask_rects_) {
      std::cout << "  - [" << mask_rect.at(0) << ", " << mask_rect.at(1) << ", " << mask_rect.at(2)
                << ", " << mask_rect.at(3) << "]" << std::endl;
    }
  }
}

std::vector<float> orb_params::calc_scale_factors(
...
std::vector<float> orb_params::calc_inv_scale_factors(
  const unsigned int num_scale_levels, const float scale_factor)
{
...
std::vector<float> orb_params::calc_level_sigma_sq(
  const unsigned int num_scale_levels, const float scale_factor)
{
...
std::vector<float> orb_params::calc_inv_level_sigma_sq(
  const unsigned int num_scale_levels, const float scale_factor)
{
  float scale_factor_at_level = 1.0;
