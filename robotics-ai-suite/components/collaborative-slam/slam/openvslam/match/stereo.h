...
public:
  ...
  stereo(
    const std::vector<cv::Mat> & left_image_pyramid,
    const std::vector<cv::Mat> & right_image_pyramid, const std::vector<cv::KeyPoint> & keypts_left,
    const std::vector<cv::KeyPoint> & keypts_right, const cv::Mat & descs_left,
    const cv::Mat & descs_right, const std::vector<float> & scale_factors,
...
private:
   ...
   * @return
   */
  std::vector<std::vector<unsigned int>> get_right_keypoint_indices_in_each_row(
   ...
   */
  void find_closest_keypoints_in_stereo(
   ...
   * @return
   */
  bool compute_subpixel_disparity(
  ...
  static constexpr unsigned int hamm_dist_thr_ =
    (match::HAMMING_DIST_THR_HIGH + match::HAMMING_DIST_THR_LOW) / 2;
};
