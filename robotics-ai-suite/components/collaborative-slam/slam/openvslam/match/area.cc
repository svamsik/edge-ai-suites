...
unsigned int area::match_in_consistent_area(
  data::frame & frm_1, data::frame & frm_2, std::vector<cv::Point2f> & prev_matched_pts,
  std::vector<int> & matched_indices_2_in_frm_1, int margin)
{
  ...
  for (unsigned int idx_1 = 0; idx_1 < frm_1.undist_keypts_.size(); ++idx_1) {
    ...
    const auto indices = frm_2.get_keypoints_in_cell(
      prev_matched_pts.at(idx_1).x, prev_matched_pts.at(idx_1).y, margin, scale_level_1,
      scale_level_1);
    if (indices.empty()) {
      continue;
    }
    ...
    // best_idx_2
    // (matched_indices_1_in_frm_2.at(best_idx_2)
