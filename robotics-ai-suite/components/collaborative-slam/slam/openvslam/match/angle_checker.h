...
public:
   ...
   * Constructor
   */
  explicit angle_checker(
    const unsigned int histogram_length = 30, const unsigned int num_bins_thr = 3);
...
template <typename T>
angle_checker<T>::angle_checker(
  const unsigned int histogram_length, const unsigned int num_bins_thr)
: histogram_length_(histogram_length),
  inv_histogram_length_(1.0f / histogram_length),
...
// difference of major angle of the landmark
template <typename T>
inline void angle_checker<T>::append_delta_angle(float delta_angle, const T & match)
{
  ...
  // inv is 1/30, thus if angle is less than 30, then bin will be 0 or 1
  const auto bin = static_cast<unsigned int>(cvRound(delta_angle * inv_histogram_length_));

  if (bin >= histogram_length_) {
    spdlog::warn(
      "{}: beyond the boundary of angle_histogram_ vector, size: {}", __func__, histogram_length_);
    return;
  }
...
std::vector<T> angle_checker<T>::get_valid_matches() const
{
  ...
  for (unsigned int bin = 0; bin < histogram_length_; ++bin) {
    const bool is_valid_match = std::any_of(
      bins.begin(), bins.begin() + num_bins_thr_, [bin](const unsigned int i) { return bin == i; });
    if (is_valid_match) {
...
std::vector<T> angle_checker<T>::get_invalid_matches() const
{
  ...
  // get the three maximum angle difference
  const auto bins = index_sort_by_size(angle_histogram_);
  // bin is from 0~30
  for (unsigned int bin = 0; bin < histogram_length_; ++bin) {
    // three bins to check if at least one satisfies the condition
    const bool is_valid_match = std::any_of(
      bins.begin(), bins.begin() + num_bins_thr_, [bin](const unsigned int i) { return bin == i; });
    if (!is_valid_match) {
      const auto bin_begin_iter = angle_histogram_.at(bin).cbegin();
...
template <typename T>
std::vector<unsigned int> angle_checker<T>::index_sort_by_size(
  const std::vector<std::vector<T>> & vec) const
{
  std::vector<unsigned int> indices(vec.size());
  std::iota(indices.begin(), indices.end(), 0);
  std::sort(indices.begin(), indices.end(), [&vec](const unsigned int a, const unsigned int b) {
