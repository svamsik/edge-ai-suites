...
unsigned int projection::match_frame_and_landmarks(
  data::frame & frm, const std::vector<data::landmark *> & local_landmarks,
  const float original_margin) const
{
  ...
  for (auto local_lm : local_landmarks) {
    ...
    int start_predict_scale_level, end_predict_scale_level;

    if (local_lm->come_from_server()) {
      margin = original_margin * 1.5;
      start_predict_scale_level = 0;
      end_predict_scale_level = frm.num_scale_levels_ - 1;
    } else {
      start_predict_scale_level = pred_scale_level - 1;
      end_predict_scale_level = pred_scale_level;
      margin = original_margin;
    }

    const auto indices_in_cell = frm.get_keypoints_in_cell(
      local_lm->reproj_in_tracking_(0), local_lm->reproj_in_tracking_(1),
      margin * frm.scale_factors_.at(pred_scale_level), start_predict_scale_level,
      end_predict_scale_level);
    ...
    if (!lm_desc.ptr<uint32_t>()) {
      spdlog::debug(
        "invalid descriptor of this landmark: {}  from server : {}", local_lm->id_,
        local_lm->come_from_server());
      spdlog::debug("This landmark has  {} keyframe obsevations", local_lm->num_observations());
      continue;
    }
    ...
    for (const auto idx : indices_in_cell) {
      ...
      if (0 < frm.stereo_x_right_.at(idx)) {
        const auto reproj_error =
          std::abs(local_lm->x_right_in_tracking_ - frm.stereo_x_right_.at(idx));
    ...
    if (best_hamm_dist <= HAMMING_DIST_THR_HIGH) {
      // lowe's ratio test
      if (
        best_scale_level == second_best_scale_level &&
        best_hamm_dist > lowe_ratio_ * second_best_hamm_dist) {
        continue;
      }
...
unsigned int projection::match_current_and_last_frames(
  data::frame & curr_frm, const data::frame & last_frm, const float margin) const
{
  ...
  const bool assume_forward =
    (curr_frm.camera_->setup_type_ == camera::setup_type_t::Monocular ||
     (curr_frm.camera_->setup_type_ == camera::setup_type_t::Monocular_Inertial &&
      data::IMU_Preintegration::state_ == data::IMU_Preintegration::Not_Initialized))
      ? false
      : trans_lc(2) > curr_frm.camera_->true_baseline_;

  const bool assume_backward =
    (curr_frm.camera_->setup_type_ == camera::setup_type_t::Monocular ||
     (curr_frm.camera_->setup_type_ == camera::setup_type_t::Monocular_Inertial &&
      data::IMU_Preintegration::state_ == data::IMU_Preintegration::Not_Initialized))
      ? false
  ...
  for (unsigned int idx_last = 0; idx_last < last_frm.num_keypts_; ++idx_last) {
    ...
    if (assume_forward) {
      indices = curr_frm.get_keypoints_in_cell(
        reproj(0), reproj(1), margin * curr_frm.scale_factors_.at(last_scale_level),
        last_scale_level, last_frm.num_scale_levels_ - 1);
    } else if (assume_backward) {
      indices = curr_frm.get_keypoints_in_cell(
        reproj(0), reproj(1), margin * curr_frm.scale_factors_.at(last_scale_level), 0,
        last_scale_level);
    } else {
      indices = curr_frm.get_keypoints_in_cell(
        reproj(0), reproj(1), margin * curr_frm.scale_factors_.at(last_scale_level),
...
unsigned int projection::match_frame_and_keyframe(
  ...
  const unsigned int hamm_dist_thr) const
{
  ...
  for (unsigned int idx = 0; idx < landmarks.size(); idx++) {
    ...
    const int min_level = is_server_node ? 0 : pred_scale_level - 1;
    const int max_level = is_server_node ? curr_frm.num_scale_levels_ - 1 : pred_scale_level + 1;
    const auto indices = curr_frm.get_keypoints_in_cell(
      reproj(0), reproj(1), margin * curr_frm.scale_factors_.at(pred_scale_level), min_level,
      max_level);
    ...
    if (hamm_dist_thr < best_hamm_dist || best_idx < 0) {
      continue;
    }
...
unsigned int projection::match_by_Sim3_transform(
  data::keyframe * keyfrm, const Mat44_t & Sim3_cw, const std::vector<data::landmark *> & landmarks,
  std::vector<data::landmark *> & matched_lms_in_keyfrm, const float margin) const
{
  ...
  std::set<data::landmark *> already_matched(
    matched_lms_in_keyfrm.begin(), matched_lms_in_keyfrm.end());
  ...
  for (auto lm : landmarks) {
    ...
    float x_right;
    const bool in_image =
      keyfrm->camera_->reproject_to_image(rot_cw, trans_cw, pos_w, reproj, x_right);
    ...
    const auto indices = keyfrm->get_keypoints_in_cell(
      reproj(0), reproj(1), margin * keyfrm->scale_factors_.at(pred_scale_level));
    ...
    for (const auto idx : indices) {
      ...
      // TODO: keyfrm->get_keypts_in_cell()
...
unsigned int projection::match_keyframes_mutually(
  ...
  const Mat33_t & rot_12, const Vec3_t & trans_12, const float margin) const
{
  ...
  if (s_12 == 0.0f) {
    throw std::runtime_error("s_12 must be non-zero");
  }
  ...
  // = s_rot_21 * rot_1w * pos_w + s_rot_21 * trans_1w + trans_21
  {
    ...
    for (unsigned int idx_1 = 0; idx_1 < landmarks_1.size(); ++idx_1) {
      ...
      float x_right;
      const bool in_image =
        keyfrm_2->camera_->reproject_to_image(s_rot_21w, trans_21w, pos_w, reproj, x_right);
      ...
      const auto indices = keyfrm_2->get_keypoints_in_cell(
        reproj(0), reproj(1), margin * keyfrm_2->scale_factors_.at(pred_scale_level));
  ...
  // = s_rot_12 * rot_2w * pos_w + s_rot_12 * trans_2w + trans_12
  {
    ...
    for (unsigned int idx_2 = 0; idx_2 < landmarks_2.size(); ++idx_2) {
      ...
      float x_right;
      const bool in_image =
        keyfrm_2->camera_->reproject_to_image(s_rot_12w, trans_12w, pos_w, reproj, x_right);
      ...
      const auto indices = keyfrm_1->get_keypoints_in_cell(
        reproj(0), reproj(1), margin * keyfrm_1->scale_factors_.at(pred_scale_level));
