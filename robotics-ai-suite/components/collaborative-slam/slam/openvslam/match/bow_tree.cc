...
unsigned int bow_tree::match_frame_and_keyframe(
    ...
    if (keyfrm_itr->first == frm_itr->first) {
      ...
      for (const auto keyfrm_idx : keyfrm_indices) {
        if (keyfrm_idx >= keyfrm_lms.size()) {
          spdlog::warn(
            "{} beyond the boundary of keyfrm_lms vector, size: {}", keyfrm_idx, keyfrm_lms.size());
          continue;
        }
        ...
        if (
          keyfrm->descriptors_.rows < 0 ||
          keyfrm_idx >= static_cast<unsigned int>(keyfrm->descriptors_.rows)) {
          spdlog::warn(
            "{} beyond the boundary of keyfrm descriptors_ rows, size: {}", keyfrm_idx,
        ...
        for (const auto frm_idx : frm_indices) {
          if (frm_idx >= matched_lms_in_frm.size()) {
            spdlog::warn(
              "{} beyond the boundary of matched_lms_in_frm vector, size: {}", frm_idx,
          ...
          if (
            frm.descriptors_.rows < 0 ||
            frm_idx >= static_cast<unsigned int>(frm.descriptors_.rows)) {
            spdlog::warn(
              "{} beyond the boundary of keyfrm descriptors_ rows, size: {}", frm_idx,
        ...
        if (
          best_frm_idx < 0 ||
          static_cast<unsigned int>(best_frm_idx) >= matched_lms_in_frm.size()) {
          spdlog::warn(
            "{} beyond the boundary of matched_lms_in_frm vector, size: {}", best_frm_idx,
            matched_lms_in_frm.size());
          continue;
        }
        ...
        if (check_orientation_ && can_check_orientation) {
          if (
            keyfrm_idx >= keyfrm->keypts_.size() ||
            static_cast<unsigned int>(best_frm_idx) >= frm.keypts_.size()) {
            spdlog::warn(
              "{} beyond the boundary of keyfrm keypts_ vector, size: {} or {} \
                                     beyond the boundary of frm keypts vector, size: {}",
              keyfrm_idx, keyfrm->keypts_.size(), best_frm_idx, frm.keypts_.size());
            continue;
          }
  ...
  if (check_orientation_ && can_check_orientation) {
    ...
    for (const auto invalid_idx : invalid_matches) {
      if (invalid_idx < 0 || static_cast<unsigned int>(invalid_idx) >= matched_lms_in_frm.size()) {
        spdlog::warn(
          "{} beyond the boundary of matched_lms_in_frm vector, size: {}", invalid_idx,
          matched_lms_in_frm.size());
        continue;
      }
...
unsigned int bow_tree::match_keyframes(
