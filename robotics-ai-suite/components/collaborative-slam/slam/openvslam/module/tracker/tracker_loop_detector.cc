...
#include "tracker_loop_detector.h"
...
void tracker_loop_detector::set_current_keyframe(data::keyframe * keyfrm) { cur_keyfrm_ = keyfrm; }

bool tracker_loop_detector::detect_loop_candidates()
{
  ...
  // 1-2. inquiring to the BoW database about the similar keyframe whose score
  //      is lower thanmin_score
  ...
  //    the number of each of the candidate keyframe sets that detected are counted every time when
  //    this member functions is called if the keyframe sets were detected at the previous call,
  //    is is contained in `cont_detected_keyfrm_sets_` (note that "match of two keyframe sets"
  //    means the intersection of the two sets is NOT empty)
  ...
  std::cout << "loop_candidates_to_validate_size: " << loop_candidates_to_validate_.size()
            << std::endl;
...
bool tracker_loop_detector::validate_candidates()
{
  ...
  //    keyframe using the observed landmarks then, select ONE candaite
  ...
  const bool candidate_is_found = select_loop_candidate_via_Sim3(
    ...
    curr_match_lms_observed_in_cand_);
  ...
  //    keyframe, then acquire the extra 2D-3D matches

  // matches between the keypoints in the current and the landmarks observed in the covisibilities
...
bool tracker_loop_detector::select_loop_candidate_via_Sim3(
  ...
  std::vector<data::landmark *> & curr_match_lms_observed_in_cand) const
{
  ...
  // the observed landmarks the Sim3 is estimated both in linear and non-linear ways
