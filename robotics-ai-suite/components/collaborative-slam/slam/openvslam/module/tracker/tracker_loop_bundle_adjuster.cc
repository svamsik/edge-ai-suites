...
#include "tracker_loop_bundle_adjuster.h"
...
void tracker_loop_bundle_adjuster::set_mapping_module(mapping_module * mapper) { mapper_ = mapper; }

void tracker_loop_bundle_adjuster::optimize(const KeyframeID identifier)
{
  ...
  const auto global_bundle_adjuster =
    optimize::global_bundle_adjuster(map_db_->origin_keyfrm_->id_, map_db_, num_iter_, false);
  global_bundle_adjuster.optimize(identifier, &abort_loop_BA_);

  {
    std::unique_lock<std::mutex> lock1(mtx_thread_);
    ...
    // unlock the mtx_thread_ in case that current thread goes to sleep
    lock1.unlock();
