...
two_view_triangulator::two_view_triangulator(
  data::keyframe * keyfrm_1, data::keyframe * keyfrm_2, const float rays_parallax_deg_thr)
: keyfrm_1_(keyfrm_1),
  keyfrm_2_(keyfrm_2),
  rot_1w_(keyfrm_1->get_rotation()),
  rot_w1_(rot_1w_.transpose()),
  trans_1w_(keyfrm_1->get_translation()),
  cam_pose_1w_(keyfrm_1->get_cam_pose()),
  cam_center_1_(keyfrm_1->get_cam_center()),
  camera_1_(keyfrm_1->camera_),
  rot_2w_(keyfrm_2->get_rotation()),
  rot_w2_(rot_2w_.transpose()),
  trans_2w_(keyfrm_2->get_translation()),
  cam_pose_2w_(keyfrm_2->get_cam_pose()),
  cam_center_2_(keyfrm_2->get_cam_center()),
  camera_2_(keyfrm_2->camera_),
...
bool two_view_triangulator::triangulate(
  const unsigned idx_1, const unsigned int idx_2, Vec3_t & pos_w) const
{
  ...
  const bool triangulate_with_two_cameras =
    ...
    ((!is_stereo_1 && !is_stereo_2) && 0.0 < cos_rays_parallax &&
     cos_rays_parallax < cos_rays_parallax_thr_)
    // check if the parallax between the two cameras is larger than the stereo parallax
    || ((is_stereo_1 || is_stereo_2) && 0.0 < cos_rays_parallax &&
        cos_rays_parallax < cos_stereo_parallax);
  ...
  if (
    !check_depth_is_positive(pos_w, rot_1w_, trans_1w_, camera_1_) ||
  ...
  if (
    !check_reprojection_error(
      pos_w, rot_1w_, trans_1w_, camera_1_, keypt_1.pt, keypt_1_x_right,
      keyfrm_1_->level_sigma_sq_.at(keypt_1.octave), is_stereo_1) ||
    !check_reprojection_error(
      pos_w, rot_2w_, trans_2w_, camera_2_, keypt_2.pt, keypt_2_x_right,
  ...
  if (!check_scale_factors(
        pos_w, keyfrm_1_->scale_factors_.at(keypt_1.octave),
...
template <typename T>
bool two_view_triangulator::check_reprojection_error(
  const Vec3_t & pos_w, const Mat33_t & rot_cw, const Vec3_t & trans_cw,
  camera::base * const camera, const cv::Point_<T> & keypt, const float x_right,
  const float sigma_sq, const bool is_stereo) const
{
  ...
  if (is_stereo) {
    ...
    if (
      (chi_sq_3D * sigma_sq) <
      (reproj_err.squaredNorm() + reproj_err_x_right * reproj_err_x_right)) {
      return false;
    }
  } else {
    const Vec2_t reproj_err = reproj_in_cur - keypt;
