...
local_map_cleaner::local_map_cleaner(const bool is_monocular) : is_monocular_(is_monocular) {}

void local_map_cleaner::reset() { fresh_landmarks_.clear(); }

unsigned int local_map_cleaner::remove_redundant_landmarks(const KeyframeID cur_keyfrm_id)
{
...
unsigned int local_map_cleaner::remove_redundant_keyframes(data::keyframe * cur_keyfrm) const
{
  ...
  unsigned int num_removed = 0;
  constexpr float imu_velocity_thr = 0.5;
  ...
  for (const auto covisibility : cur_covisibilities) {
    // cannot remove the keyframe when the camera is static to avoid wrong preintegration result in vio mode
    if (cur_keyfrm->camera_->use_imu_) {
      // TODO: reference to ORBSLAM3 for further improvement
      if (cur_keyfrm->get_imu_velocity().norm() < imu_velocity_thr) {
        spdlog::debug("avoid removing keyframe {} in vio", covisibility->id_);
        continue;
      }
    }
    ...
    if (
      covisibility->id_ <= cur_keyfrm->id_ &&
    ...
    if (
      (num_valid_obs != 0) &&
      (redundant_obs_ratio_thr <= static_cast<float>(num_redundant_obs) / num_valid_obs)) {
      ++num_removed;
      spdlog::debug("Remove redundant keyframe: {}", covisibility->id_);
      covisibility->prepare_for_erasing(true, false);
    }
  }
...
void local_map_cleaner::count_redundant_observations(
  data::keyframe * keyfrm, unsigned int & num_valid_obs, unsigned int & num_redundant_obs) const
{
  ...
  for (unsigned int idx = 0; idx < landmarks.size(); ++idx) {
    ...
    // if depth is not within the valid range, it won't be considered
