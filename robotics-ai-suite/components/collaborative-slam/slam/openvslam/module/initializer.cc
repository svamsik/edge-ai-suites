...
initializer::initializer(
  std::shared_ptr<univloc_tracker::Config> cfg, camera::setup_type_t setup_type,
...
bool initializer::try_initialize_for_monocular(data::frame & curr_frm)
{
  ...
  if (
    (curr_frm.keypts_.size() <= keypts_num_thre) ||
  ...
  if (num_matches < min_num_triangulated_) {
    std::cout << "Too few num_matches " << num_matches << " below thr " << min_num_triangulated_
              << std::endl;
...
bool initializer::create_map_for_monocular(data::frame & curr_frm)
{
  ...
  for (unsigned int init_idx = 0; init_idx < init_matches_.size(); init_idx++) {
    ...
    if (curr_frm.camera_->use_imu_) {
      ...
      lm = new data::landmark(
        rot_init * init_triangulated_pts.at(init_idx) + trans_init, curr_keyfrm, map_db_);
    }
  ...
  const auto global_bundle_adjuster =
    optimize::global_bundle_adjuster(init_keyfrm->id_, map_db_, num_ba_iters_, true);
  ...
  spdlog::info(
    "new map created with {} points: frame {} - frame {}", map_db_->get_num_landmarks(),
...
void initializer::scale_map(
  data::keyframe * init_keyfrm, data::keyframe * curr_keyfrm, const double scale)
{
  // scaling keyframes
...
bool initializer::try_initialize_for_stereo(data::frame & curr_frm)
{
  ...
  unsigned int num_valid_depths = std::count_if(
    curr_frm.depths_.begin(), curr_frm.depths_.end(), [](const float depth) { return 0 < depth; });
...
bool initializer::create_map_for_stereo(data::frame & curr_frm)
{
  ...
  spdlog::info(
    "new map created with {} points: frame {}", map_db_->get_num_landmarks(), curr_frm.id_);
  state_ = initializer_state_t::Succeeded;
