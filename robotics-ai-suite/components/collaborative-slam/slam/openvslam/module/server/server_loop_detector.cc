...
server_loop_detector::server_loop_detector(
...
bool server_loop_detector::detect_loop_candidates()
{
  ...
  if (!loop_detector_is_enabled_) {
    spdlog::debug("!loop_detector_is_enabled_");
  }

  if (
    !loop_detector_is_enabled_ || !cur_keyfrm_->available_for_loop_detector_ ||
    // use the same magic number as openvslam, we don't want to perform server optimization too often
    cur_keyfrm_->id_ <
      prev_server_optim_keyfrm_id_[cur_keyfrm_->client_id_] + LOOP_DETECT_INTERVAL) {
    spdlog::debug(
      "avoid loop detection, current keyfrm id : {}, last server optimization keyfrm id : {}",
  ...
  // 1-1. before inquiring, compute the minimum score of BoW similarity between the current
  // and each of the covisibilities

  const float min_score = compute_min_score_in_covisibilities(cur_keyfrm_);

  // 1-2. inquiring to the BoW database about the similar keyframe whose score is lower
  //      than min_score
  ...
  // 2. From now on, we treat each of the candidates as "keyframe set" in order to improve
  //    robustness of loop detection. The number of each of the candidate keyframe sets that
  //    detected are counted every time when this member functions is called if the keyframe sets
  //    were detected at the previous call, it is contained in `cont_detected_keyfrm_sets_` (note
  //    that "match of two keyframe sets" means the intersection of the two sets is NOT empty)
...
bool server_loop_detector::validate_candidates(g2o::Sim3 & g2o_sim3_cand_to_curr)
{
  ...
  // 1. for each of the candidates, estimate and validate the Sim3 between it and the current
  //    keyframe using the observed landmarks. Then, select ONE candidate

  const bool candidate_is_found = select_loop_candidate_via_Sim3(
    loop_candidates_to_validate_, selected_candidate_, g2o_Sim3_world_to_curr_,
  ...
  spdlog::debug(
    "detect loop candidate via Sim3 estimation: keyframe {} - keyframe {}",
    selected_candidate_->id_, cur_keyfrm_->id_);

  // 2. reproject the landmarks observed in covisibilities of the selected candidate to the current
  //    keyframe, then acquire the extra 2D-3D matches between the keypoints in the current keyframe
  //    and the landmarks observed in the covisibilities of the selected candidate
  ...
  // reproject the landmarks observed in the covisibilities of the candidate to the current keyframe
  // using Sim3 `Sim3_world_to_curr_`, then, acquire the extra 2D-3D matches however, landmarks in
  // `curr_match_lms_observed_in_cand_` are already matched with keypoints in the current keyframe,
  // thus they are excluded from the reprojection
  match::projection projection_matcher(0.75, true);
  projection_matcher.match_by_Sim3_transform(
    cur_keyfrm_, Sim3_world_to_curr_, curr_match_lms_observed_in_cand_covis_,
  ...
  // if the number of matches is greater than the threshold, adopt the selected candidate
  // for the loop correction
  ...
  if (num_final_matches_thr <= num_final_matches) {
    ...
    spdlog::debug(
      "destruct loop candidate because enough matches not acquired (< {})", num_final_matches_thr);
...
bool server_loop_detector::select_loop_candidate_via_Sim3(
  const std::vector<data::keyframe *> & loop_candidates, data::keyframe *& selected_candidate,
...
{
  // estimate and the Sim3 between the current keyframe and each of the candidates using
  ...
  for (const auto candidate : loop_candidates) {
    ...
    // estimate the matches between the keypoints in the current keyframe and the landmarks
    // observed in the candidate
    curr_match_lms_observed_in_cand.clear();
    const auto num_matches =
      bow_matcher.match_keyframes(cur_keyfrm_, candidate, curr_match_lms_observed_in_cand);
    ...
    solve::sim3_solver solver(
      cur_keyfrm_, candidate, curr_match_lms_observed_in_cand, fix_scale_in_Sim3_estimation_, 20);
    ...
    spdlog::debug(
      "found loop candidate via linear Sim3 estimation: keyframe {} - keyframe {}", candidate->id_,
      cur_keyfrm_->id_);
    ...
    projection_matcher.match_keyframes_mutually(
      cur_keyfrm_, candidate, curr_match_lms_observed_in_cand, scale_cand_to_curr, rot_cand_to_curr,
      trans_cand_to_curr, 7.5);
    ...
    spdlog::debug(
      "found loop candidate via nonlinear Sim3 optimization: keyframe {} - keyframe {}",
      candidate->id_, cur_keyfrm_->id_);
    ...
    g2o_Sim3_world_to_curr =
      g2o_sim3_cand_to_curr *
      g2o::Sim3(candidate->get_rotation(), candidate->get_translation(), 1.0);
...
void server_loop_detector::set_server_optim_keyframe_id(
  const ClientID client_id, const KeyframeID server_optim_keyfrm_id)
{
