...
server_loop_bundle_adjuster::server_loop_bundle_adjuster(
  data::map_database * map_db, const unsigned int num_iter)
: loop_bundle_adjuster(map_db, num_iter)
{
}

void server_loop_bundle_adjuster::optimize(
  const KeyframeID identifier, MapID map_id1, MapID map_id2)
{
  ...
  auto fixed_frame_id = map_id1 < map_id2 ? origin_keyframe1->id_ : origin_keyframe2->id_;

  if (map_id1 != map_id2) {
    auto cur_keyframe = map_db_->get_keyframe(identifier);
    if (!cur_keyframe) {
      spdlog::error(
        "failed to get the current keyframe from map database based on the identifier {}!",
        identifier);
      return;
    }

    if (
      origin_keyframe1->client_id_ !=
      cur_keyframe->client_id_) {  // cur_keyframe is not the loop keyframe
  ...
  const auto global_bundle_adjuster =
    optimize::global_bundle_adjuster(fixed_frame_id, map_db_, num_iter_, false);
...

  {
    ...
    if (num_exec_loop_BA != num_exec_loop_BA_ || abort_loop_BA_) {
      spdlog::info(
        "abort loop bundle adjustment. num_exec_loop_BA: {}, num_exec_loop_BA_: {}, "
        "abort_loop_BA_: {}",
    ...
    for (const auto & origin_keyfrm : origin_keyframes_map) {
      if (
        origin_keyfrm.second->get_map_id() == map_id1 ||
        origin_keyfrm.second->get_map_id() == map_id2) {
    ...
    for (auto lm : landmarks) {
      ...
      if (lm->loop_BA_identifier_ == identifier) {
        ...
        assert(
          (std::string("maps :") + std::to_string(map_id1) + ", " + std::to_string(map_id2),
