...
public:
   ...
   * Constructor
   */
  explicit two_view_triangulator(
    data::keyframe * keyfrm_1, data::keyframe * keyfrm_2, const float rays_parallax_deg_thr = 1.0);
...
private:
   ...
   */
  template <typename T>
  bool check_reprojection_error(
    const Vec3_t & pos_w, const Mat33_t & rot_cw, const Vec3_t & trans_cw, camera::base * camera,
   ...
   */
  bool check_scale_factors(
    const Vec3_t & pos_w, const float scale_factor_1, const float scale_factor_2) const;
...
inline bool two_view_triangulator::check_depth_is_positive(
  const Vec3_t & pos_w, const Mat33_t & rot_cw, const Vec3_t & trans_cw,
...
inline bool two_view_triangulator::check_scale_factors(
  const Vec3_t & pos_w, const float scale_factor_1, const float scale_factor_2) const
{
