...
keyframe_inserter::keyframe_inserter(
  const camera::setup_type_t setup_type, const float true_depth_thr, data::map_database * map_db,
  data::bow_database * bow_db, const unsigned int min_num_frms, const unsigned int max_num_frms)
...
bool keyframe_inserter::new_keyframe_is_needed(
  const data::frame & curr_frm, const unsigned int num_tracked_lms,
  const data::keyframe & ref_keyfrm) const
{
  ...
  if (
    ((num_tracked_lms < num_tracked_lms_thr_high) && (num_tracked_lms > num_tracked_lms_thr_low)) &&
  ...
  if (!cond_b) {
    spdlog::debug(
      "Condition b is not satisfied! num_tracked_lms: {}, num_reliable_lms:{} ", num_tracked_lms,
      num_reliable_lms);
    return false;
  }
...
data::keyframe * keyframe_inserter::insert_new_keyframe(data::frame & curr_frm, bool send_to_server)
{
  ...
  for (unsigned int idx = 0; idx < curr_frm.num_keypts_; ++idx) {
    ...
    if (curr_frm.segment_outlier_flags_[idx]) {
      continue;
    }
#endif
    ...
    if (lm->come_from_server()) {
      ...
      if (p_landmark) {
        ...
        // This server landmark will be add to local map, so it need to be erased from server
        // landmark container
        if (send_to_server) {
  ...
  // do pre_integration calculation ahead of queue_keyframe
  if (
    depth_idx_pairs.empty() && (setup_type_ == camera::setup_type_t::Stereo_Inertial ||
