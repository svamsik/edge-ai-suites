...
relocalizer::relocalizer(
  data::bow_database * bow_db, camera::setup_type_t camera_type, const double bow_match_lowe_ratio,
...
bool relocalizer::relocalize(data::frame & curr_frm)
{
  ...
  for (unsigned int i = 0; i < num_candidates; ++i) {
    ...
    auto pnp_solver = setup_pnp_solver(
      valid_indices, curr_frm.bearings_, curr_frm.keypts_, matched_landmarks.at(i),
    ...
    const auto inlier_indices =
      util::resample_by_indices(valid_indices, pnp_solver->get_inlier_flags());
    ...
    auto num_found = proj_matcher_.match_frame_and_keyframe(
      curr_frm, reloc_candidates.at(i), already_found_landmarks, 10, 100);
    // discard the candidate if the number of the inliers is less than the threshold
    ...
    if (num_valid_obs < min_num_valid_obs_) {
      ...
      auto num_additional = proj_matcher_.match_frame_and_keyframe(
        curr_frm, reloc_candidates.at(i), already_found_landmarks, 3, 64);
    ...
    // TODO: current frame reference keyframe
...
bool relocalizer::relocalize(
  data::frame & curr_frm, MapID & map_id, data::map_database * map_db,
  data::bow_vocabulary * bow_vocab, double near_distance, double far_distance, double back_distance)
{
  curr_frm.compute_bow();
  ...
  for (unsigned int i = 0; i < num_candidates; ++i) {
    ...
    // covisibility_keyframe means a virtual frame composed of landmarks in the 5 largest
    // covisibility keyframes
    std::shared_ptr<data::keyframe> covisibility_keyframe =
      get_covisibility_keyfrm(keyfrm, map_db, bow_vocab, 5);
    ...
    // It may be because orb was not specific enough and projection keyframe has more landmarks
    // so that there will have more mismatch
    matched_landmarks.at(i) = std::vector<data::landmark *>(curr_frm.num_keypts_, nullptr);
    const auto num_matches_first =
      bow_matcher_.match_frame_and_keyframe(keyfrm, curr_frm, matched_landmarks.at(i));

    const auto num_matches_twice = bow_matcher_.match_frame_and_keyframe(
      covisibility_keyframe.get(), curr_frm, matched_landmarks.at(i));
    ...
    // projection match based on the pre-optimized camera pose
    auto num_found = proj_matcher_.match_frame_and_keyframe(
      curr_frm, projection_keyframe.get(), already_found_landmarks, 10, 100);
    // discard the candidate if the number of the inliers is less than the threshold
    if (num_valid_obs + num_found < min_num_valid_obs_) {
      continue;
    }
    ...
    if (num_valid_obs < min_num_valid_obs_) {
      ...
      auto num_additional = proj_matcher_.match_frame_and_keyframe(
        curr_frm, projection_keyframe.get(), already_found_landmarks, 3, 64);
...
std::shared_ptr<data::keyframe> relocalizer::get_covisibility_keyfrm(
  data::keyframe * keyfrm, data::map_database * map_db, data::bow_vocabulary * bow_vocab,
  int top_number)
{
  std::vector<data::keyframe *> covisi_kfs =
    keyfrm->graph_node_->get_top_n_covisibilities(top_number);
  ...
  std::shared_ptr<data::keyframe> virtual_keyframe =
    get_virtual_keyfrm(keyfrm, map_db, bow_vocab, nearby_landmarks);

  return virtual_keyframe;
}

std::shared_ptr<data::keyframe> relocalizer::get_projection_keyfrm(
  data::keyframe * keyfrm, data::map_database * map_db, data::bow_vocabulary * bow_vocab,
  double near_distance, double far_distance, double back_distance)
{
  ...
  std::vector<data::landmark *> nearby_landmarks = map_db->get_landmarks_in_frustum(
    keyfrm->get_cam_pose(), keyfrm->camera_, keyfrm->get_map_id(), 0, false, point_3d, point_2d,
    convex_hull, near_distance, far_distance, back_distance);
...
std::shared_ptr<data::keyframe> relocalizer::get_virtual_keyfrm(
  data::keyframe * keyfrm, data::map_database * map_db, data::bow_vocabulary * bow_vocab,
  std::vector<data::landmark *> & nearby_landmarks)
{
  ...
  std::shared_ptr<data::keyframe> virtual_keyframe = std::make_shared<data::keyframe>(
    static_cast<unsigned int>(-1), static_cast<unsigned int>(-1), keyfrm->timestamp_, camera_pose,
    keyfrm->camera_, keyfrm->depth_thr_, num_keypts, keypts, undist_keypts, bearings,
    stereo_x_right, depths, descriptors, num_scale_levels, scale_factor, bow_vocab, bow_db_, map_db,
    10, 10);
  if (!virtual_keyframe) {
...
std::unique_ptr<solve::pnp_solver> relocalizer::setup_pnp_solver(
  const std::vector<unsigned int> & valid_indices, const eigen_alloc_vector<Vec3_t> & bearings,
  const std::vector<cv::KeyPoint> & keypts, const std::vector<data::landmark *> & matched_landmarks,
