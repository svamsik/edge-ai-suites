...
{
public:
  explicit frame_tracker(
    camera::base * camera, const unsigned int num_matches_thr = 20, bool use_odom = false,
    bool is_localization = false, data::map_database * map_db = nullptr);

  bool motion_based_track(
    data::frame & curr_frm, const data::frame & last_frm, const Mat44_t & velocity) const;

  bool lidar_based_track(
