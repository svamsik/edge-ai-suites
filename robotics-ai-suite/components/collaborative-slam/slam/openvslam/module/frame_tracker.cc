...
frame_tracker::frame_tracker(
  camera::base * camera, const unsigned int num_matches_thr, bool use_odom, bool is_localization,
...
bool frame_tracker::motion_based_track(
  data::frame & curr_frm, const data::frame & last_frm, const Mat44_t & velocity) const
{
  ...
  if (num_valid_matches < num_matches_thr_) {
    spdlog::debug(
      "motion based tracking failed: {} inlier matches < {}", num_valid_matches, num_matches_thr_);
    return false;
  } else {
    return true;
  }
...
bool frame_tracker::lidar_based_track(
  data::frame & curr_frm, const data::frame & last_frm, const Mat44_t & velocity,
  data::frame & last_lidar_frame, Eigen::Matrix4d & tf_lidar_camera) const
{
  match::projection projection_matcher(0.9, true);
...
bool frame_tracker::odom_based_track(
  ...
  const Mat44_t & predicted_pose_from_odom) const
{
  ...
  if (num_valid_matches < matches_thr) {
    spdlog::debug(
      "odom based tracking failed: {} inlier matches < {}", num_valid_matches, matches_thr);
    return false;
  } else {
    return true;
  }
...
bool frame_tracker::imu_based_track(
  data::frame & curr_frm, const data::frame & last_frm, const Mat44_t predicted_pose_from_imu) const
{
  match::projection projection_matcher(0.9, true);
...
bool frame_tracker::bow_match_based_track(
  data::frame & curr_frm, const data::frame & last_frm, data::keyframe * ref_keyfrm) const
{
  ...
  std::vector<data::landmark *> matched_lms_in_curr =
    std::vector<data::landmark *>(curr_frm.num_keypts_, nullptr);
  // check this func, why not matches?
  auto num_matches =
    bow_matcher.match_frame_and_keyframe(ref_keyfrm, curr_frm, matched_lms_in_curr);

  spdlog::debug("bow match based tracking num_matches is {}", num_matches);
  if (num_matches < num_matches_thr_) {
    spdlog::debug(
      "bow match based tracking failed: {} matches < {}", num_matches, num_matches_thr_);
    return false;
  }
  ...
  if (num_valid_matches < num_matches_thr_) {
    spdlog::debug(
      "bow match based tracking failed: {} inlier matches < {}", num_valid_matches,
      num_matches_thr_);
    return false;
  } else {
    spdlog::debug(
      "bow match based tracking success: {} inlier matches > {}", num_valid_matches,
      num_matches_thr_);
    return true;
  }
}

bool frame_tracker::robust_match_based_track(
  data::frame & curr_frm, const data::frame & last_frm, data::keyframe * ref_keyfrm) const
{
  ...
  std::vector<data::landmark *> matched_lms_in_curr;
  auto num_matches =
    robust_matcher.match_frame_and_keyframe(curr_frm, ref_keyfrm, matched_lms_in_curr);

  if (num_matches < num_matches_thr_) {
    spdlog::debug(
      "robust match based tracking failed: {} matches < {}", num_matches, num_matches_thr_);
    return false;
  }
  ...
  if (num_valid_matches < num_matches_thr_) {
    spdlog::debug(
      "robust match based tracking failed: {} inlier matches < {}", num_valid_matches,
      num_matches_thr_);
    return false;
  } else {
    spdlog::debug(
      "robust match based tracking success: {} inlier matches > {}", num_valid_matches,
      num_matches_thr_);
    return true;
  }
}
