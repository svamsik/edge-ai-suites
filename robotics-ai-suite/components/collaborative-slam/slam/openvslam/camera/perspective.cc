...
perspective::perspective(
  ...
  const unsigned int cols, const unsigned int rows, const double fps, const double fx,
  const double fy, const double cx, const double cy, const double k1, const double k2,
  const double p1, const double p2, const double k3, const double focal_x_baseline)
: base(
    name, setup_type, model_type_t::Perspective, color_order, cols, rows, fps, focal_x_baseline,
    focal_x_baseline / fx),
  fx_(fx),
  fy_(fy),
  cx_(cx),
  ...
  k3_(k3)
{
  ...
  inv_cell_height_ =
    static_cast<double>(num_grid_rows_) / (img_bounds_.max_y_ - img_bounds_.min_y_);
}

perspective::perspective(const YAML::Node & yaml_node)
: perspective(
    yaml_node["Camera.name"].as<std::string>(), load_setup_type(yaml_node),
    load_color_order(yaml_node), yaml_node["Camera.cols"].as<unsigned int>(),
    yaml_node["Camera.rows"].as<unsigned int>(), yaml_node["Camera.fps"].as<double>(),
    yaml_node["Camera.fx"].as<double>(), yaml_node["Camera.fy"].as<double>(),
    yaml_node["Camera.cx"].as<double>(), yaml_node["Camera.cy"].as<double>(),
    yaml_node["Camera.k1"].as<double>(), yaml_node["Camera.k2"].as<double>(),
    yaml_node["Camera.p1"].as<double>(), yaml_node["Camera.p2"].as<double>(),
    yaml_node["Camera.k3"].as<double>(), yaml_node["Camera.focal_x_baseline"].as<double>(0.0))
{
}

perspective::~perspective() { spdlog::debug("DESTRUCT: camera::perspective"); }
...
void perspective::show_parameters() const
{
  ...
  std::cout << "  - Horizontal FOV angle: " << fov_angle_ << std::endl;
  std::cout << "cv_cam_matrix: " << cv_cam_matrix_ << std::endl;
  std::cout << "cv_dist_params: " << cv_dist_params_ << std::endl;
}
...
image_bounds perspective::compute_image_bounds() const
{
  ...
  if (k1_ == 0 && k2_ == 0 && p1_ == 0 && p2_ == 0 && k3_ == 0) {
    ...
    const std::vector<cv::KeyPoint> corners{
      cv::KeyPoint(0.0, 0.0, 1.0),       // left top
...
void perspective::undistort_keypoints(
  const std::vector<cv::KeyPoint> & dist_keypts, std::vector<cv::KeyPoint> & undist_keypts) const
{
...
void perspective::convert_keypoints_to_bearings(
  const std::vector<cv::KeyPoint> & undist_keypts, eigen_alloc_vector<Vec3_t> & bearings) const
{
...
void perspective::convert_bearings_to_keypoints(
  const eigen_alloc_vector<Vec3_t> & bearings, std::vector<cv::KeyPoint> & undist_keypts) const
{
...
bool perspective::reproject_to_image(
  const Mat33_t & rot_cw, const Vec3_t & trans_cw, const Vec3_t & pos_w, Vec2_t & reproj,
  float & x_right) const
{
...
bool perspective::reproject_to_image_bound(
  const Mat33_t & rot_cw, const Vec3_t & trans_cw, const Vec3_t & pos_w, Vec2_t & reproj,
  float & x_right, float x_bound_expand, float y_bound_expand) const
{
  // convert to camera-coordinates
  const Vec3_t pos_c = rot_cw * pos_w + trans_cw;
  ...
  // check if the point is visible
  return img_bounds_.min_x_ - x_bound_expand < reproj(0) &&
         reproj(0) < img_bounds_.max_x_ + x_bound_expand &&
         img_bounds_.min_y_ - y_bound_expand < reproj(1) &&
         reproj(1) < img_bounds_.max_y_ + y_bound_expand;
}

bool perspective::reproject_to_bearing(
  const Mat33_t & rot_cw, const Vec3_t & trans_cw, const Vec3_t & pos_w, Vec3_t & reproj) const
{
  // convert to camera-coordinates
  ...
  // check if the point is visible
  return img_bounds_.min_x_ < x && x < img_bounds_.max_x_ && img_bounds_.min_y_ < y &&
         y < img_bounds_.max_y_;
}
...
void perspective::set_cv_cam_matrix(float fx, float fy, float cx, float cy)
{
  cv_cam_matrix_ = (cv::Mat_<float>(3, 3) << fx, 0, cx, 0, fy, cy, 0, 0, 1);
}
