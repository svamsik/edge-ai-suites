...
enum class setup_type_t {
  ...
  RGBD_Inertial = 3,
  Stereo_Inertial = 4,
  Monocular_Inertial = 5
};

const std::array<std::string, 6> setup_type_to_string = {
  {"Monocular", "Stereo", "RGBD", "RGBD_Inertial", "Stereo_Inertial", "Monocular_Inertial"}};

enum class model_type_t { Perspective = 0, Fisheye = 1, Equirectangular = 2 };

const std::array<std::string, 3> model_type_to_string = {
  {"Perspective", "Fisheye", "Equirectangular"}};

enum class color_order_t { Gray = 0, RGB = 1, BGR = 2 };
...
public:
  //! Constructor
  base(
    const std::string & name, const setup_type_t setup_type, const model_type_t model_type,
    const color_order_t color_order, const unsigned int cols, const unsigned int rows,
    const double fps, const double focal_x_baseline, const double true_baseline,
  ...
  virtual void undistort_keypoints(
    const std::vector<cv::KeyPoint> & dist_keypts,
  ...
  virtual void convert_keypoints_to_bearings(
  ...
  virtual void convert_bearings_to_keypoints(
  ...
  virtual bool reproject_to_image(
    const Mat33_t & rot_cw, const Vec3_t & trans_cw, const Vec3_t & pos_w, Vec2_t & reproj,
    float & x_right) const = 0;

  virtual bool reproject_to_image_bound(
    const Mat33_t & rot_cw, const Vec3_t & trans_cw, const Vec3_t & pos_w, Vec2_t & reproj,
    float & x_right, float x_bound_expand = 0, float y_bound_expand = 0) const = 0;

  //! Reproject the specified 3D point to bearing vector using camera pose
  //! (Not depends on any projection models)
  //! (reprojected to inside of image -> true, to outside of image -> false)
  virtual bool reproject_to_bearing(
    const Mat33_t & rot_cw, const Vec3_t & trans_cw, const Vec3_t & pos_w,
    Vec3_t & reproj) const = 0;

  //! Encode camera information as JSON
  virtual nlohmann::json to_json() const = 0;

  //! Set cv_cam_matrix_ variable
  virtual void set_cv_cam_matrix(float fx, float fy, float cx, float cy) = 0;
};
