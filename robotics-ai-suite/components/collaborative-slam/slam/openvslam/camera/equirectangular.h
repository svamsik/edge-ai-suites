...
public:
  ...
  void undistort_keypoints(
    ...
    std::vector<cv::KeyPoint> & undist_keypts) const override final;
  ...
  void convert_keypoints_to_bearings(
    const std::vector<cv::KeyPoint> & undist_keypts,
    eigen_alloc_vector<Vec3_t> & bearings) const override final;
  ...
  void convert_bearings_to_keypoints(
    const eigen_alloc_vector<Vec3_t> & bearings,
    std::vector<cv::KeyPoint> & undist_keypts) const override final;

  bool reproject_to_image(
    const Mat33_t & rot_cw, const Vec3_t & trans_cw, const Vec3_t & pos_w, Vec2_t & reproj,
    float & x_right) const override final;

  bool reproject_to_image_bound(
    const Mat33_t & rot_cw, const Vec3_t & trans_cw, const Vec3_t & pos_w, Vec2_t & reproj,
    float & x_right, float x_bound_expand = 0, float y_bound_expand = 0) const override final;

  bool reproject_to_bearing(
    const Mat33_t & rot_cw, const Vec3_t & trans_cw, const Vec3_t & pos_w,
    Vec3_t & reproj) const override final;

  nlohmann::json to_json() const override final;

  void set_cv_cam_matrix(
    [[maybe_unused]] float fx, [[maybe_unused]] float fy, [[maybe_unused]] float cx,
    [[maybe_unused]] float cy) override
  {
    throw std::runtime_error("set_cv_cam_matrix is not supported for equirectangular camera model");
  }
};
