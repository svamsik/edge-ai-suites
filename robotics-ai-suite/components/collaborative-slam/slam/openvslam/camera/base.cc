  ...
  num_grid_rows_(num_grid_rows)
{
  ...
  use_imu_ =
    (setup_type_ == camera::setup_type_t::RGBD_Inertial ||
     setup_type_ == camera::setup_type_t::Monocular_Inertial ||
     setup_type_ == camera::setup_type_t::Stereo_Inertial);
}

base::~base() { spdlog::debug("DESTRUCT: camera::base"); }
...
setup_type_t base::load_setup_type(const YAML::Node & yaml_node)
{
  ...
  } else if (setup_type_str == "Stereo_Inertial") {
    return camera::setup_type_t::Stereo_Inertial;
  } else if (setup_type_str == "Monocular_Inertial") {
    return camera::setup_type_t::Monocular_Inertial;
  }
...
{
  const auto itr =
    std::find(setup_type_to_string.begin(), setup_type_to_string.end(), setup_type_str);
...
{
  const auto itr =
    std::find(model_type_to_string.begin(), model_type_to_string.end(), model_type_str);
...
{
  const auto itr =
    std::find(color_order_to_string.begin(), color_order_to_string.end(), color_order_str);
...
void base::show_common_parameters() const
{
  ...
  std::cout << "- Use IMU: " << (use_imu_ ? "YES" : "No") << std::endl;
}
