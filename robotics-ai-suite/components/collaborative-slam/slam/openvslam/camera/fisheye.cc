...
fisheye::fisheye(
  ...
  const double k3, const double k4, const double focal_x_baseline)
: base(
    name, setup_type, model_type_t::Fisheye, color_order, cols, rows, fps, focal_x_baseline,
    focal_x_baseline / fx),
  fx_(fx),
  fy_(fy),
  cx_(cx),
...
: fisheye(
    ...
    yaml_node["Camera.k3"].as<double>(), yaml_node["Camera.k4"].as<double>(),
    yaml_node["Camera.focal_x_baseline"].as<double>(0.0))
{
}

fisheye::~fisheye() { spdlog::debug("DESTRUCT: camera::fisheye"); }
  ...
  if (k1_ == 0 && k2_ == 0 && k3_ == 0 && k4_ == 0) {
    ...
    if (theta_d > M_PI_2) {
      ...
      const std::vector<cv::KeyPoint> corners{
        cv::KeyPoint(cx_, 0.0, 1.0),     // top: min_y
      ...
      return image_bounds{
        (undist_min_x < min_x_thr || undist_min_x > cx_) ? min_x_thr : undist_min_x,
...
void fisheye::undistort_keypoints(
  const std::vector<cv::KeyPoint> & dist_keypt, std::vector<cv::KeyPoint> & undist_keypt) const
{
  ...
  cv::fisheye::undistortPoints(
    mat, mat, cv_cam_matrix_, cv_dist_params_, cv::Mat(), cv_cam_matrix_);
  mat = mat.reshape(1);
...
void fisheye::convert_keypoints_to_bearings(
  const std::vector<cv::KeyPoint> & undist_keypt, eigen_alloc_vector<Vec3_t> & bearings) const
{
...
void fisheye::convert_bearings_to_keypoints(
  const eigen_alloc_vector<Vec3_t> & bearings, std::vector<cv::KeyPoint> & undist_keypt) const
{
...
bool fisheye::reproject_to_image(
...
bool fisheye::reproject_to_image_bound(
  const Mat33_t & rot_cw, const Vec3_t & trans_cw, const Vec3_t & pos_w, Vec2_t & reproj,
...
bool fisheye::reproject_to_bearing(
...
void fisheye::set_cv_cam_matrix(float fx, float fy, float cx, float cy)
{
