...
public:
  ...
  fisheye(
    ...
    const double k3, const double k4, const double focal_x_baseline = 0.0);
  ...
  void undistort_keypoints(
    const std::vector<cv::KeyPoint> & dist_keypt,
    std::vector<cv::KeyPoint> & undist_keypt) const override final;
  ...
  void convert_keypoints_to_bearings(
    const std::vector<cv::KeyPoint> & undist_keypt,
  ...
  void set_cv_cam_matrix(float fx, float fy, float cx, float cy) override;
