...
equirectangular::equirectangular(
  const std::string & name, const color_order_t & color_order, const unsigned int cols,
  const unsigned int rows, const double fps)
: base(
    name, setup_type_t::Monocular, model_type_t::Equirectangular, color_order, cols, rows, fps, 0.0,
    0.0)
{
...
: equirectangular(
    yaml_node["Camera.name"].as<std::string>(), load_color_order(yaml_node),
    yaml_node["Camera.cols"].as<unsigned int>(), yaml_node["Camera.rows"].as<unsigned int>(),
    yaml_node["Camera.fps"].as<double>())
{
}

equirectangular::~equirectangular() { spdlog::debug("DESTRUCT: camera::equirectangular"); }

void equirectangular::show_parameters() const { show_common_parameters(); }
...
void equirectangular::convert_bearings_to_keypoints(
...
bool equirectangular::reproject_to_image(
...
bool equirectangular::reproject_to_image_bound(
  const Mat33_t & rot_cw, const Vec3_t & trans_cw, const Vec3_t & pos_w, Vec2_t & reproj,
  float & x_right, [[maybe_unused]] float x_bound_expand,
  [[maybe_unused]] float y_bound_expand) const
{
  // convert to camera-coordinates
  const Vec3_t bearing = (rot_cw * pos_w + trans_cw).normalized();
...
bool equirectangular::reproject_to_bearing(
