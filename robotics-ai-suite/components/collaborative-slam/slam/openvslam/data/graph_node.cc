...
void graph_node::update_connections()
{
  ...
  for (const auto lm : landmarks) {
    ...
    for (const auto & obs : observations) {
      ...
      // Modified for multi-slam
      if (is_server_node) {
        if (keyfrm->id_ < owner_keyfrm_->id_ || keyfrm->client_id_ != owner_keyfrm_->client_id_) {
...

  {
    ...
    if (spanning_parent_is_not_set_ && !owner_keyfrm_->is_origin()) {  // change to
...
void graph_node::update_covisibility_orders()
{
  ...
  for (const auto & keyfrm_and_weight : connected_keyfrms_and_weights_) {
    weight_keyfrm_pairs.emplace_back(
      std::make_pair(keyfrm_and_weight.second, keyfrm_and_weight.first));
  }
...
std::vector<keyframe *> graph_node::get_top_n_covisibilities(
  const unsigned int num_covisibilities) const
{
  std::lock_guard<std::mutex> lock(mtx_);
  if (ordered_covisibilities_.size() < num_covisibilities) {
    ...
    return std::vector<keyframe *>(
      ordered_covisibilities_.begin(), ordered_covisibilities_.begin() + num_covisibilities);
  }
}
...
std::vector<keyframe *> graph_node::get_covisibilities_over_weight(const unsigned int weight) const
{
  ...
  auto itr = std::upper_bound(
    ordered_weights_.begin(), ordered_weights_.end(), weight, std::greater<unsigned int>());
  if (itr == ordered_weights_.end()) {
    return std::vector<keyframe *>();
  } else {
    const auto num = static_cast<unsigned int>(itr - ordered_weights_.begin());
    return std::vector<keyframe *>(
      ordered_covisibilities_.begin(), ordered_covisibilities_.begin() + num);
  }
}
...
{
  std::lock_guard<std::mutex> lock(mtx_);
  assert(!spanning_parent_ || !keyfrm);
...
void graph_node::recover_spanning_connections()
{
  ...
  if (!spanning_parent_) {
    spdlog::info("keyframe {} doesn't has spanning parent!", owner_keyfrm_->id_);
    for (auto & spanning_child : spanning_children_) {
      if (!spanning_child) {
        continue;
      }
      ...
      spanning_child->graph_node_->set_spanning_parent(nullptr);
      spdlog::debug(
        "set spanning parent of its spanning child {} to nullptr!", spanning_child->id_);
    }
  ...
  while (!spanning_children_.empty()) {
    ...
    for (auto & spanning_child : spanning_children_) {
      ...
      // find the new parent (which has the maximum weight with the spanning child)
    ...
    if (max_is_found) {
      ...
      spdlog::debug(
        "Case 1: keyframe {} changes spanning parent from {} to {}!", max_weight_child->id_,
        owner_keyfrm_->id_, max_weight_parent->id_);
  ...
  if (!spanning_children_.empty()) {
    ...
    for (auto & spanning_child : spanning_children_) {
      spdlog::debug(
        "Case 2: keyframe {} changes spanning parent from {} to {}!", spanning_child->id_,
        owner_keyfrm_->id_, spanning_parent_->id_);
...
void graph_node::erase_loop_edge(keyframe * keyfrm)
{
  std::unique_lock<std::mutex> lock(mtx_);
  if (keyfrm->graph_node_->has_loop_edge()) {
    spdlog::warn(
      "must first clear the loop_edges_ of {} before erasing it from loop_edges of {}", keyfrm->id_,
      owner_keyfrm_->id_);
    return;
  }

  loop_edges_.erase(keyfrm);
  lock.unlock();
  owner_keyfrm_->set_to_be_erased();
}

void graph_node::erase_all_loop_edges()
{
  std::unique_lock<std::mutex> lock(mtx_);
  std::set<keyframe *> loop_edges_copy(std::move(loop_edges_));
  loop_edges_.clear();
  lock.unlock();

  for (const auto & loop_keyfrm : loop_edges_copy) {
    // In our current logic, loop_edges won't include the owner_keyfrm_,
    // but just add this check to double confirm
    if (owner_keyfrm_ != loop_keyfrm) {
      spdlog::debug(
        "keyframe {}: erase itself from the loop_edges of keyframe {}", owner_keyfrm_->id_,
        loop_keyfrm->id_);
      loop_keyfrm->graph_node_->erase_loop_edge(owner_keyfrm_);
    }
  }
  owner_keyfrm_->set_to_be_erased();
}
...
void graph_node::add_constraint_edge(int constraint_id, keyframe * keyfrm)
{
  // TODO
  std::lock_guard<std::mutex> lock(mtx_front_rear_camera_constraint_);
  if (
    front_rear_camera_constraint_.first < 0 || !front_rear_camera_constraint_.second ||
    front_rear_camera_constraint_.second->will_be_erased()) {
    front_rear_camera_constraint_ = std::make_pair(constraint_id, keyfrm);
  }
}

std::pair<int, data::keyframe *> graph_node::get_front_rear_camera_constraint()
{
  std::lock_guard<std::mutex> lock(mtx_front_rear_camera_constraint_);

  return front_rear_camera_constraint_;
}

bool graph_node::has_front_rear_camera_constraint()
{
  std::lock_guard<std::mutex> lock(mtx_front_rear_camera_constraint_);

  return front_rear_camera_constraint_.first >= 0 && front_rear_camera_constraint_.second &&
         !front_rear_camera_constraint_.second->will_be_erased();
}
