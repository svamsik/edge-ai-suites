...
std::vector<keyframe *> bow_database::acquire_loop_candidates(
  keyframe * qry_keyfrm, const float min_score)
{
  ...
  // Step 1.
  // Count up the number of nodes, words which are shared with query_keyframe, for all the keyframes
  // in DoW database
...
template <typename T>
bool bow_database::set_candidates_sharing_words(
  const T * const qry_shot, const std::set<keyframe *> & keyfrms_to_reject)
{
  init_candidates_.clear();
  num_common_words_.clear();
...
template <typename T>
bool bow_database::compute_scores(
  const T * const qry_shot, const unsigned int min_num_common_words_thr)
{
  scores_.clear();
...
bool bow_database::align_scores_and_keyframes(
  const unsigned int min_num_common_words_thr, const float min_score)
{
  score_keyfrm_pairs_.clear();
...
float bow_database::align_total_scores_and_keyframes(
  const unsigned int min_num_common_words_thr, const float min_score)
{
  total_score_keyfrm_pairs_.clear();
  ...
  for (const auto & score_keyframe : score_keyfrm_pairs_) {
    ...
    for (const auto & covisibility : top_n_covisibilities) {
      // Loop for which is included in the initial loop candidates and satisfies the minimum
      // shared word number
      if (
        static_cast<bool>(init_candidates_.count(covisibility)) &&
