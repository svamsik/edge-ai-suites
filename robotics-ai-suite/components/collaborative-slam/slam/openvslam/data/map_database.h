...
public:
  ...
  std::vector<landmark *> get_landmarks_in_frustum(
    ...
    std::vector<cv::Point2f> & convex_hull, double near = 0.2, double far = 10.0, double back = 0);

  std::vector<landmark *> get_landmarks_in_frustum(
    keyframe * camera_frame, std::vector<cv::Point3f> & point_3d,
    std::vector<cv::Point2f> & point_2d, std::vector<cv::Point2f> & convex_hull, double near = 0.2,
    double far = 10.0, double back = 0);

  std::vector<landmark *> get_all_landmarks(MapID map_id1, MapID map_id2) const;

  void get_all_landmarks_map(
...
private:
   ...
   * @param json_keyfrm
   */
  void register_keyframe(
    camera_database * cam_db, bow_vocabulary * bow_vocab, bow_database * bow_db,
    const KeyframeID id, const nlohmann::json & json_keyfrm);
  void register_keyframe_tracker(
    camera_database * cam_db, bow_vocabulary * bow_vocab, bow_database * bow_db,
    const KeyframeID id, const nlohmann::json & json_keyfrm);

  void register_keyframe_server(
    camera_database * cam_db, bow_vocabulary * bow_vocab, bow_database * bow_db,
    const KeyframeID id, const nlohmann::json & json_keyfrm);
