...
//! IMU states, just setted for visualizing the imu states.It can be deleted if IMU has been
...
class IMU_Preintegration
{
  void pre_integrate_imu_measurement(
    const Eigen::VectorXd & imu_measurement, const double delta_time);
...
public:
  ...
  static Eigen::Matrix4d predict_velocity(
    const double start_time, const double end_time,
    const std::vector<std::pair<double, Eigen::VectorXd>> & imu_measurements,
