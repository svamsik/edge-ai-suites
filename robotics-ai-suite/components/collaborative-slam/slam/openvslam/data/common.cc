...
nlohmann::json convert_keypoints_to_json(const std::vector<cv::KeyPoint> & keypts)
{
  ...
  for (unsigned int idx = 0; idx < keypts.size(); ++idx) {
    json_keypts.at(idx) = {
      {"pt", {keypts.at(idx).pt.x, keypts.at(idx).pt.y}},
...
std::vector<cv::KeyPoint> convert_json_to_keypoints(const nlohmann::json & json_keypts)
{
  ...
  for (unsigned int idx = 0; idx < json_keypts.size(); ++idx) {
    ...
    keypts.at(idx) = cv::KeyPoint(
      json_keypt.at("pt").at(0).get<float>(), json_keypt.at("pt").at(1).get<float>(), 0,
      json_keypt.at("ang").get<float>(), 0, json_keypt.at("oct").get<unsigned int>(), -1);
  }
...
std::vector<cv::KeyPoint> convert_json_to_undistorted(
  const nlohmann::json & json_undist_keypts, const std::vector<cv::KeyPoint> & keypts)
{
  auto undist_keypts =
    (keypts.empty() ? std::vector<cv::KeyPoint>(json_undist_keypts.size()) : keypts);
  ...
  std::vector<std::vector<std::vector<unsigned int>>> & keypt_indices_in_cells)
{
  ...
  const unsigned int num_to_reserve =
    0.5 * num_keypts / (camera->num_grid_cols_ * camera->num_grid_rows_);
  ...
  const int max_level)
{
  ...
  const int min_cell_idx_x =
    std::max(0, cvFloor((ref_x - camera->img_bounds_.min_x_ - margin) * camera->inv_cell_width_));
  ...
  const int min_cell_idx_y =
    std::max(0, cvFloor((ref_y - camera->img_bounds_.min_y_ - margin) * camera->inv_cell_height_));
  if (static_cast<int>(camera->num_grid_rows_) <= min_cell_idx_y) {
    return indices;
  }

  const int max_cell_idx_y = std::min(
    static_cast<int>(camera->num_grid_rows_ - 1),
    cvCeil((ref_y - camera->img_bounds_.min_y_ + margin) * camera->inv_cell_height_));
  if (max_cell_idx_y < 0) {
    return indices;
  }
