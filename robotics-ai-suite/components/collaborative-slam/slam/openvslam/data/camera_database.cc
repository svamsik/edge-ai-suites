...
camera::base * camera_database::get_client_camera(const ClientID client_id) const
{
  ...
  try {
    ...
    std::string clients;
    for (const auto & it : clients_database_) {
      clients = clients + ", " + std::to_string(it.first);
    }
    spdlog::critical(
      "Requesting non-exist camera for client {}. Existing cameras: ", client_id, clients);
    throw;
  }
}
...
void camera_database::from_json(const nlohmann::json & json_cameras)
{
  ...
  for (const auto & json_id_camera : json_cameras.items()) {
    ...
    const std::string & camera_name =
      is_server_node ? json_camera.at("name").get<std::string>() : json_id_camera.key();
    if (is_server_node) {
    ...
    const auto setup_type =
      camera::base::load_setup_type(json_camera.at("setup_type").get<std::string>());
    const auto model_type =
      camera::base::load_model_type(json_camera.at("model_type").get<std::string>());
    const auto color_order =
      camera::base::load_color_order(json_camera.at("color_order").get<std::string>());
