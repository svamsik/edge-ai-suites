...
public:
  ...
  /**
   * Erase the loop edge (Only used within the erase_all_loop_edges function!)
   */
  void erase_loop_edge(keyframe * keyfrm);

  /**
   * Erase all the loop edges (Only for removing keyframes within remapping region!)
   */
  void erase_all_loop_edges();
  ...
  void add_constraint_edge(int constraint_id, keyframe * keyfrm);

  std::pair<int, data::keyframe *> get_front_rear_camera_constraint();

  bool has_front_rear_camera_constraint();
...
private:
  ...
  // For multi-slam, its keyframe start id may not be 0
  // const unsigned int start_keyframe_id_;

  std::pair<int, data::keyframe *> front_rear_camera_constraint_ = std::make_pair(-1, nullptr);

  mutable std::mutex mtx_front_rear_camera_constraint_;
};
