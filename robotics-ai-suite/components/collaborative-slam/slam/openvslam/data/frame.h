...
public:
   ...
   */
  void extract_rgbd_features(
    const cv::Mat & img_gray, const cv::Mat & img_depth, const cv::Mat & mask = cv::Mat{});

  /**
   * Extract features from given input
   */
  void extract_stereo_features(
    const cv::Mat & img_left, const cv::Mat & img_right, const cv::Mat & mask = cv::Mat{});
  ...
  std::vector<unsigned int> get_keypoints_in_cell(
    const float ref_x, const float ref_y, const float margin, const int min_level = -1,
  ...
  std::vector<std::tuple<cv::Point2d, cv::Point2d, double>>
    matched_feature_points_;  // <prev, curr>
...
private:
  ...
  void extract_orb(
    const cv::Mat & img, const cv::Mat & mask, const image_side & img_side = image_side::Left);
#endif
  ...
  std::vector<std::pair<std::pair<cv::Point2d, int>, std::pair<cv::Point2d, int>>>
    matched_lidar_lm_;
};
