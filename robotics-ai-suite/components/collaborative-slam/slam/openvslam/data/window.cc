...
window::window(unsigned int window_id) : window_id_(window_id) {}

window::~window() { spdlog::debug("DESTRUCT: window"); }
...
void window::get_opt_keframes(
  std::vector<keyframe *> & origin_keyframes,
  std::unordered_map<KeyframeID, keyframe *> & keyfrm_ids, const size_t edge_size)
{
  std::lock_guard<std::mutex> lock(mtx_kf_);
...
void window::update_cam_pose_slerp(
  const std::vector<::g2o::Sim3, Eigen::aligned_allocator<::g2o::Sim3>> & cam_pose_cw_new,
  std::unordered_map<unsigned int, ::g2o::Sim3> & corrected_Sim3s_wc, size_t edge_size,
  unsigned int new_map_id, unsigned int loop_identifier)
{
  std::lock_guard<std::mutex> lock(mtx_kf_);
  ...
  Quat_t quat_res_start =
    calc_average_quart(std::accumulate(quats_start.begin(), quats_start.end(), init));
  Quat_t quat_res_end =
    calc_average_quart(std::accumulate(quats_end.begin(), quats_end.end(), init));
  Vec3_t tran_res_start =
    std::accumulate(trans_start.begin(), trans_start.end(), Vec3_t(0, 0, 0)) / edge_size;
  Vec3_t tran_res_end =
    std::accumulate(trans_end.begin(), trans_end.end(), Vec3_t(0, 0, 0)) / edge_size;
  scale_start /= edge_size;
  scale_end /= edge_size;
  const ::g2o::Sim3 opt_sim3_start(
    quat_res_start.normalized().toRotationMatrix(), tran_res_start, scale_start);
  const ::g2o::Sim3 opt_sim3_end(
    quat_res_end.normalized().toRotationMatrix(), tran_res_end, scale_end);
  ...
  for (size_t idx = 0; idx < edge_size; idx++) {
    ...
    Mat44_t cam_pose_cw = util::converter::to_eigen_cam_pose(
      pose_sim3.rotation().toRotationMatrix(), pose_sim3.translation() / pose_sim3.scale());
    ...
    cam_pose_cw = util::converter::to_eigen_cam_pose(
      pose_sim3.rotation().toRotationMatrix(), pose_sim3.translation() / pose_sim3.scale());
  ...
  for (auto iter = iter_start; iter != iter_end; iter++) {
    ...
    auto diff = end_distance - start_distance;
    if (
      FP_ZERO == std::fpclassify(diff) || FP_NAN == std::fpclassify(diff) ||
      FP_INFINITE == std::fpclassify(diff)) {
      spdlog::warn("window update_cam_pose_slerp: division by zero");
      continue;
    }
    double weight_end = ((double)keyfrm->window_distance_ - start_distance) / diff;
  ...
  while (iter_curr != keyframes_.end()) {
    const Mat44_t velocity_now =
      iter_curr->second->get_cam_pose() * iter_last->second->get_cam_pose_inv();
    Vec7_t distance_vec =
