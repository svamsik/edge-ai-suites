...
landmark::landmark(
  const Vec3_t & pos_w, keyframe * ref_keyfrm, map_database * map_db, LandmarkID id,
...
landmark::landmark(
  const LandmarkID id, const KeyframeID first_keyfrm_id, const Vec3_t & pos_w,
  keyframe * ref_keyfrm, const unsigned int num_visible, const unsigned int num_found,
  map_database * map_db)
: client_id_(0xFF),
  replace_other_id_(0),
  id_(id),
...
landmark::landmark(
  const LandmarkID id, const ClientID client_id, const Vec3_t & pos_w, const cv::Mat & descriptor,
...
landmark::landmark(
  const LandmarkID id, const KeyframeID first_keyfrm_id, const Vec3_t & pos_w,
  keyframe * ref_keyfrm, const unsigned int num_visible, const unsigned int num_found,
  map_database * map_db, ClientID client_id, MapID map_id)
: client_id_(client_id),
  replace_other_id_(0),
  id_(id),
  first_keyfrm_id_(first_keyfrm_id),
...
void landmark::set_update_from_server(bool update_from_server)
{
...
void landmark::add_observation(keyframe * keyfrm, unsigned int idx, bool is_relocalization)
{
  ...
  if (!is_server_node) {
    if ((!ref_keyfrm_ || observations_.empty()) && come_from_server_) {
      ref_keyfrm_ = keyfrm;
      // spdlog::debug(
      //   "Set current keyframe {} as this server landmark {} reference keyframe",
      //   keyfrm->id_, id_);
    }
  }
...
void landmark::compute_descriptor()
{
  ...
  std::vector<std::vector<unsigned int>> hamm_dists(
    num_descs, std::vector<unsigned int>(num_descs));
  ...
  for (unsigned idx = 0; idx < num_descs; ++idx) {
    std::vector<unsigned int> partial_hamm_dists(
      hamm_dists.at(idx).begin(), hamm_dists.at(idx).begin() + num_descs);
    std::sort(partial_hamm_dists.begin(), partial_hamm_dists.end());
    const auto median_dist =
...
void landmark::update_normal_and_depth()
{
  if (is_server_node) {
    ...
    if (!observations_.count(ref_keyfrm_)) {
      // This will rarely happen when landing map from local disk!
...
unsigned int landmark::predict_scale_level(const float cam_to_lm_dist, const frame * frm) const
{
  ...
  const auto pred_scale_level =
    static_cast<int>(std::ceil(std::log(ratio) / frm->log_scale_factor_));
...
unsigned int landmark::predict_scale_level(
  const float cam_to_lm_dist, const keyframe * keyfrm) const
{
  float ratio;
  {
    std::lock_guard<std::mutex> lock(mtx_position_);
  ...
  const auto pred_scale_level =
    static_cast<int>(std::ceil(std::log(ratio) / keyfrm->log_scale_factor_));
  if (pred_scale_level < 0) {
    return 0;
...
void landmark::prepare_for_erasing(bool is_redundant, bool is_server)
{
  ...
  if (!is_server_node) {
    if (!is_server) {
      map_db_->erase_landmark(this, is_redundant);
    }
  } else {
...
void landmark::replace(landmark * lm)
{
  ...
  if (!is_server_node) {
    ...
    if (lm->come_from_server() && !lm->get_ref_keyframe()) {
      lm->set_ref_keyframe(ref_keyfrm_);
    }
  }
  for (const auto & keyfrm_and_idx : observations) {
  ...
  if (!is_server_node) {
    if (!lm->come_from_server() || lm->has_observation()) {
...
nlohmann::json landmark::to_json() const
{
  if (is_server_node) {
    return {
      {"1st_keyfrm", std::to_string(first_keyfrm_id_)},
