  ...
  is_origin_(false)
{
  ...
  imu_velocity_
    .setZero();  // Or get it from frame? Maybe calculate from camera pose change is enough!
  imu_bias_ = IMU_Preintegration::last_bias_;
}
#else   // #ifdef ENABLE_SEMANTIC_SLAM
...
#endif  // #ifdef ENABLE_SEMANTIC_SLAM

keyframe::keyframe(
  const frame & frm, map_database * map_db, bow_database * bow_db, ClientID client_id, MapID map_id)
:  // meta information
  ref_keyfrm_(frm.ref_keyfrm_),
...
keyframe::keyframe(
  const KeyframeID id, const KeyframeID src_frm_id, const double timestamp,
...
{
  // TODO :- Add the network model file path to config file
  ...
  // if the model performs ArgMax, its output type can be I32 but for models that return
  // heatmaps for each class the output is usually FP32. Reset the precision to avoid handling
  // different types with switch in postprocessing
  ...
  switch (outSizeVector.size()) {
    ...
    default:
      ...
      //   "Unexpected output blob shape. Only 4D and 3D output blobs are supported.");
  }
...
void keyframe::segment_video_frame(const cv::Mat & img)
{
  ...
  InferenceEngine::TensorDesc tDesc(
    InferenceEngine::Precision::U8, {1, img_rgb.channels(), img_rgb.rows, img_rgb.cols},
  ...
  InferenceEngine::LockedMemory<const void> outMapped =
    InferenceEngine::as<InferenceEngine::MemoryBlob>(inferRequest.GetBlob(keyframe::outName))
      ->rmap();
...
bool keyframe::can_observe(const landmark * lm) const
{
  ...
  return camera_->reproject_to_image(
    cam_pose_cw_.block(0, 0, 3, 3), cam_pose_cw_.block(0, 3, 3, 1), pos_w, reproj, right_x);
}
...
nlohmann::json keyframe::to_json_server() const
{
  ...
  return {
    {"src_frm_id", std::to_string(src_frm_id_)},
    {"ts", timestamp_},
    {"cam", camera_->name_},
    {"depth_thr", depth_thr_},
    // camera pose
...
void keyframe::set_update_from_server(bool update_from_server)
{
  update_from_server_ = update_from_server;
}
...
void keyframe::set_imu_constraint(
  data::keyframe * ref_keyframe, std::shared_ptr<data::IMU_Preintegration> imu_constraint)
{
  imu_constraint_ = std::make_pair(ref_keyframe, imu_constraint);
}

std::pair<data::keyframe *, std::shared_ptr<IMU_Preintegration>> keyframe::get_imu_constraint()
  const
{
...
{
  if (next_keyframe_) {
    next_keyframe_->pre_keyframe_ = nullptr;
  }

  if (next_keyframe_ && pre_keyframe_) {
    spdlog::debug(
      "Merge IMU data between keyframe {} and keyframe {}", next_keyframe_->id_,
...
unsigned int keyframe::get_num_tracked_landmarks(const unsigned int min_num_obs_thr) const
{
  ...
  if (0 < min_num_obs_thr) {
    for (const auto lm : landmarks_) {
      ...
      // spdlog::debug(
      //   "min_num_obs_thr: {}, lm observation num: {}", min_num_obs_thr, lm->num_observations());
...
landmark * keyframe::get_landmark(const unsigned int idx) const
{
  std::lock_guard<std::mutex> lock(mtx_observations_);
  assert(
    ("no such landmark" + std::to_string(idx) + "in this keyframe " + std::to_string(id_),
     idx < landmarks_.size()));

  return landmarks_.at(idx);
}

std::vector<unsigned int> keyframe::get_keypoints_in_cell(
  const float ref_x, const float ref_y, const float margin) const
{
  return data::get_keypoints_in_cell(
    camera_, undist_keypts_, keypt_indices_in_cells_, ref_x, ref_y, margin);
}
...
void keyframe::prepare_for_erasing(bool is_redundant, bool is_server)
{
  ...
  if (!will_be_erased_) {
    // If will_be_erased_, it means this keyframe has been removed from local map and the imu link
  ...
  if (
    !map_db_->get_server_virtual_keyframe() || id_ != map_db_->get_server_virtual_keyframe()->id_) {
...
void keyframe::prepare_for_erasing_remapping()
{
  ...
  if (graph_node_->has_loop_edge()) {
    graph_node_->erase_all_loop_edges();
  }
  // recover spanning tree
  if (!skip_recover_spanning_connections) {
    graph_node_->recover_spanning_connections();
  }
...
void keyframe::replace_origin_with_other_candidate()
{
  ...
  if (all_keyframes_in_map.empty()) {
    spdlog::warn(
      "Map database is empty except the origin keyframe, so just erase it from origin_keyfrms_ \
  ...
  if (candidate == all_keyframes_in_map.end()) {
    spdlog::warn(
      "Failed to find a replacement for origin keyframe {} for map {} based on the keyframe ID!");
    spdlog::warn(
      "Just try to choose the keyframe with the smallest keyframe ID {} in map database");
    candidate = all_keyframes_in_map.begin();
  }
  spdlog::info(
    "Replace current origin keyframe {} for map {} with keyframe {}", id_, map_id_,
    (*candidate).first);
  ...
  for (auto & spanning_child : spanning_children) {
    if (!spanning_child || spanning_child->will_be_erased()) {
      continue;
    }
    if (spanning_child == (*candidate).second) {
      continue;
    }
    spanning_child->graph_node_->change_spanning_parent((*candidate).second);
    spdlog::debug(
      "Change the spanning parent of spanning child {} from current origin keyframe to {}",
