...
Eigen::Matrix3d jacobian_right_inverse(const Eigen::Vector3d v)
{
  ...
  if (norm > EPSILON) {
    Eigen::Matrix3d v_hat = skew(v);
    J_r_inverse +=
      0.5 * v_hat + (1.0 / (norm * norm) - 0.5 * (1.0 + std::cos(norm)) / (norm * std::sin(norm))) *
                      (v_hat * v_hat);
  }
...
Eigen::MatrixXd IMU_Preintegration::noise_covaraince_ =
  Eigen::MatrixXd::Identity(6, 6) * DEFAULT_NOISE_COV;

Eigen::MatrixXd IMU_Preintegration::Bias_walk_ =
  Eigen::MatrixXd::Identity(6, 6) * DEFAULT_BIAS_WALK;

Eigen::Matrix4d IMU_Preintegration::Tci_ = Eigen::Matrix4d::Identity();

IMU_Preintegration::IMU_State IMU_Preintegration::state_ =
  IMU_Preintegration::IMU_State::Not_Initialized;
...
void IMU_Preintegration::set_imu_status(
  const double & duration, const Eigen::Matrix3d & preintegration_R,
...
void IMU_Preintegration::initialize(std::shared_ptr<univloc_tracker::Config> cfg)
{
  ...
  // Refer: OpenVINS: Discrete-time IMU Propagation
  // <https://docs.openvins.com/propagation.html#disc_prop>
  for (int i = 0; i < 6; i++) {
...
: start_time_(start_time), end_time_(end_time), integration_time_(end_time - start_time)
{
  ...
  spdlog::debug(
    "IMU_Preintegration time interval {} ms, imu mearsurements num: {}, start_time: {}, end_time: "
    "{}, first imu "
    "time: {}, last imu time: {}",
  ...
  spdlog::debug(
    "vec_imu_mearsurements_.size:{}, imu_measurements.size: {}, "
    "imu_mearsurements_withtimestamp.size: {}",
...
void IMU_Preintegration::merge_preintegration(
  std::shared_ptr<IMU_Preintegration> merged_preintegration)
{
  ...
  spdlog::debug(
    "t1 {} t2 {} t3 {} t4 {}", older_vec_imu_mearsurements.front().first,
  ...
  vec_imu_mearsurements_.assign(
    older_vec_imu_mearsurements.begin(), older_vec_imu_mearsurements.end());
  vec_imu_mearsurements_.insert(
    vec_imu_mearsurements_.end(), newer_vec_imu_mearsurements.begin(),
...
void IMU_Preintegration::pre_integrate_imu_measurement()
{
  ...
  for (unsigned int i = 0; i < num; i++) {
    if (i == 0) {
      ...
      m_imu =
        (vec_imu_mearsurements_[i].second + vec_imu_mearsurements_[i - 1].second) * 0.5 - bias_;
      // m_imu = vec_imu_mearsurements_[i].second - bias_;
      if (delta_t < 0) {
        spdlog::debug(
          "Last imu timestamp {}, this timestamp: {}, time interval {}",
...
{
  // To verify whether the IMU_Preintegration output is correct! the input and result
  // can be aquired from ORB_SLAM3
...
void IMU_Preintegration::pre_integrate_imu_measurement(
  const Eigen::VectorXd & imu_measurement, const double delta_t)
{
  ...
  delta_R = skew(m_gyro * delta_t)
              .exp();  // TODO: check if correct, similar to IntegratedRotation in ORB_SLAM3
  preintegration_p_ +=
    0.5 * preintegration_R_ * m_accel * delta_t * delta_t + preintegration_v_ * delta_t;
  ...
  A.block(0, 0, 3, 3) =
    delta_R.transpose();  // update at the end in ORB_SLAM3, the basic logic is the same
  ...
  J_p_bg_ +=
    -0.5 * preintegration_R_ * skew(m_accel) * J_R_bg_ * delta_t * delta_t + J_v_bg_ * delta_t;
  ...
  preintegration_R_ = normalize_rotation(
    preintegration_R_ * delta_R);  // update is the same, ORB_SLAM3 use doublecheck(NORMALIZE)
}
...
Eigen::VectorXd IMU_Preintegration::get_delta_bias(const Eigen::VectorXd & bias)
{
  ...
  db << bias[0] - bias_[0], bias[1] - bias_[1], bias[2] - bias_[2], bias[3] - bias_[3],
    bias[4] - bias_[4], bias[5] - bias_[5];
...
void IMU_data::get_imu_data(
  double start_time, double end_time, std::vector<std::pair<double, Eigen::VectorXd>> & imu_vec,
  bool erase_data)
{
  assert(start_time < end_time);
