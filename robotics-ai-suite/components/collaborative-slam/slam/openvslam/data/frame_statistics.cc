...
void frame_statistics::update_frame_statistics(const data::frame & frm, const bool is_lost)
{
  if (frm.cam_pose_cw_is_valid_) {
    const Mat44_t rel_cam_pose_from_ref_keyfrm =
      frm.cam_pose_cw_ * frm.ref_keyfrm_->get_cam_pose_inv();
...
void frame_statistics::replace_reference_keyframe(
  data::keyframe * old_keyfrm, data::keyframe * new_keyfrm)
{
  ...
  for (const auto frm_id : frm_ids) {
    ...
    const Mat44_t new_rel_cam_pose_cr =
      old_rel_cam_pose_cr * old_ref_cam_pose_cw * new_ref_cam_pose_cw.inverse();
...
std::unordered_map<data::keyframe *, std::vector<KeyframeID>>
frame_statistics::get_frame_id_of_reference_keyframes() const
{
  return frm_ids_of_ref_keyfrms_;
}

unsigned int frame_statistics::get_num_valid_frames() const { return num_valid_frms_; }

std::map<KeyframeID, data::keyframe *> frame_statistics::get_reference_keyframes() const
{
  return {ref_keyfrms_.begin(), ref_keyfrms_.end()};
}

eigen_alloc_map<KeyframeID, Mat44_t> frame_statistics::get_relative_cam_poses() const
{
  return {rel_cam_poses_from_ref_keyfrms_.begin(), rel_cam_poses_from_ref_keyfrms_.end()};
}

std::map<KeyframeID, double> frame_statistics::get_timestamps() const
{
  return {timestamps_.begin(), timestamps_.end()};
}

std::map<KeyframeID, bool> frame_statistics::get_lost_frames() const
{
