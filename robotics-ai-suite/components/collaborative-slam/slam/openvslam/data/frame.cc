...
#else
#include "feature/default/orb_extractor.h"
#endif
#include "data/imu.h"
...
frame::frame(
  const double timestamp, orb_extractor * extractor, bow_vocabulary * bow_vocab,
...
frame::frame(
  const double timestamp, orb_extractor * extractor_left, orb_extractor * extractor_right,
...
frame::frame(
  const double timestamp, orb_extractor * extractor, bow_vocabulary * bow_vocab,
  camera::base * camera, const float depth_thr, const cv::Mat & img_rgb)
: id_(next_id_++),
  bow_vocab_(bow_vocab),
  extractor_(extractor),
  extractor_right_(nullptr),
  ...
  depth_thr_(depth_thr)
{
  // save rgb image using deep copy in case that it is a keyframe for
  // segmentation
...
void frame::extract_rgbd_features(
  const cv::Mat & img_gray, const cv::Mat & img_depth, const cv::Mat & mask)
{
  image_ = img_gray;
  ...
  spdlog::debug(
    "frame {} extract {} valid features; {} keypoints; {} descriptors", id_, valid_features,
...
void frame::extract_stereo_features(
  const cv::Mat & img_left, const cv::Mat & img_right, const cv::Mat & mask)
{
  image_ = img_left;
  const int min_valid_features_thr = 100;
...
// Extract ORB feature
  ...
  std::thread thread_left(
    &frame::extract_orb, this, std::ref(img_left), std::ref(mask), image_side::Left);
  std::thread thread_right(
    &frame::extract_orb, this, std::ref(img_right), std::ref(mask), image_side::Right);
  thread_left.join();
  thread_right.join();
#endif
  ...
  // Undistort keypoints
  camera_->undistort_keypoints(
    keypts_,
    undist_keypts_);  // This camera is the same for left and right image
...
#endif
  match::stereo stereo_matcher(
    left_image_pyramid_, right_image_pyramid_, keypts_, keypts_right_, descriptors_,
    descriptors_right_, scale_factors_, inv_scale_factors_, camera_->focal_x_baseline_,
    camera_->true_baseline_);
  ...
  spdlog::debug(
    "frame {}: extract {} valid features; {} left keypoints; {} "
    "right keypoints; {} left "
    "descriptors; "
    "{} right descriptors",
    id_, valid_features, num_keypts_, keypts_right_.size(), descriptors_.rows,
...
void frame::extract_mono_features(const cv::Mat & img_gray, const cv::Mat & mask)
{
  ...
  spdlog::debug(
    "frame {} extract {} undistorted keypoints; {} keypoints; {} descriptors", id_,
    undist_keypts_.size(), num_keypts_, descriptors_.rows);
...
std::pair<Mat44_t, int> frame::get_prev_lidar_cam_pose() const
{
  return {prev_lidar_cam_pose_cw_, prev_lidar_id_};
}
...
{
  return cam_pose_cw_.block(0, 0, 3, 3).transpose() *
         data::IMU_Preintegration::Tci_.block(0, 0, 3, 3);
}
...
bool frame::can_observe(
  landmark * lm, const float ray_cos_thr, Vec2_t & reproj, float & x_right,
...
std::vector<unsigned int> frame::get_keypoints_in_cell(
  const float ref_x, const float ref_y, const float margin, const int min_level,
  const int max_level) const
{
  return data::get_keypoints_in_cell(
    camera_, undist_keypts_, keypt_indices_in_cells_, ref_x, ref_y, margin, min_level, max_level);
}
