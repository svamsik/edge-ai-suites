...
public:
  ...
  friend std::ostream & operator<<(std::ostream & os, const Config & cfg)
  {
    return os << "Camera: " << cfg.camera_name_ << "\n"
              << "  (" << cfg.setup_type_str_ << "; " << cfg.color_order_str_ << "; "
              << cfg.model_type_str_ << ")\n"
              ...
              << "  intrinsics: " << cfg.fx_ << ", " << cfg.fy_ << ", " << cfg.cx_ << ", "
              << cfg.cy_ << "\n"
  ...
  std::string
    setup_type_str_;  // options: "RGBD", "RGBD_Inertial", "Monocular", "Monocular_Inertial"
  ...
  {
    return setup_type_str_.rfind("Monocular", 0) != std::string::npos;
  }
  ...
  {
    return setup_type_str_.find(
             "Inertial", setup_type_str_.length() - std::string("Inertial").length()) !=
           std::string::npos;
  }
  ...
  void set_distortion_params(const T & params)
  {
    ...
    if (params.size() == expected_distortion_size) {
      ...
      throw std::invalid_argument(
        "Expecting " + std::to_string(expected_distortion_size) + " distortion params for " +
        model_type_str_ + " camera but got " + std::to_string(params.size()));
    }
  }
  ...
  std::vector<double>
    remapping_region_vertexes_;  // In the two-sessions remapping solution, the tracker
  ...
  int trigger_relocalization_request_divisor_ =
    1;                            // The threshold of frames to trigger relocalization request
  ...
  int correction_threshold_ =
    1;  // Correction threshold in voxels for triggering a map reconstruction
