...
void ImuCamPose::Update(const double * pu)
{
  ...
  tcw_ = data::IMU_Preintegration::Tci_.block(0, 0, 3, 3) * tbw +
         data::IMU_Preintegration::Tci_.block(0, 3, 3, 1);
}
...
VertexPose::VertexPose(Keyframe * pKF) { setEstimate(ImuCamPose(pKF)); }

VertexPose::VertexPose(Frame * pF) { setEstimate(ImuCamPose(pF)); }
...
VertexPose4DoF::VertexPose4DoF(Keyframe * pKF) { setEstimate(ImuCamPose(pKF)); }

VertexPose4DoF::VertexPose4DoF(Frame * pF) { setEstimate(ImuCamPose(pF)); }
...
GDirection::GDirection(Eigen::Matrix3d Rwg) : Rwg_(Rwg) {}
...
VertexGDir::VertexGDir(Eigen::Matrix3d Rwg) { setEstimate(GDirection(Rwg)); }
...
VertexScale::VertexScale() { setEstimate(1.0); }

VertexScale::VertexScale(double ps) { setEstimate(ps); }

void VertexScale::setToOriginImpl() { setEstimate(1.0); }

void VertexScale::oplusImpl(const double * update_) { setEstimate(estimate() * exp(*update_)); }
...
EdgeMonoOnlyPose::EdgeMonoOnlyPose(const Eigen::Vector3d & Xw) : Xw_(Xw) {}
...
EdgeStereoOnlyPose::EdgeStereoOnlyPose(const Eigen::Vector3d & Xw) : Xw_(Xw) {}
  ...
  g_(Eigen::Vector3d::Zero())
{
  ...
  Eigen::Matrix<double, 9, 9> Info =
    imu_preintegration_ptr_->covariance_.block(0, 0, 9, 9).inverse();
  Info = (Info + Info.transpose()) / 2;
  Eigen::SelfAdjointEigenSolver<Eigen::Matrix<double, 9, 9>> es(Info);
  Eigen::Matrix<double, 9, 1> eigs = es.eigenvalues();
  for (int i = 0; i < 9; i++) {
    if (eigs[i] < INERTIAL_EDGE_EPSILON) {
      eigs[i] = 0;
    }
  }
...
{
  // TODO Maybe Reintegrate inertial measurments when difference between linearization point
  // and current estimate is too big
  ...
  bias << VA->estimate()[0], VA->estimate()[1], VA->estimate()[2], VG->estimate()[0],
  ...
  const Eigen::Vector3d er =
    log_so3(dR.transpose() * VP1->estimate().Rwb_.transpose() * VP2->estimate().Rwb_);
  const Eigen::Vector3d ev = VP1->estimate().Rwb_.transpose() *
                               (s * (VV2->estimate() - VV1->estimate()) - g_ * integration_time_) -
                             dV;
...
void EdgeInertialGS::linearizeOplus()
{
  ...
  _jacobianOplus[2].block<3, 3>(0, 0) =
    -invJr * eR.transpose() * jacobian_right(J_R_bg_ * dbg) * J_R_bg_;
  ...
  _jacobianOplus[6].block<3, 2>(6, 0) =
    -0.5 * Rbw1 * dGdTheta * integration_time_ * integration_time_;
...
void EdgeInertial::computeError()
{
  ...
  const VertexVelocity * VV2 = static_cast<const VertexVelocity *>(_vertices[5]);
  Eigen::VectorXd bias = Eigen::VectorXd(6);
  bias << VA1->estimate()[0], VA1->estimate()[1], VA1->estimate()[2], VG1->estimate()[0],
  ...
  const Eigen::Vector3d er =
    log_so3(dR.transpose() * VP1->estimate().Rwb_.transpose() * VP2->estimate().Rwb_);
  const Eigen::Vector3d ev = VP1->estimate().Rwb_.transpose() *
                               (VV2->estimate() - VV1->estimate() - g_ * integration_time_) -
                             dV;
  const Eigen::Vector3d ep =
    VP1->estimate().Rwb_.transpose() *
...
void EdgeInertial::linearizeOplus()
{
  ...
  _jacobianOplus[2].setZero();
  _jacobianOplus[2].block<3, 3>(0, 0) =
    -invJr * eR.transpose() * jacobian_right(J_R_bg_ * dbg) * J_R_bg_;  // OK
...
EdgePriorAcc::EdgePriorAcc(const Eigen::Vector3d & bprior) : bprior_(bprior) {}
...
EdgePriorGyro::EdgePriorGyro(const Eigen::Vector3d & bprior) : bprior_(bprior) {}
...
void Edge4DoF::computeError()
{
  ...
  _error << log_so3(
    VPi->estimate().Rcw_ * VPj->estimate().Rcw_.transpose() * delta_Rij_.transpose()),
    VPi->estimate().Rcw_ * (-VPj->estimate().Rcw_.transpose() * VPj->estimate().tcw_) +
      VPi->estimate().tcw_ - delta_tij_;
}

ConstraintPoseImu::ConstraintPoseImu(
  const Eigen::Matrix3d & Rwb, const Eigen::Vector3d & twb, const Eigen::Vector3d & vwb,
  const Eigen::Vector3d & bg, const Eigen::Vector3d & ba, const Eigen::Matrix<double, 15, 15> & H)
: Rwb_(Rwb), twb_(twb), vwb_(vwb), bg_(bg), ba_(ba), H_(H)
{
  H_ = (H_ + H_) / 2;
...
: Rwb_(c->Rwb_), twb_(c->twb_), vwb_(c->vwb_), bg_(c->bg_), ba_(c->ba_)
{
  resize(4);
  setInformation(c->H_);
}
