...
#include "optimize/g2o/sim3/forward_reproj_edge.h"
#include "optimize/g2o/sim3/backward_reproj_edge.h"
...
public:
  ...
  mutual_reproj_edge_wapper(
    T * shot1, unsigned int idx1, data::landmark * lm1, T * shot2, unsigned int idx2,
    data::landmark * lm2, g2o::sim3::transform_vertex * Sim3_12_vtx, const float sqrt_chi_sq);

  inline bool is_inlier() const { return edge_12_->level() == 0 && edge_21_->level() == 0; }

  inline bool is_outlier() const { return !is_inlier(); }
...
template <typename T>
mutual_reproj_edge_wapper<T>::mutual_reproj_edge_wapper(
  T * shot1, unsigned int idx1, data::landmark * lm1, T * shot2, unsigned int idx2,
  data::landmark * lm2, g2o::sim3::transform_vertex * Sim3_12_vtx, const float sqrt_chi_sq)
