...
// Previously the edgese3expmap edge from the original g2o library is used to construct
// the odom (pose graph) edge. However, it seems that the error formulation is not correct
// and the optimization result depends on the origin of the coordinate. Therefore, we experience
// the drift of z axis in previous tests for the geekplus data (kongkuang). After correcting
// the error formulation bug by defining a new edge (mostly the same as edgese3expmap except
// for error formulation and jacobian matrix), we are now able to get the correct output.
