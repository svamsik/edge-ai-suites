...
void equirectangular_pose_opt_edge::linearizeOplus()
{
  ...
  VecR_t<6> d_pcx_d_x;
  d_pcx_d_x << d_pc_d_rx(0), d_pc_d_ry(0), d_pc_d_rz(0), d_pc_d_tx(0), d_pc_d_ty(0), d_pc_d_tz(0);
  VecR_t<6> d_pcy_d_x;
  d_pcy_d_x << d_pc_d_rx(1), d_pc_d_ry(1), d_pc_d_rz(1), d_pc_d_tx(1), d_pc_d_ty(1), d_pc_d_tz(1);
  VecR_t<6> d_pcz_d_x;
  d_pcz_d_x << d_pc_d_rx(2), d_pc_d_ry(2), d_pc_d_rz(2), d_pc_d_tx(2), d_pc_d_ty(2), d_pc_d_tz(2);

  const Vec6_t d_L_d_x = (1.0 / L) * (pcx * d_pcx_d_x + pcy * d_pcy_d_x + pcz * d_pcz_d_x);

  MatRC_t<2, 6> jacobian = MatRC_t<2, 6>::Zero();
  jacobian.block<1, 6>(0, 0) =
    -(cols_ / (2 * M_PI)) * (1.0 / (pcx * pcx + pcz * pcz)) * (pcz * d_pcx_d_x - pcx * d_pcz_d_x);
