...
#include "optimize/g2o/se3/perspective_pose_opt_edge.h"
#include "optimize/g2o/se3/equirectangular_pose_opt_edge.h"
...
public:
  ...
  pose_opt_edge_wrapper(
    T * shot, shot_vertex * shot_vtx, const Vec3_t & pos_w, const unsigned int idx,
    const float obs_x, const float obs_y, const float obs_x_right, const float inv_sigma_sq,
...
template <typename T>
pose_opt_edge_wrapper<T>::pose_opt_edge_wrapper(
  T * shot, shot_vertex * shot_vtx, const Vec3_t & pos_w, const unsigned int idx, const float obs_x,
  const float obs_y, const float obs_x_right, const float inv_sigma_sq, const float sqrt_chi_sq)
...
bool pose_opt_edge_wrapper<T>::depth_is_positive() const
{
  switch (camera_->model_type_) {
    case camera::model_type_t::Perspective: {
      if (is_monocular_) {
        return static_cast<mono_perspective_pose_opt_edge *>(edge_)
          ->mono_perspective_pose_opt_edge::depth_is_positive();
      } else {
        return static_cast<stereo_perspective_pose_opt_edge *>(edge_)
          ->stereo_perspective_pose_opt_edge::depth_is_positive();
      }
    }
    case camera::model_type_t::Fisheye: {
      if (is_monocular_) {
