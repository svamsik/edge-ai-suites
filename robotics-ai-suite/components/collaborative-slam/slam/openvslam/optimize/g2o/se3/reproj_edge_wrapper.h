...
#include "optimize/g2o/se3/perspective_reproj_edge.h"
#include "optimize/g2o/se3/equirectangular_reproj_edge.h"
...
public:
  ...
  reproj_edge_wrapper(
    T * shot, shot_vertex * shot_vtx, data::landmark * lm, landmark_vertex * lm_vtx,
...
template <typename T>
reproj_edge_wrapper<T>::reproj_edge_wrapper(
  T * shot, shot_vertex * shot_vtx, data::landmark * lm, landmark_vertex * lm_vtx,
  const unsigned int idx, const float obs_x, const float obs_y, const float obs_x_right,
...
bool reproj_edge_wrapper<T>::depth_is_positive() const
{
  switch (camera_->model_type_) {
    case camera::model_type_t::Perspective: {
      if (is_monocular_) {
        return static_cast<mono_perspective_reproj_edge *>(edge_)
          ->mono_perspective_reproj_edge::depth_is_positive();
      } else {
        return static_cast<stereo_perspective_reproj_edge *>(edge_)
          ->stereo_perspective_reproj_edge::depth_is_positive();
      }
    }
    case camera::model_type_t::Fisheye: {
      if (is_monocular_) {
