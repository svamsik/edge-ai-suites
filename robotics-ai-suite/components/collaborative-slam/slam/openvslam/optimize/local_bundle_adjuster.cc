...
local_bundle_adjuster::local_bundle_adjuster(
  data::map_database * map_db, const bool use_odom, const unsigned int num_first_iter,
...
void local_bundle_adjuster::optimize(
  data::keyframe * curr_keyfrm, bool * const force_stop_flag,
...
void local_bundle_adjuster::optimize_tracker_node(
  data::keyframe * curr_keyfrm, bool * const force_stop_flag, const bool is_large) const
{
...
void local_bundle_adjuster::optimize_server_node(
  data::keyframe * curr_keyfrm, bool * const force_stop_flag) const
{
  {
    // 1. local/fixed keyframes, local landmarks
    ...
    // 3. keyframe g2o vertex optimizer

    // shot vertex container
    g2o::se3::shot_vertex_container keyfrm_vtx_container(
      0, local_keyfrms.size() + fixed_keyfrms.size());
    // vertex keyframes
    std::unordered_map<KeyframeID, data::keyframe *> all_keyfrms;

    // local keyframes optimizer
    ...
    // 4. keyframe landmark vertex reprojection edge

    // landmark vertex container
    g2o::landmark_vertex_container lm_vtx_container(
      keyfrm_vtx_container.get_max_vertex_id() + 1, local_lms.size());

    // reprojection edge container
    ...
    for (auto & id_local_lm_pair : local_lms) {
      ...
      for (const auto & obs : observations) {
        ...
        auto reproj_edge_wrap = reproj_edge_wrapper(
          keyfrm, keyfrm_vtx, local_lm, lm_vtx, idx, undist_keypt.pt.x, undist_keypt.pt.y, x_right,
...
void local_bundle_adjuster::optimize_without_imu(
  openvslam::data::keyframe * curr_keyfrm, bool * const force_stop_flag) const
{
  // 1. local/fixed keyframes, local landmarks
  std::unordered_map<KeyframeID, data::keyframe *> local_keyfrms;
  ...
  for (auto & id_local_lm_pair : local_lms) {
    ...
    for (const auto & obs : observations) {
      ...
      // This will happen rarely, this means the landmark observation has been modified
      // at the same time! or new landmark has been added!
      ...
      const auto sqrt_chi_sq =
        (keyfrm->camera_->setup_type_ == camera::setup_type_t::Monocular ||
  ...
  if (use_odom_) {
    ...
    std::sort(local_kfs.begin(), local_kfs.end(), [](const auto & a, const auto & b) {
      return a->id_ < b->id_;
    });
    ...
    odom1 = kf1->odom_;
    // by avoid start tracking before getting valid odom data, the odom for original keyframe
    // should be valid
...
void local_bundle_adjuster::optimize_from_server(
  openvslam::data::keyframe * curr_keyfrm, bool * const force_stop_flag) const
{
  // 1. local/fixed keyframes, local landmarks
  ...
  for (auto & id_relative_keyfrm_pair : relative_keyfrms) {
    ...
    auto keyfrm_vtx =
      keyfrm_vtx_container.create_vertex(relative_keyfrm, relative_keyfrm->update_from_server_);
    optimizer.addVertex(keyfrm_vtx);
  }
  ...
  for (auto & id_local_lm_pair : local_lms) {
    ...
    for (const auto & obs : observations) {
      ...
      const float inv_sigma_sq = keyfrm->inv_level_sigma_sq_.at(undist_keypt.octave);
      const auto sqrt_chi_sq =
        (curr_keyfrm->camera_->setup_type_ == camera::setup_type_t::Monocular ||
...
void local_bundle_adjuster::optimize_with_imu(
  data::keyframe * curr_keyfrm, bool * const force_stop_flag, bool is_large) const
{
  ...
  const unsigned int optimize_window_size =
    std::min(map_db_->get_num_keyframes() - 2, max_optimize_window_size);
  ...
  const std::vector<data::keyframe *> covisibility_keyframes =
  ...
  for (auto & keyfrm : temporal_keyframes) {
    ...
    if (!keyfrm->imu_is_initialized_ || !pre_keyframe->imu_is_initialized_) {
      spdlog::debug(
        "keyframe {} or keyframe {} isn't do imu initialization", keyfrm->id_, pre_keyframe->id_);
      continue;
    }
    ...
    int temp_velocity_vtx1_id =
      uint64ID_to_IMUvtxID(pre_keyframe->id_, IMU_VertexType::VelocityVtx);
    ...
    if (
      !temp_pose_vtx1 || !temp_velocity_vtx1 || !temp_pose_vtx2 || !temp_velocity_vtx2 ||
    ...
    inertial_edge->setVertex(
      1, dynamic_cast<::g2o::OptimizableGraph::Vertex *>(temp_velocity_vtx1));
    ...
    inertial_edge->setVertex(
      5, dynamic_cast<::g2o::OptimizableGraph::Vertex *>(temp_velocity_vtx2));

    // if the third times of imu initialization hasn't been down or the keyframe is at the window
    // boundary always enable robust kernel to decrease the randomness caused by bad landmarks.

    // if(
    //   keyfrm == temporal_keyframes.back() || data::IMU_Preintegration::imu_initialize_times_ < 3)
    {
  ...
  const unsigned int edges_size =
    (temporal_keyframes.size() + fixed_keyframes.size()) * local_landmarks.size();
  ...
  for (auto & landmark : local_landmarks) {
    ...
    for (auto & observation : observations) {
      ...
      if (keyfrm->camera_->setup_type_ == camera::setup_type_t::Monocular_Inertial) {
        ...
        temp_pose_vtx =
          dynamic_cast<::g2o::OptimizableGraph::Vertex *>(optimizer.vertex(temp_pose_vtx_id));
        mono_edge->setVertex(
  ...
  if (curr_keyfrm->camera_->setup_type_ == camera::setup_type_t::Monocular_Inertial) {
    ...
    for (auto & mono_edge : mono_edges) {
      ...
      if (
        (mono_edge->chi2() > 1.05 * chi2Mono2 && !is_close) ||
        (mono_edge->chi2() > 3 * chi2Mono2 && is_close) || !mono_edge->is_depth_positive()) {
  ...
  for (auto & keyfrm : temporal_keyframes) {
    int temp_pose_vtx_id = uint64ID_to_IMUvtxID(keyfrm->id_, IMU_VertexType::PoseVtx);
    g2o::VertexPose * temp_pose_vtx =
      dynamic_cast<g2o::VertexPose *>(optimizer.vertex(temp_pose_vtx_id));
    ...
    if (keyfrm->imu_is_initialized_) {
      ...
      g2o::VertexVelocity * temp_velocity_vtx =
        dynamic_cast<g2o::VertexVelocity *>(optimizer.vertex(velocity_id));
  ...
  for (auto & landmark : local_landmarks) {
    ...
    ::g2o::VertexPointXYZ * visual_point_vtx =
      dynamic_cast<::g2o::VertexPointXYZ *>(optimizer.vertex(vpt_id));
