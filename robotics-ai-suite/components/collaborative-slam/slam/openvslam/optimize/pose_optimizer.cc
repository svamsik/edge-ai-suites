...
pose_optimizer::pose_optimizer(
  camera::setup_type_t camera_type, bool use_odom, bool is_localization,
  data::map_database * map_db, const unsigned int num_trials, const unsigned int num_each_iter)
: use_odom_(use_odom),
...
void pose_optimizer::set_camera_type(camera::setup_type_t camera_type)
{
  ...
  sqrt_chi_sq_ = (camera_type_ == camera::setup_type_t::Monocular ||
                  camera_type_ == camera::setup_type_t::Monocular_Inertial)
                   ? sqrt_chi_sq_2D
                   : sqrt_chi_sq_3D;
}

unsigned int pose_optimizer::optimize(
  data::frame & frm, bool compute_covariance, data::frame * last_lidar_frame,
  Eigen::Matrix4d * tf_lidar_camera) const
{
  return std::invoke(
    optimize_node_, this, frm, compute_covariance, last_lidar_frame, tf_lidar_camera);
}

unsigned int pose_optimizer::optimize_tracker(
  data::frame & frm, bool compute_covariance, data::frame * last_lidar_frame,
  Eigen::Matrix4d * tf_lidar_camera) const
{
  // 1. optimizer
  ...
  // default eigen solver cannot produce covariance from g2o, so will use csparse solver when
  // we need covariance
  if (compute_covariance) {
    auto linear_solver =
      ::std::make_unique<::g2o::LinearSolverCSparse<::g2o::BlockSolver_6_3::PoseMatrixType>>();
    // TODO rtabmap set as false for all solvers, but shouldn't cause problem here as only
  ...
  // 3. landmark vertex reprojection edge
  ...
  for (unsigned int idx = 0; idx < num_keypts; ++idx) {
    ...
    // frame vertex reprojection edge
    ...
    auto pose_opt_edge_wrap = pose_opt_edge_wrapper(
      &frm, frm_vtx, lm->get_pos_in_world(), idx, undist_keypt.pt.x, undist_keypt.pt.y, x_right,
  ...
  // Phase 1
  // In the feature_extraction_module: everytime a lidar frame is recived with an image frame,
  // we set the lidar_landmarks_.updated to true.  This way, we know that this image frame also has
  // lidar data so here, if the current image frame has lidar data, we want to do the bundle
  // adjustment based on both the lidar and image.
  // in order to know if we are working with two consecutive lidar frames, we have an id for each
  // lidar frame so we only want to do the BA based on lidar and image, if the two lidar frames
  // are consecutive to check for this condtion:
  //   last_lidar_frame->get_prev_lidar_cam_pose().second == (frm.lidar_frame_id_-1)
  if (
    frm.is_lidar_enable() && frm.lidar_landmarks_.updated && last_lidar_frame != NULL &&
    ...
    std::sort(
      ...
      [](
        ...
        const std::tuple<cv::Point2d, cv::Point2d, double> & b) {
    ...
    for (size_t i = 0; i < frm.matched_feature_points_.size(); i++) {
      ...
      g2o::se3::EdgeLidarPose * lidar_edge =
        new g2o::se3::EdgeLidarPose(landmark_position_in_world_frame);
  ...
  for (unsigned int trial = 0; trial < num_trials_; ++trial) {
    ...
    unsigned int diff = 0;
    __builtin_sub_overflow(num_init_obs, num_bad_obs, &diff);
    if (diff < 5) {
      break;
    }
  }
...
// compute_covariance flag, last Lidar frame and transformation of Lidar to camera are not used
// on server side
unsigned int pose_optimizer::optimize_server(
  data::frame & frm, bool /* compute_covariance */, data::frame * /* last_lidar_frame */,
  Eigen::Matrix4d * /* tf_lidar_camera */) const
{
