...
void inertial_only_optimize(
  std::vector<data::keyframe *> & keyframes, KeyframeID max_keyfrm_id, Mat33_t & Rwi,
    ...
    if (keyfrm->pre_keyframe_ && keyfrm->pre_keyframe_->id_ <= max_keyfrm_id) {
      ...
      if (
        ...
        std::cout << "Error " << temp_pose_vtx1 << ", " << temp_velocity_vtx1 << ", "
                  ...
                  << temp_acc_vtx << ", " << temp_gravity_dir_vtx << ", " << temp_scale_vtx
                  << std::endl;
        continue;
      }
      ...
      inertial_edge->setVertex(
        6, dynamic_cast<::g2o::OptimizableGraph::Vertex *>(temp_gravity_dir_vtx));
...
void inertial_only_optimize(
  data::map_database * map_db, KeyframeID max_keyfrm_id, Mat33_t & Rwi, double & scale)
{
...
void visual_inertial_optimize(
  data::map_database * map_db, KeyframeID max_keyfrm_id, int iterations, bool is_monocular,
  bool is_gyro_acc_init, double acc_weight, double gyro_weight)
{
  const std::vector<data::keyframe *> keyframes = map_db->get_all_keyframes();
  ...
  for (auto & keyfrm : keyframes) {
    ...
    if (!is_gyro_acc_init) {
      temp_gyro_vtx1 =
        optimizer.vertex(uint64ID_to_IMUvtxID(pre_keyframe->id_, IMU_VertexType::GyroBiasVtx));
      temp_acc_vtx1 =
        optimizer.vertex(uint64ID_to_IMUvtxID(pre_keyframe->id_, IMU_VertexType::AccBiasVtx));
      temp_gyro_vtx2 =
        optimizer.vertex(uint64ID_to_IMUvtxID(keyfrm->id_, IMU_VertexType::GyroBiasVtx));
      temp_acc_vtx2 =
        optimizer.vertex(uint64ID_to_IMUvtxID(keyfrm->id_, IMU_VertexType::AccBiasVtx));
    } else {
