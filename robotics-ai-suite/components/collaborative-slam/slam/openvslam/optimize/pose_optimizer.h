...
public:
   ...
   * @param num_each_iter
   */
  explicit pose_optimizer(
    camera::setup_type_t camera_type, bool use_odom = false, bool is_localization = false,
    data::map_database * map_db = nullptr, const unsigned int num_trials = 4,
    const unsigned int num_each_iter = 10);
  ...
  unsigned int optimize(
    data::frame & frm, bool compute_covariance = false, data::frame * last_lidar_frame = nullptr,
...
private:
  ...
  unsigned int optimize_server(
    data::frame & frm, bool /* compute_covariance */, data::frame * /* last_lidar_frame */,
    Eigen::Matrix4d * /* tf_lidar_camera */) const;
  typedef unsigned int (pose_optimizer::*optimize_node)(
    data::frame & frm, bool compute_covariance, data::frame * last_lidar_frame,
    Eigen::Matrix4d * tf_lidar_camera) const;
