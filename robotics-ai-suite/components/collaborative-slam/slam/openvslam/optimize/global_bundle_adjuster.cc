...
global_bundle_adjuster::global_bundle_adjuster(
  const KeyframeID first_keyfrm_id_in_db, data::map_database * map_db, const unsigned int num_iter,
  const bool use_huber_kernel)
: first_keyfrm_id_in_db_(first_keyfrm_id_in_db),
  map_db_(map_db),
  num_iter_(num_iter),
...
void global_bundle_adjuster::optimize(
  const KeyframeID lead_keyfrm_id_in_global_BA, bool * const force_stop_flag) const
{
  // 1.
  ...
  for (const auto keyfrm : keyfrms) {
    ...
    auto keyfrm_vtx =
      keyfrm_vtx_container.create_vertex(keyfrm, keyfrm->id_ == first_keyfrm_id_in_db_);
  ...
  for (auto keyfrm : keyfrms) {
    ...
    if (lead_keyfrm_id_in_global_BA == first_keyfrm_id_in_db_) {
...
void global_bundle_adjuster::optimize(
  const KeyframeID lead_keyfrm_id_in_global_BA, bool * const force_stop_flag, const MapID map_id1,
  const MapID map_id2, const KeyframeID fixed_frame_id) const
{
  // 1.

  const auto keyfrms = map_db_->get_all_keyframes(map_id1, map_id2);
  const auto lms = map_db_->get_all_landmarks(map_id1, map_id2);
  std::vector<bool> is_optimized_lm(lms.size(), true);
  ...
  for (const auto keyfrm : keyfrms) {
    ...
    auto keyfrm_vtx = keyfrm_vtx_container.create_vertex(
      keyfrm,
      keyfrm->id_ == fixed_frame_id);  // Need to be modified for multi-slam, set the constant frame
    optimizer.addVertex(keyfrm_vtx);
  }
