...
{
  init_undist_map(camera, K_l, D_l, R_l, K_r, D_r, R_r);
}
...
: model_type_(load_model_type(yaml_node))
{
  ...
  // set camera matrices
  const auto K_l = parse_vector_as_mat(
    cv::Size(3, 3), yaml_node["StereoRectifier.K_left"].as<std::vector<double>>());
  const auto K_r = parse_vector_as_mat(
    cv::Size(3, 3), yaml_node["StereoRectifier.K_right"].as<std::vector<double>>());
  // set rotation matrices
  const auto R_l = parse_vector_as_mat(
    cv::Size(3, 3), yaml_node["StereoRectifier.R_left"].as<std::vector<double>>());
  const auto R_r = parse_vector_as_mat(
    cv::Size(3, 3), yaml_node["StereoRectifier.R_right"].as<std::vector<double>>());
...
void stereo_rectifier::init_undist_map(
  camera::base * camera, const cv::Mat & K_l, const cv::Mat & D_l, const cv::Mat & R_l,
  const cv::Mat & K_r, const cv::Mat & D_r, const cv::Mat & R_r)
{
  // set image size
  ...
  switch (model_type_) {
    case camera::model_type_t::Perspective: {
      cv::initUndistortRectifyMap(
        K_l, D_l, R_l, K_rect, img_size, CV_32F, undist_map_x_l_, undist_map_y_l_);
      cv::initUndistortRectifyMap(
        K_r, D_r, R_r, K_rect, img_size, CV_32F, undist_map_x_r_, undist_map_y_r_);
      break;
    }
    ...
    default: {
      throw std::runtime_error(
        "Invalid model type for stereo rectification: " + camera->get_model_type_string());
    }
  }
}

stereo_rectifier::~stereo_rectifier() { spdlog::debug("DESTRUCT: util::stereo_rectifier"); }

void stereo_rectifier::rectify(
...
cv::Mat stereo_rectifier::parse_vector_as_mat(
  const cv::Size & shape, const std::vector<double> & vec)
{
  cv::Mat mat(shape, CV_64F);
  uint64_t shape_data_size = (uint64_t)shape.height * (uint64_t)shape.width;
  if (shape_data_size != vec.size()) {
    throw std::runtime_error("Invalid stereo rectifier parameter!");
  }
  std::memcpy(mat.data, vec.data(), shape_data_size * sizeof(double));
  return mat;
}
