...
map_database_io::map_database_io(
  data::camera_database * cam_db, data::map_database * map_db, data::bow_database * bow_db,
  data::bow_vocabulary * bow_vocab)
: cam_db_(cam_db), map_db_(map_db), bow_db_(bow_db), bow_vocab_(bow_vocab)
{
}

bool map_database_io::save_message_pack(const std::string & path)
{
  std::lock_guard<std::mutex> lock(data::map_database::mtx_database_);

  std::ofstream ofs(path, std::ios::out | std::ios::binary);
  if (!ofs.is_open()) {
    spdlog::critical("Failed to create map database {}", path);
    return false;
  }

  spdlog::debug("Saving map database to " + path + " ...");
  ...
  nlohmann::json json{
    ...
    {"keyframes", keyfrms},
    {"landmarks", landmarks},
    {"frame_next_id", static_cast<KeyframeID>(data::frame::next_id_)},
    {"keyframe_next_id", static_cast<KeyframeID>(data::keyframe::next_id_)},
    {"landmark_next_id", static_cast<LandmarkID>(data::landmark::next_id_)}};
  ...
  ofs.close();
  spdlog::info("Saved map database to " + path);
...
void map_database_io::load_message_pack(const std::string & path)
{
  ...
  data::frame::next_id_ = json.at("frame_next_id").get<KeyframeID>();
  data::keyframe::next_id_ = json.at("keyframe_next_id").get<KeyframeID>();
  data::landmark::next_id_ = json.at("landmark_next_id").get<LandmarkID>();
  // load database
