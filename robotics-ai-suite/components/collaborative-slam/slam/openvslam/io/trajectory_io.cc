...
void trajectory_io::save_frame_trajectory(
  const std::string & path, const std::string & format) const
{
  ...
  spdlog::info(
    "dump frame trajectory in \"{}\" format from frame {} to frame {} ({} frames)", format,
    reference_keyframes.begin()->first, reference_keyframes.rbegin()->first, num_valid_frms);
  ...
  for (unsigned int i = 0; i < num_valid_frms; ++i, ++rk_itr, ++rc_itr) {
    ...
    if (format == "KITTI") {
      ofs << std::setprecision(9) << cam_pose_wc(0, 0) << " " << cam_pose_wc(0, 1) << " "
          << cam_pose_wc(0, 2) << " " << cam_pose_wc(0, 3) << " " << cam_pose_wc(1, 0) << " "
          << cam_pose_wc(1, 1) << " " << cam_pose_wc(1, 2) << " " << cam_pose_wc(1, 3) << " "
          << cam_pose_wc(2, 0) << " " << cam_pose_wc(2, 1) << " " << cam_pose_wc(2, 2) << " "
          << cam_pose_wc(2, 3) << std::endl;
    } else if (format == "TUM") {
      ...
      ofs << std::setprecision(15) << timestamps.at(frm_id) << " " << std::setprecision(9)
          << trans_wc(0) << " " << trans_wc(1) << " " << trans_wc(2) << " " << quat_wc.x() << " "
          << quat_wc.y() << " " << quat_wc.z() << " " << quat_wc.w() << std::endl;
    } else {
...
void trajectory_io::save_keyframe_trajectory(
  const std::string & path, const std::string & format) const
{
  std::lock_guard<std::mutex> lock(data::map_database::mtx_database_);
  ...
  std::sort(
    keyfrms.begin(), keyfrms.end(),
    [&](data::keyframe * keyfrm_1, data::keyframe * keyfrm_2) { return *keyfrm_1 < *keyfrm_2; });
  ...
  spdlog::info(
    "dump keyframe trajectory in \"{}\" format from keyframe {} to keyframe {} ({} keyframes)",
  ...
  for (const auto keyfrm : keyfrms) {
    ...
    if (format == "KITTI") {
      ...
      const Vec3_t & trans_wc = cam_pose_wc.block<3, 1>(0, 3);
      const Quat_t quat_wc = Quat_t(rot_wc);
      ofs << std::setprecision(15) << timestamp << " " << std::setprecision(9) << trans_wc(0) << " "
          << trans_wc(1) << " " << trans_wc(2) << " " << quat_wc.x() << " " << quat_wc.y() << " "
          << quat_wc.z() << " " << quat_wc.w() << std::endl;
    } else {
      throw std::runtime_error("Not implemented: trajectory format \"" + format + "\"");
    }
  }
