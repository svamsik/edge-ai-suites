...
std::vector<data::keyframe *> global_optimization_module::get_keyframe_local_map(
  data::keyframe * keyframe, size_t numTemporalKFs)
{
  ...
  std::vector<data::keyframe *> CovisibleKFs =
    keyframe->graph_node_->get_top_n_covisibilities(numTemporalKFs);
  ...
  if (numTemporalKFs > 0) {
    ...
    while (set_LocalWindowKFs.size() < numTemporalKFs && nNumTries < nMaxTries) {
      ...
      for (const auto & pKFi : set_LocalWindowKFs) {
        std::vector<data::keyframe *> KFiCov =
          pKFi->graph_node_->get_top_n_covisibilities(numTemporalKFs / 2);
        for (const auto & pKFcov : KFiCov) {
          if (
            pKFcov && !pKFcov->will_be_erased() &&
  ...
  std::copy(
    set_LocalWindowKFs.begin(), set_LocalWindowKFs.end(), std::back_inserter(vec_LocalWindowKFs));

  return vec_LocalWindowKFs;
}

void global_optimization_module::transform_map_via_sim3(
...
{
  std::vector<data::keyframe *> curr_map_all_keyframes = map_db_->get_all_keyframes(
    keyframe_to_be_transformed->get_map_id(), keyframe_to_be_transformed->get_map_id());

  std::lock_guard<std::mutex> lock(data::map_database::mtx_database_);
  std::lock_guard<std::mutex> lock1(
    data::map_database::mtx_map_database_[keyframe_to_be_transformed->get_map_id()]);
  ...
  Sim3s_nw_after_correction = get_Sim3s_after_loop_correction(
    cam_pose_wc_before_correction, g2o_Sim3_cw_after_correction, curr_map_all_keyframes);
...
void global_optimization_module::transform_map_via_sim3(
  data::keyframe * keyframe_to_be_transformed, g2o::Sim3 T_sim3,
  module::keyframe_Sim3_pairs_t & Sim3s_nw_before_correction,
  ...
  module::keyframe_Sim3_pairs_t & Sim3s_nw_after_correction)
{
  ...
  Sim3s_nw_after_correction =
    get_Sim3s_after_correction_via_sim3_transform(T_sim3, curr_map_all_keyframes);
...
void global_optimization_module::correct_loop() { spdlog::debug("start correcting loop!"); }
...
module::keyframe_Sim3_pairs_t
global_optimization_module::get_Sim3s_after_correction_via_sim3_transform(
...
void global_optimization_module::replace_duplicated_landmarks(
  ...
  bool replace_cand_landmarks) const
{
  ...
  spdlog::debug(
    "replaced_landmarks_num between current frame and loop frame is: {}", replaced_landmarks_num);
  // resolve duplications of landmarks between the current keyframe and the candidates
  // of the loop candidate
  ...
  for (const auto & t : Sim3s_nw_after_correction) {
    ...
    std::vector<data::landmark *> lms_to_replace(
      curr_match_lms_observed_in_cand_covis.size(), nullptr);
    fuser.detect_duplication(
      neighbor, Sim3_nw_after_correction, curr_match_lms_observed_in_cand_covis, 4, lms_to_replace);
  ...
  spdlog::debug(
    "replaced_landmarks_num between current frame and loop candidates is: {}",
    replaced_landmarks_num);
}

auto global_optimization_module::extract_new_connections(
  ...
  -> std::map<data::keyframe *, std::set<data::keyframe *>>
{
  ...
  for (auto covisibility : covisibilities) {
    ...
    if (new_connections.at(covisibility).empty()) {
      new_connections.erase(covisibility);
    }
  }
