...
#include "module/server/server_loop_bundle_adjuster.h"
#include "module/server/server_loop_detector.h"
...
public:
  ...
  //! Destructor
  virtual ~global_optimization_module();
...
protected:
  ...
  virtual void correct_loop();

  //! Compute Sim3s (world to covisibility) which are prior to loop correction
  module::keyframe_Sim3_pairs_t get_Sim3s_before_loop_correction(
    const std::vector<data::keyframe *> & neighbors) const;

  //! Compute Sim3s (world to covisibility) which are corrected using the
  //! estimated Sim3 of the current keyframe
  module::keyframe_Sim3_pairs_t get_Sim3s_after_loop_correction(
    const Mat44_t & cam_pose_wc_before_correction, const g2o::Sim3 & g2o_Sim3_cw_after_correction,
    const std::vector<data::keyframe *> & neighbors) const;

  module::keyframe_Sim3_pairs_t get_Sim3s_after_correction_via_sim3_transform(
    const g2o::Sim3 & Transform_sim3, const std::vector<data::keyframe *> & neighbors) const;

  void transform_map_via_sim3(
    module::keyframe_Sim3_pairs_t & Sim3s_nw_before_correction,
    module::keyframe_Sim3_pairs_t & Sim3s_nw_after_correction,
    const g2o::Sim3 g2o_Sim3_cw_after_correction, const data::keyframe * keyframe) const;
  ...
  std::map<data::keyframe *, std::set<data::keyframe *>> extract_new_connections(
  ...
  std::vector<data::keyframe *> get_keyframe_local_map(
    data::keyframe * keyframe, size_t numTemporalKFs = -1);
  ...
  //! loop detector
  std::unique_ptr<module::server_loop_detector> loop_detector_ = nullptr;
  //! loop bundle adjuster
  std::unique_ptr<module::server_loop_bundle_adjuster> loop_bundle_adjuster_ = nullptr;
  ...
  g2o::Sim3 g2o_sim3_cand_to_curr_;

  const double front_rear_camera_constraint_thr_;

  const int iteration_times_;

  bool segment_optimize_;

  std::atomic<bool> running_optimization_ = false;
};
