...
bool within_map_boundary(
  const std::shared_ptr<se::Octree<SE_FIELD_TYPE>> octree, const Eigen::Vector3i & coords,
...
void save_octree_map(
  std::string & file_path, const struct camera_intrinsics & intrinsics,
  const struct se_cfg & octree_cfg, se::Octree<SE_FIELD_TYPE> & octree)
{
  ...
  octree_map.write(
    reinterpret_cast<char *>(const_cast<struct camera_intrinsics *>(p_intrinsics)),
    sizeof(intrinsics));
  const struct se_cfg * p_octree_cfg = &octree_cfg;
  octree_map.write(
    reinterpret_cast<char *>(const_cast<struct se_cfg *>(p_octree_cfg)), sizeof(octree_cfg));
...
void load_octree_map(
  std::string & file_path, struct camera_intrinsics & intrinsics, struct se_cfg & octree_cfg,
...
void add_color_to_marker_points(
  visualization_msgs::msg::Marker & marker, std::string color_axis, float color_axis_min,
...
remapping_region::remapping_region(struct point_2d vertexes[4], bool log_info)
{
  ...
  if (log_info) {
    spdlog::info("Construct remapping region!");
  }
  ...
  for (unsigned int i = 0; i < 4; i++) {
    ...
    if (log_info) {
      spdlog::info(
        "Edge {}: [({}, {}), ({}, {})]", i, edges_[i].vertexes_[0].x_, edges_[i].vertexes_[0].y_,
...
bool remapping_region::two_line_segments_intersect(
  struct line_segment_2d line1, struct line_segment_2d line2)
{
  if (
    cross_product(line1, {line1.vertexes_[0], line2.vertexes_[0]}) *
      cross_product(line1, {line1.vertexes_[0], line2.vertexes_[1]}) <
    0) {
    if (
      cross_product(line2, {line2.vertexes_[0], line1.vertexes_[0]}) *
        cross_product(line2, {line2.vertexes_[0], line1.vertexes_[1]}) <
      0) {
      return true;
    }
  }
...
bool remapping_region::intersect_with_any_edges(struct line_segment_2d line)
{
  for (auto & edge : edges_) {
    if (two_line_segments_intersect(edge, line)) {
      return true;
    }
  }
...
bool remapping_region::within_remapping_region(struct point_2d point)
{
  ...
  for (int i = 0; i < 4; i++) {
    if (two_line_segments_intersect(edges_[i], infinite_line)) {
      intersection_count++;
    }
  }
...
{
  return pose_id & ((1ULL << BIT_SHIFTS) - 1);
}
...
bool remapping_region::intersect_with_region(class remapping_region region)
{
  ...
  for (auto & edge : region.edges_) {
    ...
    if (within_remapping_region(vertex)) {
      return true;
    }
  }
  ...
  for (auto & edge : edges_) {
    struct point_2d vertex = edge.vertexes_[0];
    if (region.within_remapping_region(vertex)) {
      return true;
    }
  }
  ...
  for (auto & edge : region.edges_) {
    if (intersect_with_any_edges(edge)) {
      return true;
    }
  }
