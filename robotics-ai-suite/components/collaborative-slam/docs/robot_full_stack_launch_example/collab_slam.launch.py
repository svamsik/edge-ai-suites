...
def generate_segway_description():
    ld_list = [Node(package='segwayrmp', executable='SmartCar', output='screen')]
...
def generate_tracker_description():
    ...
    ld_list = [
        DeclareLaunchArgument(name='use_odom', default_value='true'),
        DeclareLaunchArgument(name='odom_tf_query_timeout', default_value='25.0'),
        DeclareLaunchArgument(name='odom_buffer_query_timeout', default_value='25.0'),
        DeclareLaunchArgument(name='baselink_frame', default_value='base_link'),
        DeclareLaunchArgument(name='image_frame', default_value='camera_color_optical_frame'),
        DeclareLaunchArgument(name='camera', default_value='camera'),
        DeclareLaunchArgument(name='camera_setup', default_value='RGBD'),
        DeclareLaunchArgument(name='get_camera_extrin_from_tf', default_value='false'),
        DeclareLaunchArgument(name='camera_extrin_query_timeout', default_value='10.0'),
        DeclareLaunchArgument(name='publish_tf', default_value='true'),
        DeclareLaunchArgument(name='pub_tf_child_frame', default_value='odom'),
        DeclareLaunchArgument(name='pub_tf_parent_frame', default_value='map'),
        DeclareLaunchArgument(name='queue_size', default_value='1'),
        DeclareLaunchArgument(name='tracker_rviz', default_value='true'),
        DeclareLaunchArgument(name='gui', default_value='false'),
        DeclareLaunchArgument(name='tracker_log_level', default_value='warning'),
        # Please change the value of camera_fps
        # that represents the real FPS of input camera images,
        # the default value is 30.0 if you don't specify it
        DeclareLaunchArgument(name='camera_fps', default_value='30.0'),
...
def generate_server_description():
    ld_list = [
        DeclareLaunchArgument(name='fix_scale', default_value='true'),
        DeclareLaunchArgument(name='server_rviz', default_value='false'),
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(
                    get_package_share_directory('univloc_server'), 'launch/server.launch.py'
                )
            ),
            launch_arguments={
...
def generate_realsense_description():
    ld_list = [
        DeclareLaunchArgument(name='align_depth', default_value='true'),
        DeclareLaunchArgument(name='device_type', default_value='d435'),
        DeclareLaunchArgument(name='initial_reset', default_value='true'),
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(
...
def generate_fast_mapping_description():
    ...
    ld_list = [
        DeclareLaunchArgument(name='zmin', default_value='0.3'),
        DeclareLaunchArgument(name='zmax', default_value='0.6'),
        DeclareLaunchArgument(name='max_depth_range', default_value='1.5'),
...
def generate_static_tf_description():
    ld_list = [
        DeclareLaunchArgument(name='static_tf_x', default_value='0.24'),
        DeclareLaunchArgument(name='static_tf_y', default_value='0'),
        DeclareLaunchArgument(name='static_tf_z', default_value='0.17'),
        DeclareLaunchArgument(name='static_tf_yaw', default_value='0'),
        DeclareLaunchArgument(name='static_tf_pitch', default_value='0'),
        DeclareLaunchArgument(name='static_tf_roll', default_value='0'),
        DeclareLaunchArgument(name='static_tf_use_sim_time', default_value='False'),
        Node(
            package='tf2_ros',
            ...
            parameters=[{'use_sim_time': LaunchConfiguration('static_tf_use_sim_time')}],
        ),
    ]
...
def generate_navigation_description():
    navigation_launch = IncludeLaunchDescription(
        ...
        launch_arguments={'params_file': LaunchConfiguration('nav_params_file')}.items(),
    )

    ld_list = [
        DeclareLaunchArgument(name='nav_params_file', default_value=''),
...
def generate_launch_description():
    ...
    launch_description = (
        segway + slam_tracker + slam_server + realsense + fast_mapping + static_tf + navigation
    )
