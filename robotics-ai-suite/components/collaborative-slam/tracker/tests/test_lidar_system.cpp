...
public:
  TrackerMonitor() : Node("tracker_monitor")
  {
    ...
    lidar_feature_failure_threshold =
      declare_parameter<int>("lidar_feature_failure_threshold", 150);
    auto qos = rclcpp::SensorDataQoS();
    sub_lidar_states_ = this->create_subscription<univloc_msgs::msg::LidarStatus>(
      "univloc_tracker_0/lidar_states", qos,
      std::bind(&TrackerMonitor::lidar_states_callback, this, std::placeholders::_1));
  }

  std::pair<int, int> get_failure_num() const { return {states_.first, states_.second}; }
