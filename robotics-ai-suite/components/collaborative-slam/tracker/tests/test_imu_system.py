...
def args_parse(default_bagfile_dir):
    ...
    parser.add_argument(
        ...
        '--bad_imu_threshold',
        type=int,
        dest='bad_imu_threshold',
        help='The threshold of bad imu status published by tracker to pass the test',
        default=0,
    )
...
def generate_launch_description():
    ld = LaunchDescription(
        [
            ...
            DeclareLaunchArgument(name='camera_setup', default_value='Monocular_Inertial'),
            DeclareLaunchArgument(name='clean_keyframe', default_value='true'),
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    os.path.join(
                        get_package_share_directory('univloc_tracker'),
                        'launch/euroc_mono.launch.py',
                    )
                ),
                launch_arguments={
