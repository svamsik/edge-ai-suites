...
{
public:
  TrackerMonitor() : Node("tracker_monitor")
  {
    ...
    sub_imu_status_ = this->create_subscription<univloc_msgs::msg::ImuStatus>(
      "univloc_tracker_0/imu_states", qos,
      std::bind(&TrackerMonitor::imu_status_callback, this, std::placeholders::_1));
  }

  int get_bad_imu_num() const { return bad_imu_status_; }
...
TEST(testImuSystem, testImuSystem)
{
  ...
  node.reset();
  if (!imu_robust_init) {
    std::cout << "Failed to finish IMU initialization or IMU resets after initialization"
              << std::endl;
    FAIL();
  } else {
    std::cout << "Succeeded to finish IMU initialization and never reset after initialization"
              << std::endl;
  }

  if (bad_imu_num > bad_imu_threshold) {
    std::cout << "Total bad imu num is: " << bad_imu_num
              << ", which is larger than threshold: " << bad_imu_threshold << std::endl;
    FAIL();
  } else {
    std::cout << "Total bad imu num is: " << bad_imu_num
              << ", which is equal or smaller than threshold: " << bad_imu_threshold << std::endl;
  }
  SUCCEED();
}
