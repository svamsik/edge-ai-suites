...
public:
  TrackerMonitor() : Node("tracker_monitor")
  {
    ...
    sub_pose_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
      ...
      std::bind(&TrackerMonitor::pose_callback, this, std::placeholders::_1));
  }
...
int main(int argc, char * argv[])
{
  ...
  if (pose_num > pose_threshold) {
    std::cout << "Tracker total published pose num: " << pose_num
              << " larger than threshold: " << pose_threshold;
    exit(0);
  } else {
    std::cout << "Tracker total published pose num: " << pose_num
              << " smaller than threshold: " << pose_threshold;
    exit(1);
  }
}
