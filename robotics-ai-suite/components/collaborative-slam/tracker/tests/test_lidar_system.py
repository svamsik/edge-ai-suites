...
def args_parse(default_bagfile_dir):
    ...
    parser.add_argument(
        ...
        type=str,
        dest='exefile_dir',
        help='The directory which stores the test_lidar_system exe',
        default='./',
    )
    parser.add_argument(
        '-tp',
        '--lidar_pose_failure_threshold',
        type=int,
        dest='lidar_pose_failure_threshold',
        help='The failure pose threshold of lidar based tracking',
        default=15,
    )
    parser.add_argument(
        '-tf',
        '--lidar_feature_failure_threshold',
        type=int,
        dest='lidar_feature_failure_threshold',
        help='The failure lidar feature threshold',
        default=150,
    )
...
def main():
    ...
    tracker_path = os.path.join(
        get_package_prefix('univloc_tracker'), 'lib/univloc_tracker/univloc_tracker_ros'
    )
    ...
    threshold_feature_param = 'lidar_feature_failure_threshold:=' + str(
        args_dict['THRESHOLD_FEATURE']
    )
    ...
    playback_process = ExecuteProcess(
        cmd=playback_cmd, shell=True, output='screen', on_exit=EmitEvent(event=Shutdown())
    )
