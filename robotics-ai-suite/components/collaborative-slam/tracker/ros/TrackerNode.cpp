...
#include <cv_bridge/cv_bridge.h>  // Humble uses .h
#else
#include <cv_bridge/cv_bridge.hpp>  // Jazzy uses .hpp
#endif
...
bool TrackerNode::init()
{
  ...
  auto subscribe_image_topic =
    [this](ImageSubscriberFilter & sub, std::string topic, const std::string hint = "raw") {
      rclcpp::QoS qos{rclcpp::KeepAll()};
      sub.subscribe(this, topic, hint, qos.get_rmw_qos_profile());
    };
#else
  auto subscribe_image_topic =
    [this](ImageSubscriberFilter & sub, std::string topic, const std::string = "raw") {
      rclcpp::QoS qos{rclcpp::KeepAll()};
  ...
  if (p_image_topic_ == "") {
    p_image_topic_ = p_camera + "/color/image_raw";
  }
  get_ros_param("depth_topic", p_depth_topic_, std::string(""));
  if (p_depth_topic_ == "") {
    p_depth_topic_ = p_camera + "/aligned_depth_to_color/image_raw";
  }
  get_ros_param("right_image_topic", p_right_image_topic_, std::string(""));
  get_ros_param(
    "accel_measurement_scale", p_accel_measurement_scale_,
  ...
  pub_server_landmarks_observation_ =
    create_publisher<visualization_msgs::msg::Marker>("~/server_landmarks_observation", 10);
  ...
  if (p_camera_info_topic == "") {
    p_camera_info_topic = replace_last_subtopic(p_image_topic_, "camera_info");
  }
  ...
  if (publish_tf_) {
    // set ROS frames between which we should publish tf
    // default: reference frame -> RGB image frame
  ...
  if (cfg_->enable_imu()) {
    ...
    if (imu_topic_ != "") {
      ...
      if (acc_topic_ == "" || gyro_topic_ == "") {
        ROS_ERROR(
          "If IMU is enabled, provide either imu_topic, or as an alternative, \
      ...
      imu_sync_.registerCallback(
        std::bind(
          &TrackerNode::accgyr_callback, this, std::placeholders::_1, std::placeholders::_2));
    }
  ...
  if (use_lidar_) {
    ...
    laser_scan_sub_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
      "/scan", default_qos, std::bind(&TrackerNode::lidar_callback, this, std::placeholders::_1));
  ...
  if (cfg_->enable_fast_mapping_) {
    ...
    if (cfg_->zmin_ < 0 || cfg_->zmax_ < 0 || cfg_->zmin_ > cfg_->zmax_) {
      ROS_ERROR(
        "zmin and zmin should be a positive number and zmin cannot be set larger than zmax!");
      return false;
    }
    ...
    if (min_acceptable_z < MIN_DISTANCE_TO_GROUND) {
      ROS_ERROR(
        "Please set a larger zmax: from %.2f to bigger than %.2f, or Octree map may contain floor "
        "features!",
    ...
    if (cfg_->zmin_ < min_acceptable_z) {
      ROS_ERROR(
        "Please set a larger zmin: from %.2f to bigger than %.2f, or Octree map may contain floor "
        "features!",
  ...
  if (cfg_->mode_ == "remapping") {
    ...
    // the first condition "size == 1 && vector[0] = 0.0" is the default value of remapping_region
    // option in config file
    // since we allow the tracker construct the entire octree map in the two session solution,
    // the remapping_region option is not mandatory currently
    // we also provide the remaping_region option on server side and the user can only configure
    // the one on server side
    if (
      cfg_->remapping_region_vertexes_.size() != 1 ||
      cfg_->remapping_region_vertexes_[0] > EPSILON ||
      cfg_->remapping_region_vertexes_[0] < -EPSILON) {
      if (cfg_->remapping_region_vertexes_.size() != 8) {
        ROS_ERROR(
          "Option remapping_region should include 4 points in the format of [P1.x, P1.y, P2.x, \
...
bool TrackerNode::read_config_from_ros(
  std::string camera_info_topic, std::string right_camera_info_topic)
{
  ...
  if (temp_id < 0 || temp_id > 255) {
    ROS_WARN(
      "Tracker ID should be in [0, 255]: given %d, casted to %hhu", temp_id, cfg_->client_id_);
  }
  ...
  if (p_get_camera_info_from_topic) {
    ...
    cfg_->height_ = camera_info_msg->height;
    {
      ...
      // however, for fisheye camera, only the first 4 parameters are useful;
      ...
      if (cfg_->model_type_str_ == "Fisheye") {
        ...
        for (int i = 0; i < 4; ++i) {
          distortion_tmp.push_back(camera_d[i]);
        }
    ...
    if (cfg_->image_frame_ == "") {
      cfg_->image_frame_ = camera_info_msg->header.frame_id;
    }
    while (cfg_->is_stereo()) {
      ...
      if (cfg_->tf_right_camera_frame_ == "") {
        ...
        cfg_->tf_right_camera_frame_ =
          camera_info_msg->header.frame_id.substr(0, start_position) + "infra2" +
          camera_info_msg->header.frame_id.substr(start_position + 6, id_size - start_position - 6);
      }
      Eigen::Matrix4d left_to_right;
      if (!get_tf_matrix(
            cfg_->tf_left_right_, camera_info_msg->header.frame_id, cfg_->tf_right_camera_frame_,
            rosx::Time(0))) {
      ...
      }
      {
        ...
        cfg_->K1 =
          (cv::Mat_<double>(3, 3) << K1[0], K1[1], K1[2], K1[3], K1[4], K1[5], K1[6], K1[7], K1[8]);
        cfg_->K2 =
          (cv::Mat_<double>(3, 3) << K2[0], K2[1], K2[2], K2[3], K2[4], K2[5], K2[6], K2[7], K2[8]);
    ...
    if (!cfg_->is_stereo()) {
      ...
      if (intrinsics.size() != 6) {
        ROS_ERROR(
          "Please input correct intrinsics, current size = %zu, but expected size = 6",
          intrinsics.size());
        return false;
      }

      if (
        (cfg_->model_type_str_ == "Perspective" && distortion.size() != 5) ||
      ...
      if (cfg_->enable_rectifier_) {
        if (cfg_->rectifier_params_given_) {
          ...
          if (intrinsics.size() != 10) {
            ROS_ERROR(
              "Please input correct intrinsics, current size =%zu, but expected size = 10",
              intrinsics.size());
            return false;
          }
          ...
          if (cfg_->model_type_str_ == "Perspective" && distortion.size() != 10) {
            ROS_ERROR(
              "Please input correct distortions, current size = %zu, but perspective camera size = "
              "10",
              distortion.size());
            return false;
          }
          ...
          if (cfg_->model_type_str_ == "Fisheye" && distortion.size() != 8) {
            ROS_ERROR(
              "Please input correct distortions, current size = %zu, but fisheye camera size = 8",
              distortion.size());
            return false;
          }
        }
        ...
        // 1) normal case (kitti), the intrinsic and distortion matrix would be the same for stereo cameras
        ...
        if (intrinsics.size() != 10 && intrinsics.size() != 6) {
          ROS_ERROR(
            "Please input correct intrinsics, current size = %zu, but expected size = 10 or 6",
            intrinsics.size());
          return false;
        }

        if (
          cfg_->model_type_str_ == "Perspective" &&
          (distortion.size() != 10 && distortion.size() != 5)) {
          ROS_ERROR(
            "Please input correct distortions, current size = %zu, but perspective camera size = "
            "10 or 5",
            distortion.size());
        ...
        if (
          cfg_->model_type_str_ == "Fisheye" &&
          (distortion.size() != 8 && distortion.size() != 4)) {
          ROS_ERROR(
            "Please input correct distortions, current size = %zu, but fisheye camera size = 8 or "
            "4",
            distortion.size());
          return false;
        }
      ...
      for (int i = 0; i < distortion_size; ++i) {
        distortion_tmp.push_back(distortion[i]);
      }
      cfg_->set_distortion_params(distortion_tmp);
      ...
      if (cfg_->enable_rectifier_) {
        if (cfg_->rectifier_params_given_) {
          ...
          if (
            K_left.size() != 9 || K_right.size() != 9 || D_left.size() != 5 ||
            D_right.size() != 5 || R_left.size() != 9 || R_right.size() != 9) {
            ROS_ERROR("Please provide rectifier related parameters in launch file");
            return false;
          }
          cfg_->K_left_ =
            (cv::Mat_<double>(3, 3) << K_left[0], K_left[1], K_left[2], K_left[3], K_left[4],
             K_left[5], K_left[6], K_left[7], K_left[8]);
          cfg_->K_right_ =
            (cv::Mat_<double>(3, 3) << K_right[0], K_right[1], K_right[2], K_right[3], K_right[4],
             K_right[5], K_right[6], K_right[7], K_right[8]);
          cfg_->D_left_ =
            (cv::Mat_<double>(5, 1) << D_left[0], D_left[1], D_left[2], D_left[3], D_left[4]);
          cfg_->D_right_ =
            (cv::Mat_<double>(5, 1) << D_right[0], D_right[1], D_right[2], D_right[3], D_right[4]);
          cfg_->R_left_ =
            (cv::Mat_<double>(3, 3) << R_left[0], R_left[1], R_left[2], R_left[3], R_left[4],
             R_left[5], R_left[6], R_left[7], R_left[8]);
          cfg_->R_right_ =
            (cv::Mat_<double>(3, 3) << R_right[0], R_right[1], R_right[2], R_right[3], R_right[4],
             R_right[5], R_right[6], R_right[7], R_right[8]);
        } else {
          cfg_->K1 =
            (cv::Mat_<double>(3, 3) << intrinsics[2], 0, intrinsics[4], 0, intrinsics[3],
             intrinsics[5], 0, 0, 1);
          cfg_->K2 =
            (cv::Mat_<double>(3, 3) << intrinsics[6], 0, intrinsics[8], 0, intrinsics[7],
             intrinsics[9], 0, 0, 1);
          cfg_->D1 =
            (cv::Mat_<double>(4, 1) << distortion[0], distortion[1], distortion[2], distortion[3]);
          cfg_->D2 =
            (cv::Mat_<double>(4, 1) << distortion[4], distortion[5], distortion[6], distortion[7]);
          ...
          cfg_->tf_left_right_ << T[0], T[1], T[2], T[3], T[4], T[5], T[6], T[7], T[8], T[9], T[10],
            T[11], 0, 0, 0, 1;
        }
      }
    }
  ...
  do {
    ...
    if (exist(cfg_->vocabulary_path_)) {
      break;
    }
  ...
  // for stereo/RGBD input, true depth threshold equals to camera_baseline multiplied by the depth
  // threshold factor
  // for monocular input, depth threshold will not be used
  if (cfg_->is_stereo() || cfg_->is_rgbd()) {
    cfg_->depth_threshold_ = cfg_->camera_baseline_ * cfg_->depth_threshold_;
  }
  ...
  if (cfg_->mode_ == "localization") {
    ...
    if (temp_divisor < 1) {
      ROS_ERROR(
        "the divisor to trigger relocalization request should be positive, given %d", temp_divisor);
      return false;
    }
  ...
  if (T_image_to_camera.size() == 16) {
    ...
    T_image_to_camera.size() != 1 ||
    (T_image_to_camera.size() == 1 && T_image_to_camera[0] != 0.0)) {
  ...
  if (get_camera_extrin_from_tf) {
    ...
    // so we need a loop to query this transformation and also a timeout parameter for default camera
    // extrinsic parameter
    const auto query_start = std::chrono::system_clock::now();
    std::chrono::duration<float> query_duration(0.0);
    while ((get_camera_extrin_succeed = get_tf_matrix(
              cfg_->tf_base_camera_, base_frame, cfg_->image_frame_, rosx::Time(0))) == false) {
      const auto query_now = std::chrono::system_clock::now();
      query_duration = query_now - query_start;
      if (query_duration.count() > camera_extrin_query_timeout) {
        break;
      }
      std::this_thread::sleep_for(std::chrono::milliseconds(500));
    }
    // cfg_->tf_base_camera_ actually means the transformation matrix between base_link and
    // image_frame
    // typically T_image_to_camera should be identity, so it will not influence the result of
    // tf_base_camera_
    // when T_image_to_camera is non-identity (no image_frame provided in tf tree), the value should
    // be updated
    if (T_image_to_camera.size() == 16) {
  ...
  if (cfg_->lidar_enable_) {
    ...
    if (get_lidar_extrin_from_tf) {
      ...
      if (!get_tf_matrix(
            cfg_->tf_base_lidar_, cfg_->base_link_frame_, lidar_frame, rosx::Time(0))) {
  ...
  cfg_->imu_bias_ << imu_bias_vec[0], imu_bias_vec[1], imu_bias_vec[2], imu_bias_vec[3],
    imu_bias_vec[4], imu_bias_vec[5];
  cfg_->imu_noise_ << imu_noise_vec[0], imu_noise_vec[1], imu_noise_vec[2], imu_noise_vec[3],
    imu_noise_vec[4], imu_noise_vec[5];
  cfg_->imu_bias_walk_ << imu_bias_walk_vec[0], imu_bias_walk_vec[1], imu_bias_walk_vec[2],
    imu_bias_walk_vec[3], imu_bias_walk_vec[4], imu_bias_walk_vec[5];
...
{
  if (rosx::getNumSubscribers(pub_local_map_) == 0) {
    return;
  }
  ...
  std_msgs::ColorRGBA camera_color, tracked_server_landmark_marker_color,
    covisibility_keyframes_marker_color, non_covisibility_keyframes_marker_color;
  ...
  visualization_msgs::Marker camera_marker, tracked_server_landmark_marker,
    covisibility_keyframes_marker, non_covisibility_keyframes_marker;
  ...
  for (const auto & item : local_landmark_markers) {
    msg.markers.push_back(item.second);
  }
...
{
  if (rosx::getNumSubscribers(pub_occupancy_map_) == 0) {
    return;
  }
    ...
    for (int i = 0; i < block_side; ++i) {
      for (int j = 0; j < block_side; ++j) {
        ...
        for (int k = 0; k < block_side; ++k, center.z += voxel_dim, dataid += block->sideSq) {
          ...
          // we use a larger range of (zmin, zmax) to accept input voxel
          // therefore, here we need to align with the setting and manually extend the range
          if (
            center.z >= (cfg_->zmax_ + 1.5 * block_side * voxel_dim) ||
  ...
  for (const auto & node : nodes) {
    ...
    if (side > MAX_NODE_SIZE) {
      continue;  // if node side is too coarse, skip it.
    }
...
{
  if (rosx::getNumSubscribers(pub_fused_map_) == 0) {
    return;
  }
  ...
  // we use the "ns" field to identify whether the octree map merging has happened or not
  // also, the "id" field is used to indicate how many times the map merging operation has
  // happened for the current tracker
  // because rviz will preserve data with old namespace and id, we decide to set the "action"
  // filed to value "delete" to make the marker which represents pre-merge octree map disappear
  // after map merging
  // the octree merge times won't exceed the maximum value of an unsigned int in real scenarios
  // the actuall value will be no more than tens
  ...
  msg_fused.markers[0].ns =
    (current_octree_merged_times > 0) ? "octree_map_merged" : "octree_map_premerge";
...
sensor_msgs::CameraInfoPtr TrackerNode::get_camera_info_msg_from_topic(
  std::string camera_info_topic)
{
  ...
  subscription_ = create_subscription<sensor_msgs::msg::CameraInfo>(
    ...
    std::bind(&TrackerNode::camera_info_callback, this, std::placeholders::_1));
  ...
  if (
    rclcpp::spin_until_future_complete(shared_from_this(), camera_info_ready_future) !=
...
void TrackerNode::rgbd_callback(
  const sensor_msgs::ImageConstPtr msgRGB, const sensor_msgs::ImageConstPtr msgD)
{
  if (
    msgRGB->header.frame_id == "" || msgRGB->width == 0 || msgRGB->height == 0 ||
  ...
  if (
    msgRGB->width != (unsigned int)cfg_->width_ || msgRGB->height != (unsigned int)cfg_->height_) {
    ROS_WARN(
      "The resolution of the input RGB image is %d * %d! But expected resolution is %d * %d, skip "
      "this frame!",
      msgRGB->width, msgRGB->height, cfg_->width_, cfg_->height_);
    return;
  } else if (
    msgD->width != (unsigned int)cfg_->width_ || msgD->height != (unsigned int)cfg_->height_) {
    ROS_WARN(
      "The resolution of the input Depth image is %d * %d! But expected resolution is %d * %d, "
      "skip this frame!",
  ...
  std::shared_ptr<FrameData> frame =
    std::make_shared<FrameData>(cv_ptrRGB->image, cv_ptrD->image, stamp, cdata);
  tracker_->feed_image(frame);
}

void TrackerNode::stereo_callback(
  const sensor_msgs::ImageConstPtr msg_left, const sensor_msgs::ImageConstPtr msg_right)
{
  if (
    msg_left->header.frame_id == "" || msg_left->width == 0 || msg_left->height == 0 ||
  ...
  if (
    msg_left->width != (unsigned int)cfg_->width_ ||
    msg_left->height != (unsigned int)cfg_->height_) {
    ROS_WARN(
      "The resolution of the input stereo left image is %d * %d! But expected resolution is %d * "
      "%d, skip this "
      "frame!",
      msg_left->width, msg_left->height, cfg_->width_, cfg_->height_);
    return;
  } else if (
    msg_right->width != (unsigned int)cfg_->width_ ||
    msg_right->height != (unsigned int)cfg_->height_) {
    ROS_WARN(
      "The resolution of the input stereo right image is %d * %d! But expected resolution is %d * "
      "%d, skip this "
      "frame!",
  ...
  std::shared_ptr<FrameData> frame =
    std::make_shared<FrameData>(cv_ptr_l->image, cv_ptr_r->image, stamp, cdata);
  tracker_->feed_image(frame);
}
...
void TrackerNode::lidar_callback(const sensor_msgs::msg::LaserScan::ConstSharedPtr laser_scan)
{
  ...
  std::shared_ptr<LidarData> frame = std::make_shared<LidarData>(
    laser_scan->angle_min, laser_scan->angle_max, laser_scan->angle_increment,
    laser_scan->time_increment, laser_scan->scan_time, laser_scan->range_min, laser_scan->range_max,
    laser_scan->ranges, laser_scan->intensities, points, stamp);
  tracker_->feed_lidar(frame);
}

void TrackerNode::accgyr_callback(
  const sensor_msgs::ImuConstPtr acc_msg, const sensor_msgs::ImuConstPtr gyr_msg)
{
...
void TrackerNode::main_loop()
{
  ...
  if (p_stat_log_interval_ > 0) {
    check_timer_ = rclcpp::create_timer(
      this, this->get_clock(), tf2::durationFromSec(p_stat_log_interval_),
  ...
  while (rosx::ok()) {
    ...
    if (cfg_->need_covariance_) {
      maybe_covariance = result->maybe_covariance;
    }
    ...
    if (!maybe_Tcw) {
      ...
      if (
        initialized && num_lost_frames_to_reset_ > 0 &&
        ++lost_frames >= num_lost_frames_to_reset_) {
...
{
  if (rosx::getNumSubscribers(pub_server_landmarks_observation_) == 0) {
    return;
  }
  std::vector<Eigen::Vector3d> tracked_server_landmarks;
  tracker_->get_tracked_server_landmarks(tracked_server_landmarks);
...
void TrackerNode::publish_pose(
  const Eigen::Matrix4d & Tcw, const std_msgs::Header & image_header,
  const Eigen::Matrix<double, 6, 6> & covariance, bool skip_query_transform)
{
  // publish 3D pose:  x right, y down, z forward
  ...
  // since we use the transformation between image_frame and base_link to represent pose
  ...
  if (publish_tf_) {
    ...
    if (skip_query_transform) {
      ROS_DEBUG(
        "Skip query transform in tf buffer on timestamp %f, use last transform to publish tf.",
...
{
  if (rosx::getNumSubscribers(pub_pc_) == 0) {
    return;
  }
...
{
  if (rosx::getNumSubscribers(pub_imu_estimated_path_) == 0) {
    return;
  }
...
void TrackerNode::periodically_print_stats()
{
  ...
  ROS_INFO(
    "UnivLoc%s got %d images in past %.1fs. Localized/processed %d/%d (%.2lf Hz). Totally %d/%d "
    "(%.2f%%).%s",
    tracker_->connected_to_server() ? " (connected)" : " (unconnected)", nreceived, dur.toSec(),
    nlocalized, nprocessed, nprocessed / dur.toSec(), stat_total_localized_, stat_total_processed_,
...
void TrackerNode::save_trajectory_to_file(const Eigen::Matrix4d pose, rosx::Time time_stamp) const
{
  ...
  of_write << std::fixed << time_stamp.toSec() << " " << pose(0, 3) << " " << pose(1, 3) << " "
...
void TrackerNode::odom_pose(rosx::Time stamp)
{
  ...
  rosx::TfLookupResult ret =
    get_tf_matrix(odom_to_camera_pose, cfg_->image_frame_, cfg_->odom_frame_, stamp, timeout);
  if (ret) {
    ROS_DEBUG("Found aligned odom pose, timestamp is %f", stamp.toSec());
    tracker_->feed_odom_data(stamp.toSec(), odom_to_camera_pose);
  } else {
    ROS_INFO(
      "Drop odom pose transform for frame of timestamp %f: %s", stamp.toSec(), ret.error().c_str());
  }
}

rosx::TfLookupResult TrackerNode::get_tf_matrix(
  Eigen::Matrix4d & pose, std::string target, std::string source, rosx::Time stamp,
  rosx::Duration timeout)
{
  ...
  if (!ret && cfg_->tf_fix_frame_ != "") {
    ret =
      get_transform(transform, target, stamp, source, s_stamp, cfg_->tf_fix_frame_, timeout * 1.1);
  }
  if (ret) {
    auto rot = transform.getBasis();
    auto trans = transform.getOrigin();
    pose << rot[0].x(), rot[0].y(), rot[0].z(), trans.x(), rot[1].x(), rot[1].y(), rot[1].z(),
      trans.y(), rot[2].x(), rot[2].y(), rot[2].z(), trans.z(), 0.0, 0.0, 0.0, 1.0;
  }

  return ret;
}

uint32_t TrackerNode::getReceivedFramesCount() const { return this->stat_received_; }
