    ...
    for gyr, t in zip(d400_gyr, d400_gyr_t):
        ...
        if target_t >= pre_t and target_t <= last_t:
            ...
            imu.angular_velocity.x = (
                ratio * pre_gyr.angular_velocity.x + (1.0 - ratio) * last_gyr.angular_velocity.x
            )
            imu.angular_velocity.y = (
                ratio * pre_gyr.angular_velocity.y + (1.0 - ratio) * last_gyr.angular_velocity.y
            )
            imu.angular_velocity.z = (
                ratio * pre_gyr.angular_velocity.z + (1.0 - ratio) * last_gyr.angular_velocity.z
            )
            break
        elif target_t < pre_t:
            break
    ...
    for accer, t in zip(d400_accer, d400_accer_t):
        ...
        if target_t >= pre_t and target_t <= last_t:
            ...
            imu.linear_acceleration.x = (
                ratio * pre_accer.linear_acceleration.x
                + (1.0 - ratio) * last_accer.linear_acceleration.x
            )
            imu.linear_acceleration.y = (
                ratio * pre_accer.linear_acceleration.y
                + (1.0 - ratio) * last_accer.linear_acceleration.y
            )
            imu.linear_acceleration.z = (
                ratio * pre_accer.linear_acceleration.z
                + (1.0 - ratio) * last_accer.linear_acceleration.z
            )
            break
        elif target_t < pre_t:
            break
