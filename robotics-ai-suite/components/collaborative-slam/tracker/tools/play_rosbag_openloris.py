...
keep_time = 0: do not keep relative timestamp between msgs

If you choose not to use use_sim_time=0,
you also have to choose whether to keep relative timestamp between msgs(set keep_time= 0 or 1)

But if you choose to use use_sim_time=1, you don't need to set keep_time

if you are going to use IMU,
    ...
    if hasattr(msg, 'header') and hasattr(msg.header, 'stamp'):
        if not use_sim_time:
            ...
            msg.header.stamp = start_time + (
                msg.header.stamp - reset_stamp_keep_time.start_record_time
            )
        last_time = msg.header.stamp
    elif hasattr(msg, 'transforms'):
        last_tf_publish_time = last_time
        for id in range(len(msg.transforms)):
            if hasattr(msg.transforms[id], 'header') and hasattr(
                msg.transforms[id].header, 'stamp'
            ):
                    ...
                    if not use_sim_time:
                        if hasattr(reset_stamp_keep_time, 'start_record_time'):
                            msg.transforms[id].header.stamp = start_time + (
                                msg.transforms[id].header.stamp
                                - reset_stamp_keep_time.start_record_time
                            )
                        else:
                            reset_stamp_keep_time.start_record_time = msg.transforms[
                                id
                            ].header.stamp
                            msg.transforms[id].header.stamp = start_time + (
                                msg.transforms[id].header.stamp
...
def reset_stamp(msg):
    global use_sim_time, last_time, camera
    t = rospy.Time.now()
    # msg.frame = msg.frame.split("/")[0]+"/"+camera+"/"+msg.frame.split("/")[1:]
    if hasattr(msg, 'header') and hasattr(msg.header, 'stamp'):
        if not use_sim_time:
            msg.header.stamp = t
        else:
    ...
    while running and not rospy.is_shutdown():
        ...
        if keep_relative_time:
            ...
            if last_time.nsecs > 0:
                if (
                    last_tf_publish_time.secs < last_time.secs
                    or last_tf_publish_time.nsecs < last_time.nsecs
                ):
                    pub.publish(reset_stamp_keep_time(msg))
            lock_publish_thread.release()
        else:
...
def main():
    global start_time, use_sim_time, camera
    bagfile = sys.argv[1]
    print(bagfile)
    loop_static_tf = True
    ...
    rospy.init_node('play_rosbag_' + camera)
    ...
    for topic in topics:
        ...
        if loop_static_tf and topic == '/tf_static':
            for topic, msg, t in rosbag.Bag(bagfile).read_messages('/tf_static'):
                ...
                for msg_tf in static_tf_msg.transforms:
                    if 'd400' in msg_tf.header.frame_id:
                        msg_tf.header.frame_id = camera + msg_tf.header.frame_id[4:]
                    if 'd400' in msg_tf.child_frame_id:
                        msg_tf.child_frame_id = camera + msg_tf.child_frame_id[4:]
        ...
        if loop_camera_info and topic == '/d400/color/camera_info':
            ...
            if camera_info_msg is not None:
                camera_info_msg.header.frame_id = camera + camera_info_msg.header.frame_id[4:]
    ...
    if static_tf_msg is not None:
        ...
        static_tf_pub = threading.Thread(
            target=publish_loop, args=(publishers['/tf'], static_tf_msg)
        )
    ...
    if camera_info_msg is not None:
        if '/camera_info' not in publishers:
            publishers['/camera_info'] = rospy.Publisher(
                '/' + camera + '/color/camera_info', CameraInfo, queue_size=10
            )
        camera_info_pub = threading.Thread(
            target=publish_loop, args=(publishers['/camera_info'], camera_info_msg)
        )
