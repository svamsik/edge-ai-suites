...
def quaternion_to_rotation_matrix(quat):
    ...
    rot_matrix = [
        [1.0 - 2 * y2 - 2 * z2, 2 * xy - 2 * zw, 2 * xz + 2 * yw],
...
def transform_point(R, t, p_from):
    p_to = [
        R[0][0] * p_from[0] + R[0][1] * p_from[1] + R[0][2] * p_from[2] + t[0],
...
def CreateBag(args):  # Directory
    ...
    rgbd_imgs, rgbdtimestamps = ReadImages(
        args[0]
    ...
    color_camera_info_msg.D = [0.0644, -0.114, 0.00127, 0.00203, 0.0]
    ...
    pose_quaternion = [
        float(depth2color_extrinsic_msg.transform.rotation.w),
        float(depth2color_extrinsic_msg.transform.rotation.x),
        float(depth2color_extrinsic_msg.transform.rotation.y),
        float(depth2color_extrinsic_msg.transform.rotation.z),
    ]
    t = [
        float(depth2color_extrinsic_msg.transform.translation.x),
        float(depth2color_extrinsic_msg.transform.translation.y),
        float(depth2color_extrinsic_msg.transform.translation.z),
    ]
    ...
    try:
        for i in range(len(imudata)):
            ...
            imuStamp = rospy.rostime.Time.from_sec(
                float(imutimesteps[i]) / 1000000
            )  # according to the timestamp unit
        ...
        for i in range(total_img_num):
            # Adding depth image
            Stamp = rospy.rostime.Time.from_sec(
                float(rgbdtimestamps[i]) / 1000000
            )  # Modify according to your own time unit
            ...
            if align_depth_to_color:
                align_depth_to_color_image = ImagePIL.new(
                    depth_image.mode,
                    (color_camera_info_msg.width, color_camera_info_msg.height),
                    (0),
                )
                for u in range(depth_camera_info_msg.width):
                    for v in range(depth_camera_info_msg.height):
                        ...
                        position_in_depth_frame = [
                            (u - cx_depth) * depth / fx_depth,
                            (v - cy_depth) * depth / fy_depth,
                            depth,
                        ]
                        position_in_color_frame = transform_point(R, t, position_in_depth_frame)
                        position_in_norm_color_frame = [
                            position_in_color_frame[0] / position_in_color_frame[2],
                            position_in_color_frame[1] / position_in_color_frame[2],
                        ]
                        # distortion
                        ...
                        xy = x * y
                        x_distorted = (
                            x * (1 + k1 * r2 + k2 * r4) + 2 * p1 * x * y + p2 * (r2 + 2 * x2)
                        )
                        y_distorted = (
                            y * (1 + k1 * r2 + k2 * r4) + 2 * p2 * x * y + p1 * (r2 + 2 * y2)
                        )
                        ...
                        if (
                            u_image >= 0
                            and u_image < align_depth_to_color_image.width
                            and v_image >= 0
                            and v_image < align_depth_to_color_image.height
                        ):
                ...
                align_depth_to_color_image_msg = br.cv2_to_imgmsg(
                    data
            ...
            if i % 20 == 0:
                ...
                if not align_depth_to_color:
                    print(
                        '\r Progress: {0}{1}%'.format(
                            '*' * (i / 20), ((float(i) * 100) / (float(total_img_num)))
                        ),
                        end='',
                    )
            if align_depth_to_color:
