...
def generate_launch_description():
    configFilePath = os.path.join(
        get_package_share_directory('univloc_tracker'), 'config', 'kitti.yaml'
    )

    with open(configFilePath, 'r') as file:
        configParams = yaml.safe_load(file)['univloc_tracker']
    configParams['camera_setup'] = 'Monocular'
    ...
    description_cond1 = set_conditioned_launch_configuration(
        s00_02, IfCondition(LaunchConfiguration('s00_02'))
    )
    description_cond2 = set_conditioned_launch_configuration(
        s03, IfCondition(LaunchConfiguration('s03'))
    )
    description_cond3 = set_conditioned_launch_configuration(
        s04_12, IfCondition(LaunchConfiguration('s04_12'))
    )
