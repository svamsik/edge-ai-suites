...
params1 = {
    ...
    'camera_intrinsics': '[752.0, 480.0, 435.2046959714599, 435.2046959714599, 367.4517211914062, 252.2008514404297]',  # noqa: E501
    # k1, k2, p1, p2, k3
    ...
    'R_left': '[0.999966347530033, -0.001422739138722922, 0.008079580483432283, 0.001365741834644127, 0.9999741760894847, 0.007055629199258132, -0.008089410156878961, -0.007044357138835809, 0.9999424675829176]',  # noqa: E501
    'K_right': '[457.587, 0.0, 379.999, 0.0, 456.134, 255.238, 0.0, 0.0, 1.0]',
    'D_right': '[-0.28368365, 0.07451284, -0.00010473, -3.555907e-05, 0.0]',
    'R_right': '[0.9999633526194376, -0.003625811871560086, 0.007755443660172947, 0.003680398547259526, 0.9999684752771629, -0.007035845251224894, -0.007729688520722713, 0.007064130529506649, 0.999945173484644]',  # noqa: E501
...
params2 = {
    ...
    'camera_intrinsics': '[752.0, 480.0, 458.654, 457.296, 367.215, 248.375, 457.587, 456.134, 379.999, 255.238]',  # noqa: E501
    # left camera (k1, k2, p1, p2, k3), right camera (k1, k2, p1, p2, k3)
    'camera_distortion': '[-0.28340811, 0.07395907, 0.00019359, 1.76187114e-05, 0.0, -0.28368365, 0.07451284, -0.00010473, -3.555907e-05, 0.0]',  # noqa: E501
...
def declare_configurable_parameters(params):
    return [
        DeclareLaunchArgument(param, default_value=str(params[param]), description='')
        for param in params
    ]
...
def set_conditioned_launch_configuration(params, cond):
    return [
        SetLaunchConfiguration(name=param, value=params[param], condition=cond) for param in params
    ]
...
def generate_launch_description():
    configFilePath = os.path.join(
        get_package_share_directory('univloc_tracker'), 'config', 'euroc.yaml'
    )
    ...
    print(
        'type: {}; value:{}'.format(type(configParams['publish_tf']), configParams['publish_tf'])
    )
    print(
        'type: {}; value:{}'.format(
            type(configParams['rectifier_params_given']), configParams['rectifier_params_given']
        )
    )
    rectifier_params_given = LaunchConfiguration('rectifier_params_given')
    description_cond1 = set_conditioned_launch_configuration(
        params1,
        IfCondition(PythonExpression(['"', rectifier_params_given, '".lower() == "true"'])),
    )
    description_cond2 = set_conditioned_launch_configuration(
        params2,
    ...
    description.append(
        SetLaunchConfiguration(
            name='launch_prefix',
            value='gnome-terminal -- gdb -ex run --args',
    ...
    description.append(
        SetLaunchConfiguration(
            name='image_topic',
            value=['/', LaunchConfiguration('camera'), '/color/image_raw'],
            condition=IfCondition(
                PythonExpression(["'", LaunchConfiguration('image_topic'), "' ==  '\"\"' "])
            ),
        )
    )
    ...
    rviz_config = os.path.join(
        get_package_share_directory('univloc_tracker'), 'config', 'rviz2_tracker.rviz'
    )
