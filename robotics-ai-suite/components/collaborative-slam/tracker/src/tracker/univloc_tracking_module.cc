...
univloc_tracking_module::univloc_tracking_module(
  std::shared_ptr<univloc_tracker::Config> cfg, camera::base * camera, const ClientID client_id,
  data::map_database * map_db, data::bow_database * bow_db, FeatureQueue & feature_queue,
  RequestQueue & result_queue, RequestQueue & reconstruction_queue)
  ...
  switch (tracker_mode_) {
    ...
    case tracker_mode_t::None:
      throw std::invalid_argument(
        "Invalid mode. Available values: mapping, localization, relocalization");
  }

  num_tracked_lms_thr_ = camera_->use_imu_ ? 10 : 20;
  num_tracked_lms_thr_ = cfg_->use_odom_ && tracker_mode_ == tracker_mode_t::Localization
                           ? num_tracked_lms_thr_ / 2
                           : num_tracked_lms_thr_;
  ...
  if (tracking_state_ == tracker_state_t::Tracking) {
    ...
    if (!succeeded && velocity_is_valid_ && last_reloc_frm_id_ + 2 < curr_frm_->id_) {
      ...
      if (cfg_->lidar_enable_) {
        succeeded = frame_tracker_.lidar_based_track(
          *curr_frm_, *last_frm_, velocity_, *last_lidar_frm_, cfg_->tf_lidar_camera_);
        if (!succeeded) {
    ...
    if (!succeeded && data::IMU_Preintegration::state_ == data::IMU_Preintegration::Initialized) {
      succeeded =
        frame_tracker_.imu_based_track(*curr_frm_, *last_frm_, imu_predicted_camera_pose_cw_);
      if (!succeeded) {
...
int univloc_tracking_module::get_lidar_pose_failure_count() { return lidar_pose_failure_count_; }
...
void univloc_tracking_module::run()
{
  ...
  while (true) {
    ...
    if (!feature_queue_.wait_pop(input)) {
      break;
    }
    ...
    if (
      ...
      tracking_state_ == tracker_state_t::RecentlyLost) {
      input.first->maybe_pose = curr_frm_->cam_pose_cw_;
      if (cfg_->need_covariance_) {
        input.first->maybe_covariance = curr_frm_->covariance_matrix_;
      }
    } else {
    ...
    if (!result_queue_.force_push(input.first)) {
      break;
    }
    if (cfg_->enable_fast_mapping_) {
      if (!reconstruction_queue_.force_push(input.first)) {
        break;
      }
    }
...
bool univloc_tracking_module::search_server_landmarks()
{
  ...
  if (server_landmarks.size() < min_server_landmarks_num) {
    spdlog::debug(
      "Not enougn server landmarks for tracking! only {} server landmarks",
  ...
  const float margin = (curr_frm_->id_ < last_reloc_frm_id_ + 2) ? 20.0 : camera_setup_type_margin_;

  unsigned int matched_num =
    projection_matcher.match_frame_and_landmarks(*curr_frm_, nearby_landmarks, margin);

  spdlog::debug(
    " {} of {} server landmarks are observed when tracking server map",
    observed_server_landmarks_num, server_landmarks.size());

  spdlog::debug(
    " {} of {} server landmarks are succesfully matched ", matched_num, nearby_landmarks.size());
...
bool univloc_tracking_module::optimize_current_frame_with_server_map()
{
  ...
  for (unsigned int idx = 0; idx < curr_frm_->num_keypts_; ++idx) {
    ...
    if (!curr_frm_->outlier_flags_.at(idx)) {
      ...
      ++num_tracked_lms_;
      if (lm->get_num_observed() == 0) {
        new_tracked_server_lms_num++;
      }
  ...
  if (num_tracked_lms_ < num_tracked_lms_thr_) {
    spdlog::debug(
      "server map tracking failed: {} matches < {}", num_tracked_lms_, num_tracked_lms_thr_);
    tracked_server_landmarks_.clear();
    return false;
  }
...
void univloc_tracking_module::update_frm_and_keyfrm_using_relocalization(
  data::keyframe * p_keyframe, std::shared_ptr<data::frame> p_frame,
  univloc_msgs::MapConstPtr map_msg_ptr)
{
  ...
  for (uint idx = 0; idx < mp_size; ++idx) {
    ...
    if (
      !map_msg_ptr->only_add_landmarks &&
      (landmark_msg.mn_observed_in_other_keyframe_id > 0 || landmark_msg.mn_replace_other_id > 0)) {
      continue;
    }

    if (map_db_->get_landmark(landmark_msg.mn_id)) {
      continue;
    }

    std::shared_ptr<Landmark> ptr_server_landmark =
      map_db_->get_server_landmark(landmark_msg.mn_id);
    if (ptr_server_landmark) {
      Eigen::Vector3d pos_w(
        (double)landmark_msg.mv_position[0], (double)landmark_msg.mv_position[1],
      ...
      if (landmark_msg.mn_observed_in_other_keyframe_index < p_keyframe->num_keypts_) {
        p_keyframe->add_landmark(
          ptr_server_landmark.get(), landmark_msg.mn_observed_in_other_keyframe_index);
        ptr_server_landmark->add_observation(
          p_keyframe, landmark_msg.mn_observed_in_other_keyframe_index, true);
        // set the 2D-3D associations to the current frame
        p_frame->landmarks_.at(landmark_msg.mn_observed_in_other_keyframe_index) =
          ptr_server_landmark.get();
        p_frame->outlier_flags_.at(landmark_msg.mn_observed_in_other_keyframe_index) = false;
      }
    } else {
      cv::Mat descriptor(
        1, landmark_msg.mv_descriptors[0].m_descriptor.size(), CV_8UC1,
        landmark_msg.mv_descriptors[0].m_descriptor.data());
      Eigen::Vector3d pos_w(
        (double)landmark_msg.mv_position[0], (double)landmark_msg.mv_position[1],
        (double)landmark_msg.mv_position[2]);
      Eigen::Vector3d mean_normal(
        (double)landmark_msg.mv_normal_vector[0], (double)landmark_msg.mv_normal_vector[1],
        (double)landmark_msg.mv_normal_vector[2]);

      data::landmark * p_landmark = new data::landmark(
        landmark_msg.mn_id, landmark_msg.mn_client_id, pos_w, descriptor,
        landmark_msg.mf_max_distance, landmark_msg.mf_min_distance, mean_normal, map_db_, true);
...
bool univloc_tracking_module::track_server_map()
{
  ...
  succeeded =
    frame_tracker_.robust_match_based_track(*curr_frm_, *last_frm_, virtual_server_keyframe);
...
// in case all prior track-based methods (motion, odom, bow) fail, the probability for robust match
// to be positive is really low compared to the performance cost when using odom
// the system will rely on odom data until enough visual features are found to switch back to
// visual tracking
...
void univloc_tracking_module::relocalization_mode()
{
  // lock the map database
  {
    ...
    if (univloc_tracker::Client::get_instance().get_relocalization_results()) {
      relocal_num_++;
    }
  }
  request_relocalization_from_server();
  ...
  last_frm_ = curr_frm_;
  spdlog::info(
    "Relocalization success -- relocal request number: {}, relocal success number: {}",
...
void univloc_tracking_module::request_relocalization_from_server()
{
  ...
  auto keyfrm_to_be_relocalized =
    new (std::nothrow) data::keyframe(*request_frame, map_db_, bow_db_);
  ...
  spdlog::debug(
    "Relocalize send keyframe id is {}, landmark size: {}", keyfrm_to_be_relocalized->id_,
    keyfrm_to_be_relocalized->get_landmarks().size());
  univloc_tracker::Client::get_instance().send_relocalization_request_to_server(
    keyfrm_to_be_relocalized);
...
bool univloc_tracking_module::retrieve_relocalization_result()
{
  ...
  if (reloc_result) {
    ...
    update_motion_model();
    univloc_tracker::Client::get_instance().set_session_start_time(
      reloc_result->session_start_time);
    ...
    // set the reference keyframe and corresponding odom data for odom based tracking and
    // optimization
    ...
    if (
      cfg_->use_odom_ && (camera_->setup_type_ == camera::setup_type_t::Monocular ||
...
bool univloc_tracking_module::is_motion_enough_for_stereo_or_rgbd_input()
{
  if (
    ...
    imu_data_buf_->get_imu_data(
      curr_frm_->timestamp_ - 2.0 * timestamp_interval, curr_frm_->timestamp_ - timestamp_interval,
      imu_vec1, false);
    imu_data_buf_->get_imu_data(
      curr_frm_->timestamp_ - 1.0 * timestamp_interval, curr_frm_->timestamp_, imu_vec2, false);
...
tracker_state_t univloc_tracking_module::localization_track_trans_state_basic(
  bool is_succeeded, [[maybe_unused]] std::any & anydata)
{
  return is_succeeded ? tracker_state_t::Tracking : tracker_state_t::Lost;
}

tracker_state_t univloc_tracking_module::localization_track_trans_state_odom(
  bool is_succeeded, [[maybe_unused]] std::any & anydata)
{
  ...
  if (is_succeeded) {
    ...
    if (
      last_tracking_state_ != tracker_state_t::RecentlyLost &&
...
{
  camera_->use_imu_ = false;  // temporal usage!
  ...
  if (
    ...
    if (succeeded) {
      succeeded = optimize_current_frame_with_server_map();
    }
    ...
    // memory leak
    // will re-enable it if needs to do mapping/localization
  ...
  last_frm_ = curr_frm_;
  spdlog::info(
    "tracking success number: {}, fail number: {}, relocal number: {}, relocal fail number: {}",
    success_num_, lost_num_, relocal_num_, relocal_fail_num_);
}

tracker_state_t univloc_tracking_module::track_trans_state_basic(
  bool is_succeeded, [[maybe_unused]] std::any & anydata)
{
  return is_succeeded ? tracker_state_t::Tracking : tracker_state_t::Lost;
}

tracker_state_t univloc_tracking_module::track_trans_state_odom(
  bool is_succeeded, [[maybe_unused]] std::any & anydata)
{
  // when odom data is available and tracking gets lost, use odom to update the pose
  // the system should switch back to visual tracking if possible
...
tracker_state_t univloc_tracking_module::track_trans_state_imu(
  bool is_succeeded, std::any & anydata)
{
  if (is_succeeded) {
    return tracker_state_t::Tracking;
  }
  ...
  // condition 3: last tracking state is not tracker_state_t::Lost
    ...
    if (last_tracking_state_ != tracker_state_t::RecentlyLost) {
      ...
      if (curr_frm_->timestamp_ - timestamp_recently_lost_ > keep_recently_lost) {
        spdlog::debug(
          "tracking recently lost for {}s, set tracking_state_ to Lost!", keep_recently_lost);
        return tracker_state_t::Lost;
      }
    }
...
void univloc_tracking_module::track()
{
  if (camera_->use_imu_ && mapper_->bad_imu_to_reset_) {
    spdlog::info(
      "tracking: reset map because local mapper set the bad imu flag due to lack of enough "
      "motion!");
  ...
  // since we may not get odom data for the first several frames due to data setting and ROS tf
  // mechanism, thus skip the first several frames to ensure odom is obtained for the first frame
  // to initialize
  if (cfg_->use_odom_) {
    if (!get_odom_data()) {
      return;
  ...
  if (camera_->use_imu_) {
    ...
    if (!is_motion_enough_for_stereo_or_rgbd_input()) {
      return;
    }
  }
  ...
  if (tracking_state_ == tracker_state_t::Initializing) {
    ...
    if (
      curr_frm_->camera_->setup_type_ == camera::setup_type_t::Monocular_Inertial ||
      curr_frm_->camera_->setup_type_ == camera::setup_type_t::Monocular) {
      if (keyfrms.size() != 2) {
        spdlog::error(
          "{}: After monocular initializing, there must be 2 keyframes in map_db_", __func__);
        return;
      }
      ...
      spdlog::debug(
        "Successfully initialized Monocular_Inertial with keyframe {} and {}", first_keyframe->id_,
        second_keyframe->id_);
    }
    ...
    if (curr_frm_->camera_->setup_type_ == camera::setup_type_t::Monocular_Inertial) {
      if (!first_keyframe || !second_keyframe) {
        spdlog::error(
          "{}: After monocular initializing, first or second keyframes is nullptr", __func__);
        return;
      }
      ...
      imu_data_buf_->get_imu_data(
        first_keyframe->timestamp_, second_keyframe->timestamp_, imu_mearsurement_vec_);
      ...
      auto imu_preintegration_ptr = std::make_shared<data::IMU_Preintegration>(
        first_keyframe->timestamp_, second_keyframe->timestamp_, imu_mearsurement_vec_);
      spdlog::info(
        "Initializing keyframe inteval : {} s",
      ...
      if (keyfrms.size() != 1) {
        spdlog::error(
          "{}: After RGBD/Stereo initializing, there must be 1 keyframes in map_db_", __func__);
        return;
      }
    ...
    // update the camera pose of the last frame
    // because the mapping module might optimize the camera pose of the last frame's reference
    // keyframe
    update_last_frame();
    ...
    if (camera_->use_imu_) {
      ...
      imu_data_buf_->get_imu_data(
        last_keyframe_timestamp, cur_frame_timestamp, imu_mearsurement_vec_);
    ...
    spdlog::debug(
      "Started tracking! current frame id is {}, ref_keyfrm id is {}", curr_frm_->id_,
      ref_keyfrm_->id_);
    ...
    if (tracking_state_ == tracker_state_t::Tracking && new_keyframe_is_needed()) {
      // when tracking server map successfully, no local landmarks are tracked, so it is necessary
      // to track them; and meanwhile update local map
      insert_new_keyframe();
      imu_mearsurement_vec_.clear();
      ...
      data::IMU_Preintegration::imu_initialize_times_ > 0) {
      auto pre_info =
        std::any_cast<std::pair<std::shared_ptr<data::IMU_Preintegration>, Vec3_t>>(anydata);
  ...
  if (curr_frm_->cam_pose_cw_is_valid_) {
    last_cam_pose_from_ref_keyfrm_ =
      curr_frm_->cam_pose_cw_ * curr_frm_->ref_keyfrm_->get_cam_pose_inv();
  }
...
void univloc_tracking_module::remove_untracked_server_landmarks()
{
  ...
  for (auto & lm : tracked_server_landmarks_) {
    if (!lm.second->is_existed_in_tracking()) {
      erased_untracked_server_landmarks_id.push_back(lm.second->id_);
    }
  }

  for (auto id : erased_untracked_server_landmarks_id) {
    tracked_server_landmarks_.erase(id);
  }
}
...
{
  for (auto & lm : tracked_server_landmarks_) {
    lm.second->set_existed_in_tracking(false);
  }
}
...
bool univloc_tracking_module::optimize_current_frame_with_all_map()
{
  ...
  if (
    curr_frm_->id_ < last_reloc_frm_id_ + camera_->fps_ &&
    num_tracked_lms_ < 2 * num_tracked_lms_thr) {
    spdlog::debug(
      "server map tracking failed: {} matches < {}", num_tracked_lms_, 2 * num_tracked_lms_thr);
    tracked_server_landmarks_.clear();
    return false;
  }
  ...
  if (num_tracked_lms_ < num_tracked_lms_thr) {
    spdlog::debug(
      "server map tracking failed: {} matches < {}", num_tracked_lms_, num_tracked_lms_thr);
    tracked_server_landmarks_.clear();
    return false;
  }
...
void univloc_tracking_module::update_map_database()
{
  ...
  // threshold to erase server landmarks and keyframes currently set to 5 seconds and should keep
  // a slightly smaller value to reset is_previously_sent flag of landmarks in server side if
  // server only decides to send extra landmarks back to tracker
  ...
  std::vector<std::shared_ptr<data::landmark>> all_server_landmarks =
    map_db_->get_server_landmarks();
...
void univloc_tracking_module::update_local_keyframes()
{
  ...
  // count the number of sharing landmarks between the current frame and each of the neighbor
  // keyframes
  ...
  for (auto & keyfrm_weight : keyfrm_weights) {
    ...
    if (keyfrm == nullptr || keyfrm->will_be_erased()) {
      continue;
    }
...
bool univloc_tracking_module::reset_requested() const { return reset_requested_; }

void univloc_tracking_module::set_reset_requested(bool val) { reset_requested_ = val; }
...
std::any univloc_tracking_module::predict_current_pose_with_imu(data::frame & frame)
{
  ...
  if (n2 - n1) {
    spdlog::debug("localize new keyframe with IMU, got {} more imu data!", n2 - n1);
  }
  ...
  auto imu_preintegration_ptr = std::make_shared<data::IMU_Preintegration>(
    last_keyfrm_->timestamp_, frame.timestamp_, imu_mearsurement_vec_);
  ...
  Vec3_t pred_imu_pose =
    pred_wc.block(0, 0, 3, 3) * data::IMU_Preintegration::Tci_.block(0, 3, 3, 1) -
  ...
  data::frame & frame, std::pair<std::shared_ptr<data::IMU_Preintegration>, Vec3_t> predict_info)
{
  ...
  // generate keyframe base on current frame instead of creating new keyframe object directly,
  // try to reuse the implementation from keyfrm_inserter
  // since for RGBD and stereo cases, new landmarks will be created; reuse can help avoid
  // redundant code
  bool send_to_server =
    (tracker_mode_ == tracker_mode_t::Mapping || tracker_mode_ == tracker_mode_t::Remapping);
  ...
  if (new_keyframe) {
    keyfrm_inserter_.queue_keyframe(new_keyframe);
  }
...
void univloc_tracking_module::visualize_keypoints()
{
  ...
  while (!gui_queue_->closed()) {
    if (gui_queue_->wait_pop(curr_frame)) {
      ...
      std::vector<std::shared_ptr<data::landmark>> server_landmarks =
        map_db_->get_server_landmarks();
      ...
      // instead of cloning the frame twice and drawing on them and then combine them to a single
      // image (which is 2 more copies), combine the image into a single one right at start and
      // continue drawing directly on it
      // instead of 4, there are 2 frame copies
      // NOTE: Offset for the second image (server one) needs to be taken into consideration
      ...
      cv::putText(
        combine_img, " state : " + state, cv::Point(10, 10), CV_FONT_HERSHEY_COMPLEX, 0.4,
        cvScalar(0, 255, 0), 1);
      cv::putText(
        combine_img, "unmatched : " + std::to_string(unmatched_keypts.size()),
        cv::Point(10, curr_frame->image_.rows - 10), CV_FONT_HERSHEY_COMPLEX, 0.4,
        cvScalar(245, 143, 41), 1);
      cv::putText(
        combine_img, "     outlier : " + std::to_string(outlier_keypts.size()),
        cv::Point(10, curr_frame->image_.rows - 30), CV_FONT_HERSHEY_COMPLEX, 0.4,
        cvScalar(0, 255, 255), 1);
      cv::putText(
        combine_img, "       local : " + std::to_string(local_keypts.size()),
        cv::Point(10, curr_frame->image_.rows - 70), CV_FONT_HERSHEY_COMPLEX, 0.4,
        cvScalar(0, 255, 0), 1);
      cv::putText(
        combine_img, "     server : " + std::to_string(server_keypts.size()),
        cv::Point(10, curr_frame->image_.rows - 50), CV_FONT_HERSHEY_COMPLEX, 0.4,
        cvScalar(0, 0, 255), 1);
      ...
      cv::putText(
        ...
        cv::Point(curr_frame->image_.cols + 10, curr_frame->image_.rows - 10),
        CV_FONT_HERSHEY_COMPLEX, 0.4, cvScalar(0, 0, 255), 1);
      ...
      if (!camera_->use_imu_) {
        ...
        if (
          data::IMU_Preintegration::state_ ==
          data::IMU_Preintegration::IMU_State::Not_Initialized) {
      ...
      cv::putText(
        combine_img, imu_state, cv::Point(combine_img.cols * 0.5 - 150, 30),
        CV_FONT_HERSHEY_COMPLEX, 0.8, cvScalar(0, 255, 0), 0);
      ...
      if (tracker_mode_ == tracker_mode_t::Localization) {
        ...
        for (unsigned int idx = 0; idx < curr_frame->num_keypts_; ++idx) {
          ...
          Vec3_t cam_loc =
            K * (curr_frame->cam_pose_cw_.block(0, 0, 3, 3) * lm->get_pos_in_world() +
                 curr_frame->cam_pose_cw_.block(0, 3, 3, 1));
          float u = cam_loc[0] / cam_loc[2];
          float v = cam_loc[1] / cam_loc[2];

          cv::line(
            combine_img, cv::Point(kpt.pt.x, kpt.pt.y), cv::Point(u + curr_frame->image_.cols, v),
            cvScalar(245, 143, 41), 1);
        }
      }
