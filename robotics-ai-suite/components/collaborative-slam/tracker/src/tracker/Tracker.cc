...
void Tracker::feed_image(std::shared_ptr<ImageFrame> frame) { impl_->feed_image(frame); }

void Tracker::feed_lidar(std::shared_ptr<LidarFrame> scan) { impl_->feed_lidar(scan); }

std::shared_ptr<ImageFrame> Tracker::get_result() { return impl_->get_result(); }

size_t Tracker::how_many_frames_left() const { return impl_->how_many_frames_left(); }
...
void Tracker::feed_imu_data(
  double t, const Eigen::Vector3d & accel, const Eigen::Vector3d & angular_velocity)
{
  impl_->feed_imu_data(t, accel, angular_velocity);
}

void Tracker::get_keyframes(std::vector<Eigen::Matrix4d> & poses) const
{
  impl_->get_keyframes(poses);
}

void Tracker::get_keyframes(
...
void Tracker::get_landmarks(std::vector<Eigen::Vector3d> & positions) const
{
  impl_->get_landmarks(positions);
}
...
std::vector<std::pair<unsigned int, Eigen::Vector3d>>
Tracker::get_server_landmarks_clientID_position() const
{
...
void Tracker::get_imu_status(univloc_msgs::ImuStatus & msg) const
{
  return impl_->get_imu_status(msg);
}
...
bool Tracker::tracking_module_is_lost() const { return impl_->tracking_module_is_lost(); }

se::Octree<SE_FIELD_TYPE> & Tracker::get_octree() { return impl_->get_octree(); }

void Tracker::get_octree_block_list(
  std::vector<se::VoxelBlock<SE_FIELD_TYPE> *> & blocklist, bool active)
{
  return impl_->get_octree_block_list(blocklist, active);
}

unsigned int Tracker::get_tracker_octree_merged_times()
{
  return impl_->get_tracker_octree_merged_times();
}
