...
bool DataQueue<T>::close()
{
  ...
  // TODO: we shall wait here until all pop() calls have returned,
  // otherwise segfault may happen if *this is destructed before pop() returns
...
template <typename T>
bool DataQueue<T>::closed() const
{
  return queue_closed;
}

template <typename T>
size_t DataQueue<T>::max_length() const
{
