...
: cfg_(cfg),
  ...
  // the feature queue has a limited capacity to avoid extracting too many features for the tracking
  // module to process
  feature_queue_(1),
  ...
  reconstruction_queue_(1)
{
  ...
  if (cfg_->lidar_enable_) {
    feature_extraction_module_ = new feature_extraction_module(
      cfg_, camera_, bow_vocab_, request_queue_, &request_lidar_queue_, feature_queue_, rectifier_);
  } else {
    feature_extraction_module_ = new feature_extraction_module(
      cfg_, camera_, bow_vocab_, request_queue_, feature_queue_, rectifier_);
  }
  // tracking module
  tracker_ = new univloc_tracking_module(
    cfg_, camera_, cfg_->client_id_, map_db_, bow_db_, feature_queue_, result_queue_,
    reconstruction_queue_);
  // mapping module
  mapper_ = new univloc_mapping_module(
    map_db_, bow_db_, cfg_->is_monocular(), cfg_->use_odom_, cfg->clean_keyframe_);
  // fast mapping module
  if (cfg_->enable_fast_mapping_) {
    fast_mapping_module_ = new fast_mapping_module(cfg_, reconstruction_queue_);
  }
  // client module
  univloc_tracker::Client::get_instance().initialize(
    cfg_, map_db_, camera_, bow_vocab_, bow_db_, node_ptr);
  ...
  if (cfg_->mode_ == "localization" && !cfg_->octree_load_path_.empty()) {
    if (is_able_to_open(cfg_->octree_load_path_)) {
      ...
      fast_mapping::load_octree_map(
        cfg_->octree_load_path_, loaded_intrinsics, loaded_octree_cfg, loaded_octree);
    ...
    switch (camera_model_type) {
      case camera::model_type_t::Perspective: {
        camera_ = new camera::perspective(
          ...
          cfg_->dist_params_[1], cfg_->dist_params_[2], cfg_->dist_params_[3],
          cfg_->dist_params_[4], focal_x_baseline);
        break;
      }
      case camera::model_type_t::Fisheye: {
        camera_ = new camera::fisheye(
          cfg_->camera_name_, camera::base::load_setup_type(cfg_->setup_type_str_),
          camera::base::load_color_order(cfg_->color_order_str_), cfg_->width_, cfg_->height_,
          cfg_->camera_fps_, cfg_->fx_, cfg_->fy_, cfg_->cx_, cfg_->cy_, cfg_->dist_params_[0],
          cfg_->dist_params_[1], cfg_->dist_params_[2], cfg_->dist_params_[3], focal_x_baseline);
        break;
      }
...
void TrackerImpl::setup_rectifier()
{
  if (cfg_->rectifier_params_given_) {
    rectifier_ = std::make_shared<openvslam::util::stereo_rectifier>(
      camera_, cfg_->K_left_, cfg_->D_left_, cfg_->R_left_, cfg_->K_right_, cfg_->D_right_,
      cfg_->R_right_);
    return;
  }

  // for public datasets, the recitifier params are given, therefore no need to do extra work here
  // however, for self-recorded datasets, typically we need to obtain the recitifier params on
  // our own
  // below code tries to obtain the params but currently the performance is not guaranteed
  ...
  cv::Mat matR =
    (cv::Mat_<double>(3, 3) << R(0, 0), R(0, 1), R(0, 2), R(1, 0), R(1, 1), R(1, 2), R(2, 0),
     R(2, 1), R(2, 2));
  cv::Mat matt =
    (cv::Mat_<double>(3, 1) << cfg_->tf_left_right_(0, 3), cfg_->tf_left_right_(1, 3),
     cfg_->tf_left_right_(2, 3));
  ...
  if (camera_model_type == camera::model_type_t::Perspective) {
    cv::fisheye::stereoRectify(
      cfg_->K1, cfg_->D1, cfg_->K2, cfg_->D2, image_size, matR, matt, R1, R2, P1, P2, Q,
  ...
  rectifier_ = std::make_shared<openvslam::util::stereo_rectifier>(
    camera_, cfg_->K1, cfg_->D1, R1, cfg_->K2, cfg_->D2, R2);
}

void TrackerImpl::loaded_octree_map_to_msg(
  struct fast_mapping::se_cfg & octree_cfg, struct fast_mapping::camera_intrinsics & intrinsics,
  std::shared_ptr<se::Octree<SE_FIELD_TYPE>> octree)
{
  univloc_tracker::Client::get_instance().octree_param_msg_ =
    std::make_shared<univloc_msgs::OctreeParam>();
  ...
  // Octree map
  univloc_tracker::Client::get_instance().octree_map_msg_ =
    std::make_shared<univloc_msgs::Octree>();
...
void TrackerImpl::start(const bool need_initialize)
{
  ...
  feature_extraction_thread_ = std::make_unique<std::thread>(
    &openvslam::feature_extraction_module::run, feature_extraction_module_);
  tracking_thread_ =
    std::make_unique<std::thread>(&openvslam::univloc_tracking_module::run, tracker_);
  ...
  client_thread_ = std::make_unique<std::thread>(
    &univloc_tracker::Client::run, &univloc_tracker::Client::get_instance());
  if (cfg_->enable_fast_mapping_) {
    reconstruction_thread_ =
      std::make_unique<std::thread>(&fast_mapping::fast_mapping_module::run, fast_mapping_module_);
  }
  if (cfg_->gui_) {
    gui_thread_ = std::make_unique<std::thread>(
      &openvslam::univloc_tracking_module::visualize_keypoints, tracker_);
  }
}
...
void TrackerImpl::shutdown()
{
  ...
  if (traj_store_path_ != "") {
    ...
    while (true) {
      ...
      traj_file << std::setprecision(15) << keyframe_msg.md_timestamp << std::setprecision(9) << " "
                << pose(0, 3) << " " << pose(1, 3) << " " << pose(2, 3) << " " << q.x() << " "
                << q.y() << " " << q.z() << " " << q.w() << "\n";
    }
  }
  ...
  if (cfg_->enable_fast_mapping_) {
    ...
    if (!cfg_->octree_store_path_.empty()) {
      fast_mapping::save_octree_map(
        cfg_->octree_store_path_, fast_mapping_module_->get_camera_intrinsics(),
...
bool TrackerImpl::connected_to_server() const
{
...
#if false
  void TrackerImpl::save_frame_trajectory(
  ...
  void TrackerImpl::save_keyframe_trajectory(
    const std::string & path,
    const std::string & format) const
  {
    pause_other_threads();
  ...
  void TrackerImpl::enable_mapping_module()
  {
    ...
    if (!system_is_running_) {
      spdlog::critical(
        "please call TrackerImpl::enable_mapping_module() after TrackerImpl::startup()");
    }
  ...
  void TrackerImpl::disable_mapping_module()
  {
    ...
    if (!system_is_running_) {
      spdlog::critical(
        "please call TrackerImpl::disable_mapping_module() after TrackerImpl::startup()");
    }
...
size_t TrackerImpl::how_many_frames_left() const { return request_queue_.size(); }

std::shared_ptr<ImageFrame> TrackerImpl::get_result()
{
  std::shared_ptr<ImageFrame> ret;
  if (!result_queue_.wait_pop(ret)) {
    return nullptr;
  }
...
void TrackerImpl::feed_imu_data(
  double t, const Eigen::Vector3d & accel, const Eigen::Vector3d & angular_velocity)
{
...
void TrackerImpl::get_keyframes(
  std::vector<Eigen::Matrix4d> & covisibility_poses,
...
void TrackerImpl::get_imu_status(univloc_msgs::ImuStatus & msg) const
{
  ...
  msg.duration =
    static_cast<float>(openvslam::data::IMU_Preintegration::IMU_States_.preintegration_duration_);
  msg.vec_r[0] =
    static_cast<float>(openvslam::data::IMU_Preintegration::IMU_States_.preintegration_R_[0]);
  msg.vec_r[1] =
    static_cast<float>(openvslam::data::IMU_Preintegration::IMU_States_.preintegration_R_[1]);
  msg.vec_r[2] =
    static_cast<float>(openvslam::data::IMU_Preintegration::IMU_States_.preintegration_R_[2]);
  msg.vec_p[0] =
    static_cast<float>(openvslam::data::IMU_Preintegration::IMU_States_.preintegration_P_[0]);
  msg.vec_p[1] =
    static_cast<float>(openvslam::data::IMU_Preintegration::IMU_States_.preintegration_P_[1]);
  msg.vec_p[2] =
    static_cast<float>(openvslam::data::IMU_Preintegration::IMU_States_.preintegration_P_[2]);
  msg.vec_v[0] =
    static_cast<float>(openvslam::data::IMU_Preintegration::IMU_States_.preintegration_V_[0]);
  msg.vec_v[1] =
    static_cast<float>(openvslam::data::IMU_Preintegration::IMU_States_.preintegration_V_[1]);
  msg.vec_v[2] =
    static_cast<float>(openvslam::data::IMU_Preintegration::IMU_States_.preintegration_V_[2]);
...
void TrackerImpl::request_reset() { tracker_->set_reset_requested(true); }
...
void TrackerImpl::get_octree_block_list(
  std::vector<se::VoxelBlock<SE_FIELD_TYPE> *> & blocklist, bool active)
{
