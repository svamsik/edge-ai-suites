...
#ifndef FEATURE_EXTRACTION_MODULE_H
#define FEATURE_EXTRACTION_MODULE_H
...
protected:
  ...
  std::vector<std::tuple<cv::Point2d, cv::Point2d, double>> match_lidar_points(
    std::vector<std::pair<cv::Point2d, int>> & prev,
    std::vector<std::pair<cv::Point2d, int>> & curr);
  std::tuple<cv::Point2d, cv::Point2d, double> knearest_neighbor(
...
#endif  // FEATURE_EXTRACTION_MODULE_H
