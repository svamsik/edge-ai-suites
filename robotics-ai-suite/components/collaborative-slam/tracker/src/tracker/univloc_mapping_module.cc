...
univloc_mapping_module::univloc_mapping_module(
  data::map_database * map_db, data::bow_database * bow_db, const bool is_monocular, bool use_odom,
  const bool clean_keyframe)
...
univloc_mapping_module::~univloc_mapping_module()
{
  spdlog::debug("DESTRUCT: univloc_mapping_module");
}
...
void univloc_mapping_module::populate_sent_landmarks(std::vector<data::landmark *> & landmarks_out)
{
  ...
  for (auto local_keyfrm : curr_covisibilities) {
    ...
    // currently we use the same flag as localBA to indicate if the keyframe or landmark have
    // been considered
    // we can do so because the localBA and send_to_server() are in the same threads and in
    // both of them we clear such flag at the end
  ...
  spdlog::debug(
    "newly added {} landmarks from temporal window, total landmark size is {}", cnt,
    landmarks_out.size());
...
void univloc_mapping_module::send_to_server(unsigned int optimize_times)
{
  ...
  spdlog::debug("Send local map and local keyframes to server!");
  {
    ...
    univloc_tracker::Client::get_instance().queue_send_map(
      landmarks_out, keyframes_out, map_db_->get_erazed_redundant_landmarks_id(),
...
void univloc_mapping_module::update_imu_duration(double & time_after_initializing)
{
  ...
  if (data::IMU_Preintegration::imu_initialize_times_ > 0) {
    ...
    if (
      data::IMU_Preintegration::imu_initialize_times_ < imu_initialize_times_thre &&
...
unsigned int univloc_mapping_module::optimize_imu(double & time_after_initializing)
{
  ...
  if (data::IMU_Preintegration::imu_initialize_times_ == 0) {
    ...
    if (data::IMU_Preintegration::imu_initialize_times_ > 0) {
      time_after_initializing =
        cur_keyfrm_->timestamp_ - data::IMU_Preintegration::imu_initialize_timestamp_;
      spdlog::debug(
        "initial imu time duration after first initializing: {}", time_after_initializing);
    }
  ...
  // are experience values from ORB_SLAM3
    ...
    data::IMU_Preintegration::imu_initialize_times_ == 2 &&
    tracker_->tracking_state_ == tracker_state_t::Tracking) {
...
{
  spdlog::debug(
    "{}: start imu initialization, times: {}", __func__,
  ...
  auto keyframe_compare = [](const data::keyframe * a, const data::keyframe * b) {
    return a->id_ < b->id_;
  };
  auto max_keyfrm_iterator =
    std::max_element(keyfrms_to_initialize.begin(), keyfrms_to_initialize.end(), keyframe_compare);
  unsigned int max_index =
    std::abs(std::distance(keyfrms_to_initialize.begin(), max_keyfrm_iterator));
  ...
  if (data::IMU_Preintegration::imu_initialize_times_ == 0) {
    ...
    for (auto & keyfrm : keyfrms_to_initialize) {
      ...
      Vec3_t velocity =
        (keyfrm->get_imu_position() - keyfrm->pre_keyframe_->get_imu_position()) / delta_time;
  ...
  scale_ = 1.0;
  optimize::inertial_only_optimize(
    keyfrms_to_initialize, max_keyfrm_id, Rwi_, scale_, bias_acc_, bias_gyro_, is_monocular_,
    acc_weight, gyro_weight);

  // stage 4:
  // if scale isn't too small, then apply scale rotation for keyframes and landmarks
  // in map database
  if (scale_ < 0.1) {
  ...
  if (acc_weight > EPSILON) {
    optimize::visual_inertial_optimize(
      map_db_, max_keyfrm_id, 100, is_monocular_, true, acc_weight, gyro_weight);
  } else {
  ...
  spdlog::debug(
    "{}: finish imu initialization, times: {}", __func__,
    data::IMU_Preintegration::imu_initialize_times_);
  return;
}
...
{
  if (
    10 < map_db_->get_num_keyframes() && abort_local_BA_ == false &&
