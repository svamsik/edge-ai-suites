...
struct fmt::formatter<Eigen::Matrix<Scalar, Rows, Cols>>
{
  ...
  auto format(const Eigen::Matrix<Scalar, Rows, Cols> & m, FormatContext & ctx)
  {
    std::stringstream ss;
    ss << m;
...
: cfg_(cfg), reconstruction_queue_(reconstruction_queue)
{
  intrinsics_ = std::make_unique<camera_intrinsics>(
    cfg->width_, cfg->height_, cfg->cx_, cfg->cy_, cfg->fx_, cfg->fy_);
...
fast_mapping_module::~fast_mapping_module() { octree_->clear(); }
...
void fast_mapping_module::get_octree_block_list(
...
const struct se_cfg & fast_mapping_module::get_octree_cfg() const { return octree_cfg_; }

// check if the world points are exceeding the Octree boundary
Eigen::Vector3i fast_mapping_module::check_map_boundary(
  const std::vector<Eigen::Vector3f> & points_world) const
{
  ...
  for (const auto & point : points_world) {
    ...
    if (!within_map_boundary(octree_, voxel_scaled, x_dir, y_dir, z_dir)) {
      break;
    }
  }
...
bool fast_mapping_module::correct_octree_from_loop_closure(
...
{
  const double rotation_correction_min = 30.0 * M_PI / 180.0,
               rotation_correction_max = 330.0 * M_PI / 180;
  std::unique_lock<std::mutex> lock(this->octree_mutex_);
  const double inverse_voxel_size = octree_->size() / octree_->dim();
  lock.unlock();

  const double translation_correction =
    euclidian_distance(cam_pose_wc_after_correction, cam_pose_wc_before_correction);
  const int translation_correction_in_voxels = translation_correction * inverse_voxel_size;

  spdlog::debug(
    "Pose {} correction: {}m, voxels {}", pose_id, translation_correction,
    translation_correction_in_voxels);
  if (translation_correction_in_voxels >= octree_cfg_.correction_threshold) {
    return true;
  }
  ...
  if (
    (R >= rotation_correction_min && R <= rotation_correction_max) ||
...
void fast_mapping_module::update_local_octree_from_server(
  std::shared_ptr<se::Octree<SE_FIELD_TYPE>> global_octree)
{
...
void fast_mapping_module::run()
{
  ...
  while (true) {
    if (mapmsg_queue_.try_pop(map_msg)) {
      if (map_msg->is_request_octree) {
        ...
        if (cfg_->mode_ == "remapping") {
          spdlog::debug(
            "Set flag to stop integrate frame when it is outside remappin region in remapping "
            "mode!");
    ...
    if (!reconstruction_queue_.wait_pop(curr_frame)) {
      break;
    }
    ...
    // tracker's octree map has been replaced by merged octree from server.
    // because in this time interval octree's coordinate doesn't align with the local map's coordinate.
...
void fast_mapping_module::update_octree_from_server(
  std::vector<openvslam::data::keyframe *> keyfrms)
{
  ...
  for (const auto keyfrm : keyfrms) {
    ...
    if (correct_octree_from_loop_closure(
          cam_pose_wc_before_correction, cam_pose_wc_after_correction, pose_id)) {
  ...
  if (!do_octree_correction) {
    spdlog::info(
      "The coordinate changes of loop closure/map merging (except coordinate alignment) is too "
      "small"
  ...
  std::shared_ptr<se::Octree<SE_FIELD_TYPE>> corrected_octree =
  ...
  for (; node != nullptr; node = it.next()) {
    ...
    // are updated and sent to the tracker.
    // therefore the octree might consist of voxels coming from poses which are not present.
    // for most of the cases, we will keep those poses unchanged (identity matrix for transform);
    // for nearby frames of updated keyframes, we will find the closest transform to update 3D
    // location of voxels
    ...
    const Eigen::Vector3i pos_after_correction =
      (trans * pos.homogeneous()).block<3, 1>(0, 0).cast<int>();

    auto is_voxel_valid = [size](const Eigen::Vector3i & v) {
      return (v.x() >= 0) && (v.x() < size) && (v.y() >= 0) && (v.y() < size) && (v.z() >= 0) &&
             (v.z() < size);
    };

    if (!is_voxel_valid(pos_after_correction)) {
      continue;
    }
    ...
    if (node->isLeaf()) {
      ...
      auto voxel_block = corrected_octree->insert(
        pos_after_correction(0), pos_after_correction(1), pos_after_correction(2), pose_id);
      std::memcpy(
        voxel_block->getBlockRawPtr(), block->getBlockRawPtr(),
        max_blocks * sizeof(*block->getBlockRawPtr()));
    } else {
      auto n = corrected_octree->insert(
        pos_after_correction(0), pos_after_correction(1), pos_after_correction(2),
...
void fast_mapping_module::octree_integrate(std::shared_ptr<univloc_tracker::ImageFrame> frame)
{
  ...
  // update probabilities of each occupied Node/VoxelBlock
  ...
  struct bfusion_update funct(
    points.data(), computation_size, octree_cfg_.noise_factor, timestamp, octree_cfg_.logodds_lower,
    octree_cfg_.logodds_upper);
  lock.lock();
  se::functor::projective_map(
    *octree_, zmin, zmax, octree_pose.inverse(), K, computation_size, funct);
  lock.unlock();
...
void fast_mapping_module::set_coordinate_transform_matrix(
  ...
  const std::map<uint64_t, Eigen::Matrix4d> & transforms, const se::Node<SE_FIELD_TYPE> * node)
{
  ...
  // if the timestamps in transforms and the node's timestamp are within 0.5s, we think they are
  // reliable to update the node of the octree
  ...
  // all the timestamps in transforms are at leaset 0.5s smaller or bigger than the node's
  // timestamp
...
unsigned int fast_mapping_module::get_tracker_octree_merged_times()
{
  return tracker_octree_merged_times_;
}
