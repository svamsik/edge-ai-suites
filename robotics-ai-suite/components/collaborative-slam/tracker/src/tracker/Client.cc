...
void Client::initialize(
  ConfigConstPtr cfg, Map_database * p_map_db, Camera::base * camera,
  ...
  std::shared_ptr<rclcpp::Node> node_ptr)
{
  ...
  comm_client_ = node_ptr_->create_client<univloc_msgs::MapBasedComm>(
    "tracker2server", tracker2server_qos.get_rmw_qos_profile());
  comm_service_ = node_ptr_->create_service<univloc_msgs::MapBasedComm>(
    service_name,
    std::bind(
      &Client::service_message_callback, this, std::placeholders::_1, std::placeholders::_2));
...
univloc_msgs::MapConstPtr Client::get_relocalization_results()
{
  ...
  if (!relocalization_results_.empty()) {
    ...
    spdlog::debug(
      "keyframe size {}, first id: {}", kf_size, relocalization_result->keyframes[0].mn_id);
...
void Client::update_local_map_from_server(univloc_msgs::MapConstPtr map_msg_ptr)
{
  ...
  if (!map_msg_ptr->keyframe_updates.empty()) {
    if (map_msg_ptr->keyframe_updates.size() > 1) {
      spdlog::warn(
        "There should be only one msg for coordinate transform, please check server "
        "implementation!");
    } else {
  ...
  // during map merge, for the map to be aligned, Tww_aligned_ref is not empty
  // therefore, when tracker's local map is about to transform coordinate, will skip processing
  // frames in fast mapping
  if (p_fast_mapping_module_ && Tww_aligned_ref) {
    p_fast_mapping_module_->set_skip_processing_frame(true);
  }
  ...
  if (map_msg_ptr->keyframes.size() > 0) {
    ...
    for (uint i = 0; i < kf_size; ++i) {
      ...
      spdlog::debug(
        "Keyframe {} Position Before Replacing: {} {} {}", p_keyframe->id_,
      ...
      if (openvslam::data::IMU_Preintegration::imu_initialize_times_ > 0) {
        Eigen::Matrix3d rot_correct =
          p_keyframe->get_rotation().transpose() *
      ...
      spdlog::debug(
        "Position After Replacing: {} {} {}", p_keyframe->get_cam_pose_inv()(0, 3),
  ...
  for (uint idx = 0; idx < mp_size; ++idx) {
    ...
    if (p_landmark) {
      ...
      if (
        landmark_msg.mn_replace_other_id <= 0 &&
        landmark_msg.mn_observed_in_other_keyframe_id <= 0) {
        continue;
      }
  ...
  // propagate to update other keyframe pose
  ...
  if (max_keyframe_id != 0) {
    ...
    for (auto p_keyframe : keyframes) {
      if (p_keyframe->is_update_from_server()) {
        continue;
      }
      auto pose_now = p_keyframe->get_cam_pose();
      p_keyframe->cam_pose_wc_before_replacing_ = p_keyframe->get_cam_pose_inv();
      p_keyframe->set_cam_pose(
        pose_now * newest_update_frame->cam_pose_wc_before_replacing_ *
      ...
      spdlog::debug(
        "Keyframe {} Position Before Aligning: {} {} {}", p_keyframe->id_,
        p_keyframe->cam_pose_wc_before_replacing_(0, 3),
        p_keyframe->cam_pose_wc_before_replacing_(1, 3),
        p_keyframe->cam_pose_wc_before_replacing_(2, 3));
      spdlog::debug(
        "Position After Aligning: {} {} {}", p_keyframe->get_cam_pose_inv()(0, 3),
        p_keyframe->get_cam_pose_inv()(1, 3), p_keyframe->get_cam_pose_inv()(2, 3));
    ...
    for (auto & lm : landmarks) {
      ...
      if (!lm->is_update_from_server()) {
        ...
        if (ref_keyframe) {
          ...
          auto pos_in_ref =
            (T_cw.block(0, 0, 3, 3) * lm->get_pos_in_world() + T_cw.block(0, 3, 3, 1))
              .homogeneous();
  ...
  for (auto & item : updated_keyframes) {
    if (item) {
      item->set_update_from_server(false);
    }
  }
  ...
  // will lead to a diverged occupancy map
  // a better way would be to split into smallest voxels and do correction
  ...
  if (!map_msg_ptr->loop_closure) {
    dynamic_cast<openvslam::univloc_tracking_module *>(p_tracking_module_)
      ->set_coordinate_aligned(true);
  }
}

void Client::set_latest_keyfrm_local_id(KeyframeID id) { latest_keyfrm_localization_id_ = id; }
...
void Client::process_received_server_landmarks(KeyframeID id)
{
  ...
  spdlog::debug(
    "Requested ID {} : Returned ID {} : last returned ID {}", id, start_id,
    latest_keyfrm_localization_id_);
}
...
void Client::update_server_landmarks(univloc_msgs::MapConstPtr map_msg_ptr)
{
  ...
  for (uint idx = 0; idx < lms_size; ++idx) {
    ...
    if (p_map_db_->get_landmark(landmark_msg.mn_id)) {
      continue;
    }

    std::shared_ptr<Landmark> ptr_server_landmark =
      p_map_db_->get_server_landmark(landmark_msg.mn_id);
  ...
  if (p_map_db_->get_num_keyframes() > 0) {
    create_virtual_server_keyframe();
  }
...
void Client::create_virtual_server_keyframe()
{
  ...
  for (auto lm : server_landmarks) {
    ...
    // it should not judge whether in front of camera, because the pose is not accurate enough
  ...
  Keyframe * server_virtual_keyframe = new Keyframe(
    0, 0, newest_keyframe->timestamp_, camera_pose, newest_keyframe->camera_,
    newest_keyframe->depth_thr_, num_keypts, keypts, undist_keypts, bearings, stereo_x_right,
    depths, descriptors, newest_keyframe->num_scale_levels_, newest_keyframe->scale_factor_,
    bow_vocab_, bow_db_, p_map_db_);
...
void Client::process_service_message(univloc_msgs::MapConstPtr map_msg_ptr)
{
  ...
  spdlog::debug(
    "map_msg_ptr->session_start_time: {},session_start_time_ : {} ",
    map_msg_ptr->session_start_time, session_start_time_);
  if (
    std::abs(map_msg_ptr->session_start_time - session_start_time_) > 0.00001 &&
...
void Client::convert_erased_id_to_msg(
  std::vector<KeyframeID> & keyframes, std::vector<LandmarkID> & landmarks,
  univloc_msgs::Map & msg_out)
{
  uint erased_kf_size = keyframes.size(), erased_ld_size = landmarks.size();
  spdlog::debug(
    "erased keyframe size: {}; erased landmark size: {}", erased_kf_size, erased_ld_size);
...
void Client::convert_landmarks_to_msg(
  const std::vector<Landmark *> & landmarks_out, univloc_msgs::Map & msg_out)
{
  auto start = now();
  ...
  for (uint i = 0; i < size; i++) {
    ...
    if (p_cur_landmark->being_sliding_out_ || p_cur_landmark->will_be_erased()) {
      continue;
    }
    ...
    for (std::map<Keyframe *, unsigned int>::const_iterator mit = observations.begin();
...
void Client::convert_keyframes_to_msg(
  const std::vector<Keyframe *> & keyframes_out, univloc_msgs::Map & msg_out)
{
  auto start = now();
  ...
  for (uint i = 0; i < size; i++) {
    ...
    if (cur_keyframe->being_sliding_out_) {
      continue;
    }
    ...
    if (!cur_keyframe->is_new_created()) {
      if (cur_keyframe->graph_node_->get_spanning_parent()) {
        keyframe_update_msg.mn_parent_id = cur_keyframe->graph_node_->get_spanning_parent()->id_;
      }
    ...
    for (int idx = 0; idx < descriptors_num; ++idx) {
      ...
      if (descriptors_cols != MsgDesc.m_descriptor.size()) {
        spdlog::warn(
          "descriptor {} of keyframe {} has wrong size {}", idx, cur_keyframe->id_,
          descriptors_cols);
        continue;
      }
...
void Client::queue_send_map(
  const std::vector<Landmark *> & landmarks, const std::vector<Keyframe *> & keyframes,
...
void Client::get_server_landmarks_using_current_pose(
  Eigen::Matrix4d curr_pose, KeyframeID curr_id, unsigned int relocal_map_id)
{
...
void Client::run()
{
  ...
  while (running_) {
    ...
    while (!keyframes_to_be_sent_.empty()) {
      ...
      convert_erased_id_to_msg(
        sent_erased_keyframes_id, sent_erased_landmarks_id, global_optimization_msg);
...
void Client::merge_to_be_sent_map(
  std::vector<Keyframe *> & sent_keyframes, std::vector<Landmark *> & sent_landmarks,
  ...
  std::vector<KeyframeID> & sent_erased_keyframes_id)
{
  ...
  if (keyframes_to_be_sent_.size() == 1) {
    ...
    while (!keyframes_to_be_sent_.empty()) {
      ...
      for (auto & item : kfs) {
        if (!keyframes_set.count(item)) {
          keyframes_set.insert(item);
        }
      }
      for (auto & item : lms) {
        if (!landmarks_set.count(item)) {
          landmarks_set.insert(item);
        }
      }
      ...
      sent_erased_landmarks_id.insert(
        sent_erased_landmarks_id.end(), erase_mp_ids.begin(), erase_mp_ids.end());

      sent_erased_keyframes_id.reserve(sent_erased_keyframes_id.size() + erase_kf_ids.size());
      sent_erased_keyframes_id.insert(
        sent_erased_keyframes_id.end(), erase_kf_ids.begin(), erase_kf_ids.end());
    ...
    for (auto & item : keyframes_set) {
      sent_keyframes.push_back(item);
    }

    sent_landmarks.reserve(landmarks_set.size());
    for (auto & item : landmarks_set) {
      sent_landmarks.push_back(item);
    }
  }
}
...
{
  std::unique_lock<std::mutex> lock(sent_map_queue_mtx_);
  while (!landmarks_to_be_sent_.empty()) {
    landmarks_to_be_sent_.pop();
  }
  while (!keyframes_to_be_sent_.empty()) {
    keyframes_to_be_sent_.pop();
  }
  while (!erased_landmarks_id_.empty()) {
    erased_landmarks_id_.pop();
  }
  while (!erased_keyframes_id_.empty()) {
    erased_keyframes_id_.pop();
  }
}

void Client::set_tracking_module(Tracking_module * p_tracking_module)
{
  p_tracking_module_ = p_tracking_module;
}

void Client::set_mapping_module(Mapping_module * p_mapping_module)
{
  p_mapping_module_ = p_mapping_module;
}
...
void Client::service_message_callback(
