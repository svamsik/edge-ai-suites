...
#pragma once
/*
  A universal client function for multiple robot system. The function is as below:
  1. getting local landmarks(map) and local keyframe from the slam system , convert them to ROS message and send it to
server
  2. receiving updated message(local landmarks and keyframes) from and server, and then updating slam state
...
public:
  ...
  void convert_landmarks_to_msg(
    const std::vector<Landmark *> & lmbuffer, univloc_msgs::Map & msg_out);
  void convert_keyframes_to_msg(
    const std::vector<Keyframe *> & kfbuffer, univloc_msgs::Map & msg_out);

  void convert_erased_id_to_msg(
  ...
  void get_server_landmarks_using_current_pose(
