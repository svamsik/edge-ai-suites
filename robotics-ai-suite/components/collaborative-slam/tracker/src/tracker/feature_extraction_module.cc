...
feature_extraction_module::feature_extraction_module(
  std::shared_ptr<univloc_tracker::Config> cfg, camera::base * camera,
  data::bow_vocabulary * bow_vocab, RequestQueue & request_queue, FeatureQueue & feature_queue,
...
feature_extraction_module::feature_extraction_module(
  ...
  RequestLidarQueue * ptr_request_lidar_queue, FeatureQueue & feature_queue,
  std::shared_ptr<openvslam::util::stereo_rectifier> rectifier)
: cfg_(cfg),
  camera_(camera),
  bow_vocab_(bow_vocab),
...
{
  // Create orb_params for non-GPU case
#if !defined(GPU_KERNEL_PATH)
  orb_params orb_params_obj(
    cfg->max_num_keypoints_, cfg->orb_scale_factor_, cfg->orb_num_levels_,
  ...
  if (
...
#if defined(GPU_KERNEL_PATH)
    extractor_left_ = new orb_extractor(
      cfg->max_num_keypoints_, cfg->orb_scale_factor_, cfg->orb_num_levels_,
      cfg->orb_ini_fast_threshold_, cfg->orb_min_fast_threshold_, 2, cfg->mask_rects_);
    extractor_left_->set_gpu_kernel_path(GPU_KERNEL_PATH);

    extractor_right_ = new orb_extractor(
      cfg->max_num_keypoints_, cfg->orb_scale_factor_, cfg->orb_num_levels_,
      cfg->orb_ini_fast_threshold_, cfg->orb_min_fast_threshold_, 2, cfg->mask_rects_);
    extractor_right_->set_gpu_kernel_path(GPU_KERNEL_PATH);
#else
    // Jazzy: ORB-SLAM3 with orb_params
    extractor_left_ = new orb_extractor(orb_params_obj);
    extractor_right_ = new orb_extractor(orb_params_obj);
#endif
  } else if (
    ...
    extractor_left_ = new orb_extractor(
      cfg->max_num_keypoints_, cfg->orb_scale_factor_, cfg->orb_num_levels_,
      cfg->orb_ini_fast_threshold_, cfg->orb_min_fast_threshold_, 1, cfg->mask_rects_);
    extractor_left_->set_gpu_kernel_path(GPU_KERNEL_PATH);
#else
    // Jazzy: ORB-SLAM3 with orb_params
    extractor_left_ = new orb_extractor(orb_params_obj);
    ...
    throw std::runtime_error(
      "error camera type, only support 1: Monocular; 2: Stereo; 3: RGBD; 4: RGBD_Inertial; "
      "5: Stereo_Inertial; 6: Monocular_Inertial");
  }

  // TODO: handle exception of file non-existence
  if (!cfg_->mask_image_path_.empty()) {
    mask_ = cv::imread(cfg_->mask_image_path_, cv::IMREAD_GRAYSCALE);
  }
}
...
void feature_extraction_module::run()
{
  ...
  while (true) {
    ...
    if (!request_queue_.wait_pop(request)) {
      break;
    }
    ...
    if (cfg_->is_stereo() && cfg_->enable_rectifier_) {
      ...
      // request->image2 are not used for other inputs later
      // therefore, no need to save a copy, use shallow copy to speed up
      img1 = request->image1;
      img2 = request->image2;
    }
    ...
    if (cfg_->is_monocular()) {
      if (!tracking_module_) {
        throw std::runtime_error(
          "feature_extraction_module doesn't have the pointer to tracking_module!");
      }
      if (tracking_module_->is_initializing()) {
        timer_.startNextProc("construct mono frame with initialization config");
        extractor_left_->set_max_num_keypoints(cfg_->max_num_keypoints_ * 2);
        frame = std::make_shared<data::frame>(
          timestamp, extractor_left_, bow_vocab_, &*camera_, cfg_->depth_threshold_);
      ...
      frame = std::make_shared<data::frame>(
        timestamp, extractor_left_, extractor_right_, bow_vocab_, &*camera_,
      ...
      frame = std::make_shared<data::frame>(
        timestamp, extractor_left_, bow_vocab_, &*camera_, cfg_->depth_threshold_, request->image1);
      frame->extract_rgbd_features(img1, img2, mask_);
    }

    // in previous operations, img1/2 and request->image1/2 may already differ, for example,
    // in functions like convert_to_grayscale and convert_to_true_depth, new memory will be
    // allocated if the data types between input and output cv::mat are different

    // in subsequent operations of the current code base, only request->image2 will be used
    // in fast mapping module as depth image input, while request->image1 is not used anywhere
    // therefore, to avoid duplicated processing, we will perform post-processing to make sure that
    // img1/2 and request->image1/2 are the same

    // as the result, for almost all the data after feature extraction module (i.e.,
    // inside frame class, feature_queue_, result_queue_, reconstruction_queue_),
    // color images are grayscale and the data type of depth images are float with meter as the
    // unit, except in case of segmentation, raw color image will be used as input for frame
    // constructor and later saved as segment_rgb_ using deep copy
    request->image1 = img1;
    request->image2 = img2;

    if (cfg_->lidar_enable_ && frame != nullptr) {
      process_lidar_frame(frame, timestamp);
    }
    std::pair<std::shared_ptr<ImageFrame>, std::shared_ptr<data::frame>> pair =
      std::make_pair(request, frame);
    if (!feature_queue_.wait_push(pair)) {
      break;
    }
    timer_.endCycle();
  }
...
void feature_extraction_module::process_lidar_frame(
  std::shared_ptr<data::frame> frame, double & timestamp)
{
  ...
  if (request_lidar) {
    ...
    if (std::abs(delta_t_ms) > sync_th) {
      ...
      frame->matched_feature_points_ =
        match_lidar_points(prev_extracted_lidar_points_, extracted_lidar_points_);
...
void feature_extraction_module::extract_lidar_feature(const std::vector<cv::Point2d> & points)
{
  ...
  for (unsigned int i = 0; i < sorted_c.size(); i++) {
    ...
    while (it != sorted_c[i].end() && it_r != sorted_c[i].rend()) {
      ...
      if (
        it->first < C_TH_MIN &&
        !is_close_to_boundary(
          points[it->second], points[(it->second) - 1], points[(it->second) + 1]) &&
      ...
      if (
        it_r->first > C_TH_MAX &&
        !is_close_to_boundary(
          points[it_r->second], points[(it_r->second) - 1], points[(it_r->second) + 1]) &&
        !is_point_too_close(points[it_r->second])) {
        extracted_lidar_points_.push_back({points[it_r->second], lidar_frame_id_});
        count++;
      }
      if (count == MAX_LIDAR_FEATURE / 4) {
        break;  // to make sure we get points from each quadrants
      }
      it++;
      it_r++;
    }
...
void feature_extraction_module::calculate_curvature(
  const std::vector<cv::Point2d> & points, std::vector<std::set<std::pair<float, int>>> & sorted_c)
{
  ...
  for (unsigned int i = 5; i < points.size() - 5; i++) {
    cv::Point2d diffRange = points[i - 5] + points[i - 4] + points[i - 3] + points[i - 2] +
                            points[i - 1] + points[i] * 10 + points[i + 1] + points[i + 2] +
    ...
    if (!std::isinf(diff)) {
      ...
      if (
        i > (points.size() / 2) + 5 && i < ((points.size() / 4 + points.size() / 2)) - 5 &&
        !std::isinf(diff)) {
        quad_iii.insert({diff, i});
      }

      if (
        i > (points.size() / 4 + points.size() / 2) + 5 && i < (points.size()) - 5 &&
        !std::isinf(diff)) {
...
bool feature_extraction_module::is_close_to_boundary(
  const cv::Point2d & p, const cv::Point2d & pm1, const cv::Point2d & pp1)
{
  double dx = pp1.x - p.x;
  double dy = pp1.y - p.y;
...
std::vector<std::tuple<cv::Point2d, cv::Point2d, double>>
feature_extraction_module::match_lidar_points(
