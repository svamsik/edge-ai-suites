...
topics = [
    ...
    '/d455_rear/aligned_depth_to_color/image_raw',
    '/d455_rear/color/image_raw',
]
...
def reset_stamp_keep_time(original_msg):
    ...
    if hasattr(msg, 'header') and hasattr(msg.header, 'stamp'):
        if not use_sim_time:
            ...
            new_time = Time.from_msg(start_time).nanoseconds + (
                Time.from_msg(msg.header.stamp).nanoseconds
                - Time.from_msg(reset_stamp_keep_time.start_record_time).nanoseconds
            )
            msg.header.stamp = builtin_interfaces.msg.Time(
                sec=new_time // CONVERSION_CONSTANT, nanosec=new_time % CONVERSION_CONSTANT
            )
