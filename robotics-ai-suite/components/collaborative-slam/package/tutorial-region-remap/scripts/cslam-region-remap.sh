...
ros2 launch univloc_tracker collab_slam_nav.launch.py slam_mode:=remapping server_mode:=remapping queue_size:=0 publish_tf:=false camera:=camera1 baselink_frame:=base_link1 image_frame:=camera1_color_optical_frame use_odom:=true odom_frame:=odom1 odom_tf_query_timeout:=50.0 tracker_rviz:=true server_rviz:=true enable_fast_mapping:=true octree_load_path:=$TMP_DIR/octree.bin octree_store_path:=$TMP_DIR/octree_updated.bin load_map_path:=$TMP_DIR/map.msg save_map_path:=$TMP_DIR/map_updated.msg server_remapping_region:="[-1.0, -0.5, -1.0, 0.5, 5.0, 0.5, 5.0, -0.5]" &
pid1=$!
