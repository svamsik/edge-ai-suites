...
class TrajectoryBenchmark:
    ...
    def result_isavailable(self):
        ...
        with open(os.path.join(self.resultfile_dir, self.__resultfile)) as resultfile:
    ...
    def run_single_test(self):
        ...
        launch_params = ['rviz:=false', 'fix_scale:=true']
        ...
        launch_params = ['gui:=false', 'rviz:=false', 'publish_tf:=false', 'log_level:=warning']
...
def args_parse(default_bagfile_dir):
    parser = argparse.ArgumentParser(
        description='Automatically run TUM dataset with RGBD input and print the accuracy data to standard output'  # noqa: E501
    )
    parser.add_argument(
        'dataset',
        type=str,
        help='The dataset to be tested, option: tum',
        choices=['tum'],
        default='./',
    )
    parser.add_argument(
    ...
    if not os.path.isdir(bagfile_dir):
        sys.exit('"{}" is not a directory'.format(bagfile_dir))

    resultfile_dir = os.path.realpath(args.resultfile_dir)
    if not os.path.isdir(resultfile_dir):
        print('Create {} as the directory of the result file'.format(resultfile_dir))
    ...
    if os.path.isdir(args.groundtruth_dir):
        ...
        print(
            'Since the directory of groundtruth files is not a directory, will use the directory of bag files instead'  # noqa: E501
        )
    ...
    for name in bagfile_dir_list:
        ...
        if len(name_suffix_list) == 2 and name_suffix_list[1] == 'bag':
            ...
            if name_suffix_list[0] not in bagfile_prefix_list:
                ...
                index = bagfile_prefix_list.index(name_suffix_list[0])
                bagfile_list[index] = name
        else:
            continue
...
# Groundtruth name format: 1. TUM name-groundtruth.txt
...
def main():
    ...
    for item in benchmark_list:
        ...
        for test_loop in range(args_dict['REPEAT']):
            ...
            if status != 0:
                ret = -1
                print(
                    'The result evaluation of test "{}" is failed'.format(item.name),
                    file=sys.stderr,
                )
                continue
    ...
    print('{0}\nThe evaluaton for each test is listed below\n{0}'.format(separator))
