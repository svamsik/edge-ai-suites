...
class StereoBenchmark:
    __evo_ape_cmd = ['evo_ape', 'tum', '-va']
    __evo_rpe_cmd = ['evo_rpe', 'tum', '-va']
    ...
    def run_single_test(self):
        ...
        if self.dataset == 'euroc':
            launch_file_cmd = ['ros2', 'launch', 'univloc_tracker', 'euroc_stereo.launch.py']
...
def args_parse(default_bagfile_dir):
    parser = argparse.ArgumentParser(
        description='Automatically run EuRoC dataset with stereo input and print the accuracy data to standard output'  # noqa: 501
    )
    parser.add_argument(
        'dataset',
        type=str,
        help='The dataset to be tested, supported options: euroc and kitti',
        choices=['euroc', 'kitti'],
    )
    parser.add_argument(
        '-b',
    ...
    parser.add_argument(
        ...
        type=str,
        dest='bagfile_list',
        help='The list specifies which dataset need to be tested',
        default=default_bagfile_dir,
    )
    parser.add_argument(
        '-f',
    ...
    parser.add_argument(
        '--output', action='store_true', help='Output accuracy data for each test loop.'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--from_scratch',
        action='store_true',
        help='from_scratch will remove the old reult.xlsx and test all the benchmark from scratch, otherwise the result will be added to the current result.xlsx',  # noqa: 501
    )
    parser.add_argument(
        '--retest_failures_once',
        action='store_true',
        help='only test the dataset in the filtered sheet of result.xlsx, this should be used together with --save_to_excel',  # noqa: 501
    )
...
def filter_bagfile_list(bagfile_list, bagfile_prefix_list, listfile):
    if not os.path.exists(listfile):
        sys.exit(
            '"{}" not found, run benchmark with --save_to_excel to get a result.xlsx file before filter results'.format(  # noqa: 501
                listfile
            )
        )
    ...
    for bag, prex in zip(bagfile_list, bagfile_prefix_list):
        if ['stereo', prex] in filtered_list:
...
def generate_groundtruth_list_euroc(groundtruth_dir, bagfile_prefix_list):
...
def main():
    ...
    if args_dict['DATASET'] == 'euroc':
        groundtruth_list = generate_groundtruth_list_euroc(
    ...
    for item in benchmark_list:
        ...
        for test_loop in range(args_dict['REPEAT']):
            ...
            if status != 0:
                ret = -1
                print(
                    'Test "{}" failed due to timeout or some other reason, jump to next case'.format(  # noqa: 501
                        item.name
                    ),
                    file=sys.stderr,
                )
