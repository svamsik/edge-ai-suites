...
class MonocularBenchmark:
    ...
    def __init__(self, name, bagfile, groundtruth, ros1_path, dataset, wait_time, resultfile_dir):
        ...
        self.evaluate_ape = True
        self.sequence = 0  # so far used only for kitti
        self.traj_coverage = 0
        self.gt_coverage = 0
        if self.dataset == 'kitti':
            self.__get_sequence_number()
    ...
    def __get_sequence_number(self):
        sequence_string = self.bagfile.split('.bag')
        seq = sequence_string[0]
        self.sequence = int(seq[len(seq) - 2:])  # fmt: skip

    def __generate_fr_param(self):
        fr_param = ''
        if len(self.name.split('freiburg')) == 2:
            if self.name.split('freiburg')[1][0] == '1':
                fr_param = 'fr1:=true'
            elif self.name.split('freiburg')[1][0] == '2':
                fr_param = 'fr2:=true'
            elif self.name.split('freiburg')[1][0] == '3':
                fr_param = 'fr3:=true'

        # print("Using intrinsic camera parameters {}".format(fr_param))
        return fr_param
    ...
    def result_isavailable(self):
        ...
        # The time period of generating keyframe
        # must be longer than the 90% of the total time period
        # The time of generating the last keyframe
        # must be within 1 seconds till the end of the total time period
        ...
        # if (result_duration / refer_duration) > 0.9 and (
        #     refer_end_timestamp - result_end_timestamp
        # ) < 1:
        self.traj_coverage = '{:.2f}'.format(result_duration)
        self.gt_coverage = '{:.2f}'.format(refer_duration)
        print(
            'trajectory coverage: {:.2f}s; groundtruth coverage:{:.2f}s'.format(
                result_duration, refer_duration
            )
        )
        ...
        #    return 0
        # else:
        #    print("Produced trajectory duration did not cover at least 50% of reference trajectory!")  # noqa: E501
        #    return -2
        return 0
    ...
    def evaluate_result(self, is_ape=True):
        ...
        if is_ape:
            evaluate_cmd = self.__evo_ape_cmd + [
                self.referfile,
                os.path.join(self.resultfile_dir, self.resultfile),
            ]
        else:
            evaluate_cmd = self.__evo_rpe_cmd + [
                self.referfile,
                os.path.join(self.resultfile_dir, self.resultfile),
            ]
    ...
    def average_evaluation(self, is_ape=True):
        if is_ape:
            if len(self.ape_rmse_list) == 0:
                ...
                average_rmse = (
                    sum([item**2 for item in self.ape_rmse_list]) / len(self.ape_rmse_list)
                ) ** 0.5
        else:
            if len(self.rpe_rmse_list) == 0:
                ...
                average_rmse = (
                    sum([item**2 for item in self.rpe_rmse_list]) / len(self.rpe_rmse_list)
                ) ** 0.5
    ...
    def store_trajectory_file(self):
        ...
        dst_file = os.path.join(
            self.resultfile_dir, ('result_' + self.name + '_' + str(dst_file_index) + '.txt')
        )
    ...
    def run_single_test(self):
        ...
        launch_params = ['fix_scale:=false', 'rviz:=false']
        ...
        if self.dataset == 'euroc':
            ...
            launch_file_cmd = ['ros2', 'launch', 'univloc_tracker', 'tum_rgbd.launch.py']
        elif self.dataset == 'kitti':
            if self.sequence < 3:
                seq = 's00_02'
            elif self.sequence == 3:
                seq = 's03'
            elif self.sequence > 3 and self.sequence < 11:
                seq = 's04_12'
            else:
                raise RuntimeError('Only sequences [0, 10] are supported for KITTI dataset!')
            seq_cmd = seq + ':=true'
            launch_file_cmd = [
                'ros2',
                'launch',
                'univloc_tracker',
                'kitti_mono.launch.py',
                seq_cmd,
            ]
        launch_params = [
        ...
        wait_to_shutdown = TimerAction(
            period=self.wait_time, actions=[EmitEvent(event=Shutdown())]
        )
        ...
        if self.dataset == 'tum':
            fr_param = self.__generate_fr_param()
            if fr_param == '':
                return -1
            else:
                tracker_cmd.append(fr_param)
        ...
        print(
            'Done playing bag file! Waiting for {}secs to start the next process...'.format(
                self.wait_time
            )
        )
...
def args_parse(default_bagfile_dir):
    parser = argparse.ArgumentParser(
        description='Automatically run TUM/EuRoC dataset with monocular input and print the accuracy data to standard output'  # noqa: E501
    )
    parser.add_argument(
        'dataset',
        type=str,
        help='The dataset to be tested, option: tum/euroc/kitti',
        choices=['tum', 'euroc', 'kitti'],
    )
    parser.add_argument(
        '-b',
        '--bagfile_dir',
        type=str,
        dest='bagfile_dir',
        help='The directory which stores the .bag file',
    ...
    parser.add_argument(
        ...
        type=str,
        dest='resultfile_dir',
        help='The directory of the result file which stores the trajectory data',
        default='./',
    )
    parser.add_argument(
        '-r',
        ...
        type=int,
        dest='repeat_num',
        help='The number to repeat for each test',
        default=1,
    )
    parser.add_argument(
        '-g',
        ...
        help='The directory of the groudtruth files',
        default='',
    )
    parser.add_argument(
        '--rmse',
        type=float,
        dest='rmse_threshold',
        help='The threshold of evaluated RMSE to pass a single test',
        default=-1.0,
    )
    parser.add_argument(
        '-w',
        ...
        type=float,
        dest='wait',
        help='The delay to exit nodes after playback of .bag file',
        default=5.0,
    )
    parser.add_argument(
        '-p',
        ...
        type=str,
        dest='ros1_path',
        help='The installation path of the ROS1',
        default='/opt/ros/noetic',
    )
    parser.add_argument(
        '--store', action='store_true', help='Store tracjectory files for each test loop.'
    )
    parser.add_argument(
        '--evaluate_rpe', action='store_true', help='Evalute rpe for trajectory output.'
    )
    parser.add_argument(
        '--save_to_excel',
        action='store_true',
    ...
    parser.add_argument(
        '--from_scratch',
        action='store_true',
        help='from_scratch will remove the old reult.xlsx and test all the benchmark from scratch, otherwise the result will be added to the current result.xlsx',  # noqa: E501
    )
    parser.add_argument(
        '--retest_failures_once',
        action='store_true',
        help='only test the dataset in the filtered sheet of result.xlsx, this should be used together with --save_to_excel',  # noqa: E501
    )
    ...
    if os.path.isdir(args.groundtruth_dir):
        ...
        print(
            'Since the directory of groundtruth is not a directory, will use the directory of .bag files instead'  # noqa: E501
        )
...
def generate_bagfile_list(bagfile_dir):
    ...
    for name in bagfile_dir_list:
        ...
        if len(name_suffix_list) == 2 and name_suffix_list[1] == 'bag':
            ...
            len(name_suffix_list) == 3
            and name_suffix_list[1] == 'orig'
            and name_suffix_list[2] == 'bag'
        ):
            if name_suffix_list[0] not in bagfile_prefix_list:
                bagfile_list.append(name)
                bagfile_prefix_list.append(name_suffix_list[0])
...
def filter_bagfile_list(bagfile_list, bagfile_prefix_list, listfile):
    if not os.path.exists(listfile):
        sys.exit(
            '"{}" not found, run benchmark with --save_to_excel to get a result.xlsx file before filter results'.format(  # noqa: E501
                listfile
            )
        )
...
# Groundtruth name format: 1. TUM name-groundtruth.txt 2. EuRoC name-groundtruth.tum
def generate_groundtruth_list(groundtruth_dir, bagfile_prefix_list, is_tum=True):
    ...
    for name in groundtruth_dir_list:
        ...
        name_suffix_list = name.split('.')
        if len(name_suffix_list) == 2 and name_suffix_list[1] == ('txt' if is_tum else 'tum'):
...
# Groundtruth name format: 1. kitti_data_odometry_gray_sequence_<number>.txt
def generate_groundtruth_list_kitti(groundtruth_dir, bagfile_prefix_list):
    groundtruth_list = list()
    bagfile_index_list = list()
    groundtruth_dir_list = os.listdir(groundtruth_dir)
    for name in groundtruth_dir_list:
        ...
        name_suffix_list = name.split('.')
        if len(name_suffix_list) == 2 and name_suffix_list[1] == 'txt':
            groundtruth_name = name_suffix_list[0]
            if groundtruth_name in bagfile_prefix_list:
                groundtruth_list.append(name)
...
def generate_test_list(bagfile_list, bagfile_prefix_list, groundtruth_list, args_dict):
    ...
    for name in groundtruth_list:
        ...
        item = MonocularBenchmark(
            bagfile_prefix_list[bagfile_index],
            bagfile,
            groundtruth,
            args_dict['ROS1PATH'],
            args_dict['DATASET'],
...
def main():
    ...
    if args_dict['RETEST_FAILURES_ONCE']:
        bagfile_list, bagfile_prefix_list = filter_bagfile_list(
            bagfile_list, bagfile_prefix_list, args_dict['RESULT'] + '/result.xlsx'
        )
    if args_dict['DATASET'] == 'tum':
        groundtruth_list = generate_groundtruth_list(args_dict['GROUNDTRUTH'], bagfile_prefix_list)
    elif args_dict['DATASET'] == 'euroc':
        groundtruth_list = generate_groundtruth_list(
            args_dict['GROUNDTRUTH'], bagfile_prefix_list, False
        )
    elif args_dict['DATASET'] == 'kitti':
        groundtruth_list = generate_groundtruth_list_kitti(
            args_dict['GROUNDTRUTH'], bagfile_prefix_list
        )
    benchmark_list = generate_test_list(
        bagfile_list, bagfile_prefix_list, groundtruth_list, args_dict
    )
    ...
    for item in benchmark_list:
        print(
            'Test Name: {}\nBagfile: {}\nGroundtruth: {}\n'.format(
                item.name, item.bagfile, item.referfile
            )
        )
    ...
    for item in benchmark_list:
        ...
        for test_loop in range(args_dict['REPEAT']):
            ...
            if status != 0:
                ret = -1
                print(
                    'Test "{}" failed due to timeout or some other reason, jump to next case'.format(  # noqa: E501
                        item.name
                    ),
                    file=sys.stderr,
                )
            ...
            if status != 0:
                ret = -1
                print(
                    'The result evaluation of ape test "{}" is failed'.format(item.name),
                    file=sys.stderr,
                )
                continue
            ...
            if args_dict['RPE']:
                ...
                if status != 0:
                    ret = -1
                    print(
                        'The result evaluation of rpe test "{}" is failed'.format(item.name),
                        file=sys.stderr,
                    )
                    continue
    ...
    for item in benchmark_ape_rmse_dict:
        ...
        # only if the --rmse if specified as a positive value in argument,
        # it will judge whether the RMSE is larger than that value.
