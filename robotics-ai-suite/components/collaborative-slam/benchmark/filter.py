...
Time: 5/18/2023
Usage: Filter invalid test results from result.xlsx
       which is the output of benchmark when setting arg: "--save_to_excel".
       This script will output mean value in mean sheet
       and list the data that need to be tested again in filtered sheet.
       Then go back to run benchmark script with arg:
            "--dataset_sublist" to test the data in filtered sheet again.
"""
...
def average_scale_correction(scale_correction_list):
    scale_correction_list = list(
        filter(lambda n: n not in ['None', 'none', 0, None], scale_correction_list)
    )
    if len(scale_correction_list) == 0:
        return None
    res = sum([abs(float(item) - 1.0) * 100.0 for item in scale_correction_list]) / len(
        scale_correction_list
    )
    return round(res, 6)
...
def args_parse():
    ...
    parser.add_argument(
        ...
        '--traj_coverage_thre',
        type=str,
        dest='traj_coverage_thre',
        help='specify the threshold of trajectory ratio',
        default=0.6,
    )
    parser.add_argument(
        '-m',
        '--tum_euroc_ape_threshold',
        type=str,
        dest='tum_euroc_ape_threshold',
        help='specify the threshold of ape for filtering mono&stereo datatype',
        default=0.15,
    )
    parser.add_argument(
        '-k',
        '--kitti_ape_ratio_threshold',
        type=str,
        dest='kitti_ape_ratio_threshold',
        help='specify the threshold of ape for filtering KITTI data',
        default=1,
    )
...
if __name__ == '__main__':
    ...
    # exclude kitti data
    mono_ste_traj_filtered = data_traj_filtered[
        ~(data_traj_filtered['dataname'].str.contains('kitti'))
    ]
    # Group rows by ['dataset', 'dataname']
    g_mono_ste_traj_filtered = mono_ste_traj_filtered.groupby(['dataset', 'dataname'])
    mono_ste_invalid = mono_ste_traj_filtered[
        mono_ste_traj_filtered['ape']
        > g_mono_ste_traj_filtered['ape'].transform('min') + tum_euroc_ape_threshold
    ]
    ...
    kitti_traj_filtered = data_traj_filtered.loc[
        data_traj_filtered['dataname'].str.contains('kitti')
    ]
    ...
    filtered_std_deviation = std_deviation[
        std_deviation['std'] / std_deviation['mean'] > kitti_ape_ratio_threshold
    ]
    filtered_std_list = []
    for idx, row in filtered_std_deviation.iterrows():
        filtered_std_list.append([row[0], row[1]])
    # second, for each group with large std value,
    # filter the rows with 'ape' much larger than min 'ape' of the group
    kitti_invalid = kitti_traj_filtered[
        kitti_traj_filtered['ape'] / g_kitti_traj_filtered['ape'].transform('min')
        > 2 * kitti_ape_ratio_threshold
    ]
    ...
    data_valid_enough = data_valid.groupby(['dataset', 'dataname'], as_index=False).filter(
        lambda x: x['ape'].count() >= 3
    )
    data_valid_not_enough = data_valid.groupby(['dataset', 'dataname'], as_index=False).filter(
        lambda x: x['ape'].count() < 3
    )
    data_mean = data_valid_enough.groupby(['dataset', 'dataname'], as_index=False).agg(
        {'ape': mean, 'rpe': mean, 'scale': average_scale_correction}
    )
