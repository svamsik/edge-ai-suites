...
class StereoInertialBenchmark:
    ...
    def __generate_output_data(self):
        xlsxdata = {
            ...
            'scale': [
                self.scale_correction_list[-1]
                if self.evaluate_ape
                else self.scale_correction_list[-2]
            ],
    ...
    def average_scale_correction(self):
        return sum([abs(float(item) - 1.0) * 100.0 for item in self.scale_correction_list]) / len(
            self.scale_correction_list
        )
    ...
    def average_ape_scale_correction(self):
        return sum(
            [
                abs(float(item) - 1.0) * 100.0
                for idx, item in enumerate(self.scale_correction_list)
                if idx % 2 == 0
            ]
        ) / (len(self.scale_correction_list) / 2)
...
def args_parse(default_bagfile_dir):
    parser = argparse.ArgumentParser(
        description='Automatically run EuRoC dataset with stereo and imu input and print the accuracy data to standard output'  # noqa: E501
    )
    parser.add_argument(
        'dataset',
        type=str,
        help='The dataset to be tested, only support option: euroc',
        choices=['euroc'],
    )
    parser.add_argument(
        '-b',
    ...
    parser.add_argument(
        ...
        help='The threshold of scale correction percentage to pass a single test',
        default=5.0,
    )
    parser.add_argument(
        '-w',
...
def filter_bagfile_list(bagfile_list, bagfile_prefix_list, listfile):
    ...
    for bag, prex in zip(bagfile_list, bagfile_prefix_list):
        if ['stereo_imu', prex] in filtered_list:
    ...
    for item in benchmark_scale_correction_dict:
        print(
            'Test Name: {}\nScale correction percentage: {}\n\n'.format(
                item, benchmark_scale_correction_dict[item]
            )
        )
        if float(benchmark_scale_correction_dict[item]) > args_dict['SCALE']:
            ret = -1
            print(
                'Scale correction percentage error bigger than allowed {}\n'.format(
                    args_dict['SCALE']
                )
            )
