...
protected:
  ...
  ProcInfo(const std::string & name, bool verbose)
  : name(name), count(0), totalTimeNs(0), verbose_(verbose)
  {
  ...
  {
    long long timeNs =
      (time.tv_sec - startTime_.tv_sec) * 1000000000ULL + time.tv_nsec - startTime_.tv_nsec;
    totalTimeNs += timeNs;
    count++;
    if (verbose_) {
      spdlog::debug("[LoopTimer] {0} spent {1:.2f} ms\n", name, (double)timeNs * 1e-6);
    }
  }
...
public:
  ...
  inline void finish()
  {
    ...
    started_ = false;
    clock_gettime(CLOCK_MONOTONIC, &now_);
    totalTimeMs_ = (now_.tv_sec - startTime_.tv_sec) * 1000 +
                   (now_.tv_nsec - startTime_.tv_nsec) / (double)1000000;

    // Format result here so it only runs once even if user calls result() for multiple times
    formatResult();
  }

#else   // #ifndef DISABLE_TIMER
  ...
  inline void finish()
  {
    ...
    started_ = false;
    clock_gettime(CLOCK_MONOTONIC, &now_);
    double totalTimeMs = (now_.tv_sec - startTime_.tv_sec) * 1000 +
                         (now_.tv_nsec - startTime_.tv_nsec) / (double)1000000;

    // Format result here
    result_ = (boost::format("Totel elapsed time: %1$.2lf s  Frequency: %2$.1f Hz\n") %
               (totalTimeMs / 1000) % (count_ * 1000 / totalTimeMs))
                .str();
  }
#endif  // #ifndef DISABLE_TIMER
  ...
  inline void formatResult()
  {
    ...
    if (!procs_.empty()) {
      ...
      for (current_ = procs_.begin(); current_ != procs_.end(); current_++) {
        if (current_->name.size() > nameLen) {
          nameLen = current_->name.size();
        }
      }
    ...
    ss << boost::format("Total elapsed time: %1$.2lf %2$3s    Frequency: %3$.1f Hz\n") %
            ((totalTimeMs_ > 1000) ? totalTimeMs_ / 1000 : totalTimeMs_) %
            ((totalTimeMs_ > 1000) ? "s" : "ms") % (loop_.count * 1000 / totalTimeMs_);
    result_ = ss.str();
  }
